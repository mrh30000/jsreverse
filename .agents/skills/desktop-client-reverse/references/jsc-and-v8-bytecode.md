# `.jsc` 的**三类**形态：Cocos xxtea、V8 字节码、Mozjs 字节码
...
> `52pojie-2115726`（Typora 授权逻辑在 JSC；**V8 JSC → JS 的三条现成路线**，§5.3 的来源）、
> **`52pojie-814217`（Cocos2d-js 手游，**Mozjs / SpiderMonkey 字节码**：反编译 + 就地改字节码，§10）**。
...
| **V8 字节码 / code cache** | Node/Electron 的 `bytenode`、`vm.Script({cachedData})`、`CodeSerializer` | 头部 4 字节以 **`C0 DE`** 开头（`2115726` 实测 `C0 DE 06 87`）；权威判据仍是"交给 V8 反序列化" | 本文件 §5：**先试"不解字节码"**（劫持运行时），要看懂再走 §5.3 |
| **Mozjs / SpiderMonkey 字节码** | **Cocos2d-js 手游**（旧引擎线，实测 2018 年产物） | 同为 Cocos 系、但 **`xxtea_decrypt` 出不了东西**（它不是密文，**本身就是字节码**） | 本文件 **§10**：可反编译（`jsc-decompile-mozjs-34`），也可**就地改字节码** |

**铁律**：拿 xxtea 去试 V8 字节码、或拿字节码去猜 xxtea 密钥，都是浪费时间；
先按上表定性，再选路线。

⚠️ **第 3 类是最容易被漏掉的一类**：它和 Cocos 系**在同一个目录、同一个后缀、同一个引擎家族**里，
唯一的区分动作是"**先用 xxtea 试一次，解出来不像 JS 就改判为字节码**"——
不要因为"这是 Cocos 游戏"就直接断定是 xxtea 密文（§10 的来源就是先踩了这个坑）。

> ⚠️ **别把 `C0DE` 当成版本无关的常量**：`2115726` 实测的是 `C0 DE 06 87`，
> 而 V8 的 code cache 头把**低 16 位编码成版本相关值**（**推断**：来源只给了这一个观测值，
> 未说明低位含义）⇒ 工具只允许把它当**前缀**判据（`--magic C0DE`），
> 不要写"首 4 字节必须等于 0xC0DE0687"。
...
### 5.3 要看懂时的三条现成路线（`2115726`，成本已比 §5.1 低一个量级）

**前置条件（原文点名"最重要的一点"）：先拿到目标用的 V8 版本。**
原文的做法是**把 Electron 开调试，然后在 console 里读版本**；
版本必须与 `.jsc` 的构建版本一致，否则 `Deserialize` 直接失败（§5.1 的坑 6）。

| 路线 | 做法 | 代价 / 边界 |
| --- | --- | --- |
| ① 现成工具链 `jsc2js`（基于 `view8`） | 直接喂 `.jsc`；工具内置了一批 V8 版本（原文用到 `13.4.114.21`） | **零编译成本**；产物可读性差（原文："很像 IDA 的 F5"） |
| ② `d8` + `view8` 两段式 | `.\d8.exe -e "loadjsc('C:/path/atom.jsc')" > disasm.txt` → `python view8.py --disassembled disasm.txt out.decompiled.js` | 这条 `d8` **就是 §5.1 那个 `LoadJSC` 补丁的成品**——原作者已编好，不必自己补 |
| ③ AI + CDP 动态调用 | AI 静态读 + 用 CDP 控制目标**实际调用**函数，把被触发的函数变可读 | 可读性最好，但**只能覆盖被触发的函数**；没触发就永远不知道 ⇒ **覆盖不了整个文件** |

原文另外两条判断：
- 有网站提供"指定 V8 版本的 JSC → JS"的**付费**服务，作者判断"**不值得特地给钱去弄**"（`jsc2js` 已含所需版本）。
- 作者把整个过程沉淀成了一个 skill（`1903247335/jsc-lifter`）⇒ **这是一条已经工具化的路**，
  不再是 §5.1 那种"一个下午起步"。

**路线选择（把 §5.2 与本节接起来）**：
1. 目标是**拿业务逻辑 / 绕过校验** ⇒ 仍然优先 §5.2（劫持运行时）：**天然正确、零还原误差**。
2. 目标是**看懂关键分支**（授权判定在哪、常量是什么）⇒ 走本节 ①②，必要时叠加 ③。
3. **不要**为了"把整个 `.jsc` 还原成源码"投入 —— 三条路都给不出完整源码（③ 明确只能覆盖触发路径）。

**这条判据同时也是"是否加壳"的判据**：原文看到首 4 字节是 `0xC0DE0687` 时的第一反应是
"**庆幸没有加壳**" ⇒ 头部仍是完整的 V8 code cache 头，说明作者没有额外包一层壳；
头 4 字节不是 `C0 DE` 开头时，先怀疑"外面还有一层包装"，而不是"V8 版本不对"。
...
| 11 | 走 §5.3 反编译却**没对齐 V8 版本** | 先开调试读版本；版本不符 ⇒ `Deserialize` 失败（不是"文件坏了"） |
| 12 | 把 `0xC0DExxxx` 的低 16 位当固定值 | 只当前缀判据；低位随版本变（**推断**）⇒ 写死 `0xC0DE0687` 会在别的构建上误判 |
| 13 | 以为 §5.3 能产出"完整源码" | ③ 只覆盖被触发的函数；要完整源码是伪需求，改走劫持 |
...
* ❌ 不要"因为 §5.3 有现成工具链，就先反编译再分析"。§5.3 的定位是**看懂**；
  拿业务逻辑仍然先走 §5.2。**顺序反了会白白多花几天。**
* ❌ 不要拿 §5.3 的产物（可读性差的伪代码）当"源码"去改并回写 ——
  改回 `.jsc` 需要重新编译字节码，**不可行**；要改行为就走运行时劫持。

## 10. Mozjs / SpiderMonkey 字节码：**可反编译**，也可**就地改一个字节**（`52pojie-814217`，**单源**）

### 10.1 怎么认出它（不要因为"是 Cocos 游戏"就直接上 xxtea）

来源的路径是：**先按 Cocos 系处理 → `xxtea_decrypt` 出不了东西 → 改判"它本身就是字节码"**。
所以判据只有一条可操作动作：

> **先用 `jsc_xxtea_tool.py` 试一次；解出来不像 JS、且文件里找不到 xxtea 密钥线索
> ⇒ 改判 `Mozjs（SpiderMonkey）字节码`。**

（对比 §2：Cocos xxtea 系"错密钥也会返回随机字节"，所以**不能只靠"输出不像 JS"**；
§2 的做法是**从 `libcocos*.so` 里取密钥**。若"密钥取证三处落点"全部落空，
才轮到本节这条路。）

### 10.2 反编译：`jsc-decompile-mozjs-34`

```bash
php run.php GlobalDefine.jsc > test.txt      # 项目：jsc-decompile-mozjs-34
```

| 要点 | 值 |
| --- | --- |
| 项目 | `irelance/jsc-decompile-mozjs-34`（**PHP 项目**，需 PHP 环境 + composer 导入依赖） |
| **版本对齐** | **SpiderMonkey 34**。来源原话："**一定要分清楚，应用不同的版本型号字节码解析也不同**" |
| 字节码对照表 | MDN `SpiderMonkey/Internals/Bytecode` |
| 产物性质 | **不是逐一还原**（来源自述"项目还不是很完善，很多字节码没有完全解析，不过一般还是够用"） |

⇒ **与 §5.3（V8）同型**：反编译产物用来"**看懂**"，不是"能重编译回去的源码"。

### 10.3 🔴 本形态最有价值的部分：**就地改字节码**

不必反编译再重编译（不可行），**改一个字节就能改行为**。原文解释的指令形态如下。

🔴 **操作数字节序：32 位字段是「高位在前」**（**这条是用 browsercli 在真实 JS 引擎里跑出来的修正**）：
原文写 `3B 00 00 00 11` 的语义是「取**下标 11** 的变量」，`44 00 00 00 2A` 的跳转偏移是 `0x2A` ——
⇒ 四个字节里的**有效值在最后**（`00 00 00 11` 就是 `0x11`），**不是**小端（小端会读成 `0x11000000`）。
⚠️ 来源只给了**小值**样例（`0x11` / `0x2A`），大值的字节序属**推断**；引用时不要当成已验证的通用结论。
（对比：`D7` 是「压 8 位整数」，操作数只有 1 字节，没有字节序问题。）

| 字节码 | 含义 | 备注 |
| --- | --- | --- |
| `3B 00 00 00 11` | 取**下标 11** 的变量压栈 | 32 位操作数；`11`=17=索引 |
| `D7 64` | 压入**8 位整数** `0x64`(100) | 来源注：16 位用 `58 10 10`、32 位用 `d8 00 00 00 11` |
| `12` | **相等**判断 | `13` 不等 / `14` 小于 / `15` 小于等于 / `16` 大于 / `17` 大于等于 |
| `44 00 00 00 2A` | **条件跳转**（32 位偏移 `2A`） | 栈顶为真则跳转 |
| `51` | 弹出栈顶 | — |
| `6E 00 00 00 CA <B>` | **设置布尔值** | **`CA 42` = false、`CA 43` = true** |

**原文给出的完整对照**：

```
3B 00 00 00 11  D7 64  12  44 00 00 00 2A  51   ⇒   GAME_INFO_CHANNEL == 100
3B 00 00 00 11  D7 65  12  44 00 00 00 2A  51   ⇒   GAME_INFO_CHANNEL == 101
3B 00 00 00 11  D7 66  12  44 00 00 00 2A  51   ⇒   GAME_INFO_CHANNEL == 102
```

**补丁实例（原文）**：把 `6E 00 00 00 CA 43 6F 00 00 00 00 C6 98`
里的 **`43` 改成 `42`** ⇒ `IS_USE_HTTP_ENCRYPT` 由 `true` 变 `false`
（效果：**不再使用 http 加密，明文传输**）。

### 10.4 工作方法（可迁移到任何"字节码数组 + 常量"场景）

1. **先反编译一遍拿到 `test.txt`** —— 它是**给字节码定位用的地图**；
2. **在 010 Editor 里搜目标常量**（本例搜 `d7 64`，因为目标是"和 100 比较"）；
3. **把附近十几字节贴出来，按指令表逐条"读成伪代码"**（§10.3 的表）；
4. **改** —— 布尔位改 `42`/`43`；整数改 `D7` 后的那个字节；
5. 替换回游戏验证。

**为什么值得单列**：这条路线把"改游戏行为"从"重编译（不可行）"降级为
"**找一个字节、改一个字节**"，且**判据是机械的**（对照表 + 常量搜索）。

### 10.5 本节的边界与坑（**单源**，引用时必须带限定）

| # | 坑 / 边界 | 说明 |
| --- | --- | --- |
| 1 | **`D7 64` 里的 `64` 是"100"这个值，不是固定格式** | 按目标值重算操作码宽度（8/16/32 位三种编码），别照抄 |
| 1b | **32 位操作数不要按小端读** | 原文 `3B 00 00 00 11` = 下标 `0x11`（**高位在前**）；按小端会得到 `0x11000000`（B27 由真实 JS 引擎复算发现） |
| 2 | **指令表来自单源 + MDN 对照** | 来源只逐条解释了上面这几种；**未给完整表** ⇒ 长指令（如 `D8`/`6E`）的"长度"要回 MDN 查 |
| 3 | **反编译产物不完整** | "很多字节码没有完全解析" ⇒ 只能当**地图**，误差靠"常量搜索 + 局部读码"兜 |
| 4 | **改之前必须备份** | 等长替换（与 §6 pkg 补丁同一铁律）；改动范围越小越好 |
| 5 | **来源未给完整工程** | 原文特意说明"游戏开发商心太大…**这里不能公布他的这个文件**" ⇒ 本节的字节码样例**没有可复跑的原始样本**，只能作**格式参考** |

**反例**：

- ❌ 不要因为"是 Cocos 游戏"就直接上 xxtea（来源自己先踩了这一步）；**解不出来就改判字节码**。
- ❌ 不要照抄 `d7 64`（那是"100"）。要改的常量不同 ⇒ 操作数编码与字节数都可能不同。
- ❌ 不要指望反编译产物能重编译回 `.jsc`（与 §5.3 同型）；**改行为就走就地补丁**。
- ❌ 不要把本节当作"完整指令表"使用 —— 它是**单源 + 局部**，其余指令回 MDN 对照。
