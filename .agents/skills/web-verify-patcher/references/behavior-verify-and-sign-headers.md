...
| 19 | 参数在浏览器里对、换脚本就**稳定**失败，于是反复调参数 | ★ **先做「拿到参数立刻发 vs 等一会再发」的对照**：结果不同 ⇒ 触发条件**与参数无关**（速率/画像类风控），调参数是白费（`baidu-yunjiasu-safety-check.md` §2.7） |
| 20 | 只看「成功/失败」，不看**错误码分层** | 同一站点的错误码常常是**画像读数**（如 `130049` 直接拒 / `50052` 降级滑块 / `0` 通过）⇒ 用它判断「改动是变好还是变坏」（同上） |
| 21 | 把**求助帖**里的参数当成「必需参数 / 生成公式」登记 | 提问者自己都没解决 ⇒ 只登记**被实证的那一条变量**，其余全部按「未证实」处理（`../../web-reverse-algorithm/references/16-ciphertext-structure-diagnostics.md` §5 纪律） |
