...
6. 用户从 `solution_options` 中选择方案并明确确认后，进入第二阶段：
   ...
   - **拆解极验的提交参数与交互链路（v3 三个 `w` 的分工与七步链路、v4 的 `load`/`verify` 两接口、`w`/`td`/`td_sign`/PoW/动态键名防篡改块、v3 52 片底图几何与轨迹 `aa` 编码、九宫格协议），或排查「极验验证通过却登录失败」「三个 `w` 有一个不对就 forbidden」这类问题时，必须读 `references/geetest-protocol-matrix.md`。**
