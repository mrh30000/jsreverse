...
## 6B. ★★★ 「静态一个字都搜不到」时的判据与路线（`52pojie-1471424`，某生鲜电商 `mfsig`）

**现象**：抓包拿到 `mfsig`，在反编译出来的小程序工程里**搜 `mfsig` 无结果**；
换成**值的前缀 `mfsw`** 再搜，**同样无结果**。

> ★★★ **判据**：**"参数名与值前缀都搜不到" ⇒ 该函数被针对性混淆过，不是"没找到"。**
> ⇒ **立刻切动态调试**，不要在静态上继续加关键词（源文原话：
> 「该小程序的加密相关函数是特殊对待混淆过的，静态分析明文无法定位到。算是一个安全性比较好的 case」）。

### §6B.1 动态路线（源文的完整动作序列，可照抄）

```text
① 用小程序开发者工具打开反编译后的工程
② 右上角「详情 → 本地设置」勾选：
   不校验合法域名、web-view（业务域名）、TLS 版本以及 HTTPS 证书
③ 点「编译」，按 Console 报错逐个修（源文自陈「会遇到几个很小的报错，逐步解决」）
④ 找到目标接口 → **在发请求处打断点** → 单步跟
⑤ 反编译代码**多行挤在同一行** ⇒ 点左下角 `{}` 格式化后再跟
⑥ ★ 跟的过程**用纸记**下「哪一步做了什么」——混淆过的代码靠"读"读不出来，靠"记流程"记出来
⑦ 把记录的流程翻译成 Java/Python，**拿一条真实抓包的请求体验证**（密文逐字节相等才算完）
```

> ★★ **本 case 的算法性质**：源文原话「**大体上来看其实就是编码游戏，字符串转数组，数组转为字符串，
> 再利用索引进行字符编码生成最后的 mfsig**」⇒
> **它不是"标准算法 + 盐"**，而是**索引编码**。**判据**：动态跟下来发现"没有任何已知算法名（AES/MD5/SHA）
> 出现，只有数组/字符串来回转" ⇒ 就按"编码表 + 索引"来理解，**别硬套算法名**。

### §6B.2 分包（本 case 的取件前提）

该小程序解包后目录下有**四个包**：主包（3.3M）、两个子包（1.5M / 89k）、**基础依赖包（14M）**。

```bash
# 先反编译主包
node wxappUnpacker/wxWxapkg.js <主包.wxapkg>
# 子包必须 -s= 指向主包路径，让子包内容并进主包目录
node wxappUnpacker/wxWxapkg.js -s=<主包目录> <子包.wxapkg>
```

> ★★ **判据**：单包体积上限 4M ⇒ **工程大就必然分包** ⇒
> **只反编译主包会缺文件**（业务代码常在子包里）。
> ⇒ 反编译顺序：**先主包 → 再逐个 `-s=` 子包**；顺序反了会出现同名文件覆盖。
> ★ 分包细节与工具对照见 `./unpack-and-decrypt.md` §2 与 §5。
