...
> `52pojie-974827`（消灭病毒**更早**的独立来源：`code.js`、sign 按类型选密钥）、
> `52pojie-965556`（电影小程序 `check` + AES-ECB，含"弯道超车"调试法）、
> `52pojie-1775370`（token 构造求助帖：未证字段的定位与登记范式）、
> `52pojie-2058459`（欢乐麻将：响应体改写的边界）、
> `52pojie-828363`（我要当皇上：明文存档改包 + 抓包定位自建服务器）、
> `52pojie-832333`（我要猫咪：Laya 引擎的加密定位链）。
>
...
| `game.js` + 无引擎目录 | **原生 JS 小游戏** | `game.js`（入口）**或 `code.js`**（`52pojie-974827` 的「消灭病毒」核心逻辑就在 `code.js`） | 同小程序：搜 URL 片段 / 参数名 |
| 包内是 `app.json`、有 `pages/` | 不是小游戏 | — | 回 `SKILL.md` 主流程 |

**判据补充（一）**：小游戏包里是 **`game.json`**（小程序是 `app.json`）；
两者共用 `wxapkg` 容器与 `unveilr` / `wxapkg_tool.py`，所以**容器层完全复用**，不需要新工具。

**判据补充（二）**：目录里的 `open/`（启动图、音乐）、`sd/`（音乐）是小游戏**标准目录**，
**不要据此找逻辑** —— 逻辑文件**不一定叫 `game.js`**（974827 就是 `code.js`）。
**最快的方式是拿抓包到的 URL 片段全局搜**，命中的那个文件才是核心
（974827 命中 `code.js`，2081826 命中 `game.js`）。
...
**补充（`52pojie-974827`）：也可能"根本跑不起来"，但静态照样能做。**
解包工程进微信开发者工具后，**可能加载到 100% 后无报错、也无法继续**（真机调试同样卡在 100%），
源文未定位原因（原话"不是很懂在哪里卡住了"）。
⇒ **判据：目标是拿到 `sign` 算法，不是把游戏跑对。** 974827 直接放弃运行，
改用全局搜 `archive/upload` 静态定位，照样还原了 `sign`（见 §5.1 补充）。
**不要在"跑不起来"上死磕。**
...
### 2.1 引擎封装链：业务封装 → 引擎封装 → `wx.*` 桥，逐层搜（`52pojie-832333`，Laya）

上面的"一次命中统一入口"适合手写 HTTP 的小游戏；**引擎类小游戏**（Laya / Cocos）会把网络封装成自己的类，
此时更稳的是**顺着封装一层层搜**（832333「我要猫咪」的实测链）：

| 层 | 搜什么 | 命中 |
| --- | --- | --- |
| ① 从行为推接口 | 退出游戏必走 `save` ⇒ 全局搜 `/user/save` | 业务方法，内有 `HttpPost(f, m, e, b)` 调用 + `a.code` 状态判断 |
| ② 搜业务封装名 | `HttpPost` | 30+ 处**调用**（后面都带 URL）⇒ 要找的是**定义** `function HttpPost(f, m, e, b){}` |
| ③ 搜引擎封装名 | `LayaHttpRequest` | 同样"一堆调用 + 一处定义"⇒ 定义里出现 `wx.decrypt(a, b, c)` |
| ④ 搜桥方法 | `wx.decrypt` | 参数来自导入模块 ⇒ `require("./js/utils/RdWXBizDataCrypt.js")` |
| ⑤ 打开目标文件 | 在 `RdWXBizDataCrypt.js` 里搜 `decrypt` | 最终加解密实现 |

**两条判据**：

1. **"同名多处命中"要分调用与定义**：`HttpPost` 搜出 30 多处，**只有一处是 `function` 定义**，其余都是调用 ⇒
   **命中定义才停下**（与 `runtime-and-debug.md` §2 "打最后一处"是同型问题的两种表现）。
2. **引擎封装是"必经之路"**：业务层不直接写 `wx.request`，而是过 `HttpPost` → `LayaHttpRequest`。
   所以从 URL 片段搜到业务方法后，**继续搜它调用的那个引擎方法名**，不要停在业务层。

> 顺着 `decrypt` 找到实现后，**不必还原 key/iv** —— 可直接用"替换方法体"把它当 oracle
> （做法见 `./request-crypto-and-sign.md` §3「拿 oracle：把方法体直接替换掉」）。
...
**同一游戏的更早独立来源（`52pojie-974827`，2019-06）补充两点**：

- **密钥按 `sign` 类型切换**：源文原话「根据参数的 key 升序排序，**根据 sign 类型，选择不同的密钥**，拼接起来，md5 加密」；
  两把密钥名为 **`secret` / `secret1`**。（与本节"两处接口键集不同"是**两种口径**，是否同一机制源文未说明，**未复核**。）
- **密钥不是明文常量**：源文说这两把密钥是**「解密」一段字符串**得到的，但紧接着自陈
  **「算法原理不细究了」** ⇒ **登记为"源文未展开"**：只记"密钥可由包内代码解出"这一事实，
  **不要替它补算法**（回源时按当下版本重新抠/爆破，见 `./request-crypto-and-sign.md` §4）。
...
### 5.7 找"小游戏自建服务器"与 Charles 必配项（`52pojie-828363`）

小游戏一打开，抓包里会混进一堆**腾讯官方**流量；目标是**它自己的服务器**。828363 给了一套可复用的排除法：

| 手段 | 做法（源文原话口径） |
| --- | --- |
| **域名排除** | 连接是 `qq.com` / 微信域名的**一定不是目标** —— "那些都是腾讯官方的东西，小游戏或者小程序都是**自己的服务器**" |
| **时间窗筛选（两种）** | ① 等请求稳定 → 点 Charles 的**扫帚清空** → 再打开小游戏 → 只看新增；② 先打开游戏 → **关闭前**清空左侧 → 关闭后看剩下什么 |
| **收益** | 退出游戏**必然走一次"保存"请求** ⇒ 用上述窗口就能把它挑出来（828363 靠此定位到 `h5.chiguawan.com:443`） |
| **🔧 工具必配** | Charles **`SSL Proxying Settings` 里必须加 `*`（所有域名）** —— 源文原话「**这一步一定要有，否则就算信任了证书也全都是 unknown**」 |

⚠️ 第 4 行是**工具侧坑**，与"证书装没装"无关：证书信任只解决"能不能解密"，**没配 SSL Proxying 就是不解密** ⇒ 全 `unknown`。
...
**请求侧同理**：若抓到的存包请求体**本身就是明文**，可直接改**请求体**重放（`52pojie-828363`），
不必改响应 —— 但有**版本字段 / 自动保存 / 非 200 重取**三个时序坑，
见 `./request-crypto-and-sign.md` §6「无签名的明文存档变体」。
...
| 小游戏工程**加载到 100% 后无报错、进不去**（真机同样） | 源文未定位原因（974827） | **别死磕跑起来**：直接全局搜 URL 片段静态读核心 JS（如 `code.js`，§1 补充） |
| Charles 抓包**全是 `unknown`** | 没配 `SSL Proxying Settings` | 里面加 **`*`（所有域名）**，与证书信任是两回事（§5.7） |
| 抓包里**找不到小游戏的业务域名** | 混着一堆腾讯官方流量 | 排除 `qq.com`/微信域名 + 清空列表看新增（§5.7） |
| 同一个方法名搜出几十处，分不清哪个是入口 | 命中里大多是**调用** | 只认 `function` **定义**那一处；再顺它调用的**引擎封装名**继续搜（§2.1） |
...
- **不要卡在"小游戏跑不起来"上。** 974827 的工程**加载到 100% 后无报错也进不去**（真机同样），
  源文直接放弃运行、改用全局搜 URL 片段静态定位 `sign`。目标是**拿到算法**，不是**把游戏跑对**（§1 补充）。
- **不要把"明文存档"当成"一发就成"。** 明文只省掉"重算 sign"这一步，**版本字段 / 自动保存 / 非 200 重取**
  三个时序坑照旧（§7 请求侧 + `./request-crypto-and-sign.md` §6）。
- **不要在引擎类小游戏里停在业务层。** 业务层不直接调 `wx.request`；**继续搜它调用的引擎封装名**
  （`HttpPost` → `LayaHttpRequest` → `wx.decrypt`），否则永远找不到最后的加解密实现（§2.1）。
