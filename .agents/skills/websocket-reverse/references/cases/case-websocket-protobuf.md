...
## 已实证的一类：自定义「会话信封」（长度字段 = 帧总长 − 6）

> 来源 `52pojie-1926836`（源文是**求助帖**，作者问「`o.setUint` 那段什么意思」，**没有给结论**）。
> 下面全部结论由本库**从源文贴出的 20 帧原始 hex 反推 + 逐帧断言**得到：
> `artifacts/skill-evolution/tools/b39-ws-frame-check.js`（60 项断言全绿）、
> 解码器 `pb_decode_raw.py --frame ws-env`（53 项自检含 5 条本族断言）。
> 完整结构说明见 `../../../protobuf-reverse/references/framing-and-transport.md` §1b。

**为什么值得单列**：这是「**帧头形状随帧变化**」的一类 —— 如果按固定偏移切，会**静默切出垃圾**
（不报错，只是字段看着莫名其妙）。本族的三条等式必须自己算：

```text
① buffer 大小 = 6 + n，n = 5 + r          ⇒ 头部 11 字节、payload 起点 11
② 长度字段 [2:4] == 帧总长 - 6            ⇒ ★ 不是「payload 长度」也不是「帧总长」
③ r [9:11] == payload 字节数              ⇒ 且 11 + r == 帧总长（与 ② 互为交叉校验）
```

**本批实测的族分布（20 帧，守卫断言 11 + 5 + 4 == 20）**：

| 族 | 帧数 | 判据 | 状态 |
| --- | --- | --- | --- |
| 心跳 | 4 | 帧长 6、长度字段 0（`4E20…` 发 / `4E21…` 收） | ✓ |
| 业务 | 11 | `msgId == 0x24`（含 `0x23`），`11 + r == 帧长`，payload 递归解析恰好消费完 | ✓ |
| **另一族** | **5** | `msgId ∈ {0x14,0x15,0x16,0x17}`：**长度等式 ② 照样成立**，但 `[9:11]` 不是 payload 长度 | ✗ **未解** |

★★★ **本类最值钱的一条可迁移判据**：

> **长度等式只能定位「帧边界」，不能定位「payload 起点」。**
> 上面第三族的 `6 + 长度字段 == 帧长` **完全成立**（88 字节帧：`6 + 0x52 == 88`），
> 但按偏移 11 切出来的 payload 一解析就报 `wiretype 7` / `field 0`。
> ⇒ **判 payload 起点的唯一硬判据仍是「切完能恰好解析完」**（与本技能 §「常见失败与回退」同源）。
> ⇒ 因此 `--frame ws-env` 的实现里**写了一条会主动报错的护栏**：
> 不满足 `11 + r == 帧长` 就 `raise`，**绝不返回一段"看起来还行"的 payload**。

★ **真实数据的正向验证**：本库把源文里最大的一条帧（**11886 字节**）喂给 `--frame ws-env`，
解出 `{"1": 1, "2": [ {1: id, 2: id, 3: "<中文道具名>", 5: n, 6: n, 7: 时间戳} ×156 ]}`
—— 结构自洽（156 个同类条目、字段集完全一致）⇒ **这是「帧切对了」的强证据**，
比「字段看着像」硬得多（切错的帧不会产生 156 条同构记录）。

★ **本例还有一条 OB 混淆线索**：源文贴的实现里，字符串被写成
`b('0x172', ')e4a')` 这种「**索引 + 第二参数**」形态（obfuscator.io 的 `stringArrayEncoding`），
且另有 `f['vtYVa']` / `v['WGwHb']` 这类**随机名别名表** ⇒
**先做字符串还原与别名表展平，再读字段号**（交给 `../../../ast-deobfuscation/SKILL.md`）。
