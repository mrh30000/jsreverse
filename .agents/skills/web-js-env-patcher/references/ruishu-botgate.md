...
5. **动态脚本里的 `debugger` 用中间人改 `eval` 的实参去掉**（`52pojie-1208999`）：瑞数的 `debugger` 不在静态外链 JS 里，
   而在**解密后准备送进 `eval` 的字符串**里；在 `eval` 执行前把实参替换成"删过 `debugger` 的副本"即可，
   **完全不需要知道解密算法**。配方（正则定位 + 两步注入）见
   `../../web-reverse-algorithm/references/07-antidebug-and-live-patching.md` §5.7 ①；
   该源文另给了一条 **★ 静态点判据**：动态生成的东西里，**二级数组的"长度不变、位置语义固定"**
   （源文例：`[1][14]` 这个**位置**固定用于拼出"动态字符串"，而该字符串又用来构建静态外链脚本的**变量名**）
   ⇒ 这正是"**静态脚本被动态化**"的机制。源文原话的推论是：
   「任何动态的事物都是基于静态产生的，所以只要找到静态点，那么动态就会坍缩为静态」。
   ⚠️ 源文自陈该站反爬**更新频繁、不保证时效性**（2020-06 的样本），判据可迁移、**具体形态不可迁移**。
