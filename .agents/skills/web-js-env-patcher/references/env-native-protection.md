...
9. **特殊对象**：`document.all` 这类 HTMLDDA / 不可检测对象必须优先使用 addon `createUndetectable`；JS fallback 只能标记为近似，不得声称完全一致。
   - **零依赖 fallback（addon 不可用时的有记录降级）**：用 V8 natives syntax 取 undetectable 对象，可在纯 Node 下得到近似语义。
     ...
     验证必须同时满足：`typeof document.all === 'undefined'` 且 `document.all !== undefined`。
     仅检查 `typeof` 会把普通对象误判为通过。该方法依赖 V8 私有开关，属于 fallback，需在阶段报告记录原因。
