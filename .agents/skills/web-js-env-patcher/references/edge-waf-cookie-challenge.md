...
#### 未定名厂商：三层 JS 链 + 503 表单 + **概率性通过**（`52pojie-1500547`）

**症状**：正常浏览器打开会显示「Please allow up to 5 seconds」；用抓包工具看是**五次访问**：

| 次 | 请求 | 关键点 |
|---|---|---|
| 1 | 首访 | 返回 **503**，响应体里带**待提交的表单**与其它 JS 内容 |
| 2 | 第 1 层 JS | **OB 字符串混淆 + 控制流平坦化**；内容是**通用函数**（含请求体的加解密函数） |
| 3 | 第 2 层 JS | 同样混淆；**链接来自上一个 JS**（第 1 层跑出来才拿到地址）；「这是整个反爬框架**最核心**的部分，运行后获得后续请求的上下文」 |
| 4 | 第 3 层 JS | 同样混淆；**提交上一层给的参数**：通过 ⇒ **激活第 1 次请求的那个表单**；不通过 ⇒ **自动刷新，从第一步重来** |
| 5 | 校验通过 | 状态码变成 **200** |

> ⚠️ **源文未点名厂商**（只给了两个测试站点的 base64）。⇒ **不要往 jsl / 5s 盾 / CF 上套名字**，
> 按 §1 的判层工具先跑一遍；判不出来就按本节结构处理。**"三层 JS + 503 表单 + 通过后激活表单"是结构，不是厂牌。**

**处置路线（源文原话的三步）**：

```
获取第二层 JS → AST 反混淆与函数前置 → JS 层运行拦截结果 → Python 修改拦截结果部分内容 → 发送请求
```

- **环境依赖（必须先做）**：第二层 JS **大量检测 `dom`、`nodejs` 以及其它自动化工具**
  ⇒ 源文明确要求「**需要自己补浏览器环境头**」。补环境链路见本技能 `workflow.md` / `env-module-levels.md`，
  这里不重复。
- **AST 反混淆** → `../../ast-deobfuscation/references/ob-variant-taxonomy.md`（本族两层都是 OB 混淆）。

**★★ 边界：这是概率性通过，不是确定性算法。** 源文原话：
「通过的记录，**目前在 5 次内最少有一次成功**」。⇒ 交付时**必须**如实说明成功率，
**不要**把「跑通一次」写成「稳定可复现」（与 `../SKILL.md` 的「连续 N 次」验收口径同一条）。

**两条源文未解的求助（原样登记，不得当成结论）**：

1. **同一站在浏览器里是 5 秒、在 Python 里却出现「带 hcaptcha 的 5 秒」**；且**把协议改成 `http` 则必定**
   出现带 hcaptcha 的 5 秒 ⇒ **引擎/协议侧行为差异**（源文未解释，**未复核**）。
   ⇒ 落地含义：**先确认你拿到的到底是"纯 5 秒"还是"5 秒 + hcaptcha"**，两者的路线完全不同。
2. **另一站的搜索页在浏览器与 Python 中「均」出现带 hcaptcha 的 5 秒** ⇒ 源文问「如何通过人工智能进行打码」。
   ⇒ **这类题不在本技能能力面**，移交 `../../web-verify-patcher/references/provider-execution-notes.md`
   （hcaptcha 相关条目按该技能口径处理）；本技能只负责到「拿到 challenge 并识别出它是 hcaptcha」为止。
...
| 20 | 挑战是**三层 JS 链**、首访 **503 带表单**、通过后表单才被激活 | §2.7 未定名厂商小节：按「获取第 2 层 JS → AST 反混淆 → JS 层运行拦截结果 → Python 改拦截结果」走；**必须补浏览器环境头**（该族大量检测 `dom`/`nodejs`/自动化工具） |
| 21 | 同一个站在浏览器里是「纯 5 秒」、在 Python 里变成「5 秒 + hcaptcha」 | §2.7 未解项 1：**引擎/协议侧行为差异**（源文未解释）。先确认你拿到的是哪一种；带 hcaptcha 的那一类移交 `../../web-verify-patcher/` |
| 22 | 挑战**时好时坏**，跑通一次不代表能复现 | §2.7：该族可能是**概率性通过**（源文实测「5 次内最少有一次成功」）⇒ 交付要报成功率，不能报「已稳定复现」 |
