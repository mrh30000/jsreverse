...
### 滑块「拟人化」的第二层假象：连延时都不随机

> **来源** `52pojie-2051638`（2025-08 某东滑块）+ `52pojie-1159541`（2020-04 B 站滑块）+ `52pojie-877390`（2019-02 京东登录）。
> 这三篇都是 `selenium / DrissionPage + opencv` 的常规写法，价值不在「能过」，
> 而在**它们把「假拟人」的两个层级都演示了一遍**。

| 层级 | 形态 | 实测 | 判据 |
| --- | --- | --- | --- |
| 第一层（已在上一节登记） | 位移是确定公式、只随机**延时** | `scrollTop = 300+400*i` + `random.randint(1,3)` | 位移零熵 |
| ★★ **第二层（本批新增）** | **连延时都不随机** | `path_plan(40, x)` 生成 `[40, 40, 7]`（**步进 40 + 末段余数**），再 `move(x, 10, 1)` + `time.sleep(1)` ⇒ **走一步、停一秒** | **双重假拟人**：位移是分段常量速度，**时间轴是等距台阶** |
| ★ 第三层 | 「变速拖动」（相同时间内滑不同距离） | `52pojie-1159541` 的做法 | ★ **只是把「速度」换了个分段常数**，**位移-时间曲线仍处处是直线**，没有加减速、没有过冲回调 |

> ★★ **可迁移判据**：**「拟人化」不是「有随机数」，而是「位移-时间曲线」本身要像人**——
> 起手加速、中段高速、收尾减速 + **小幅过冲后回调**。
> 只要曲线是「若干个常量速度段首尾相接」，**随机化延时也不会改变这个形状**。

### 滑块识别的两条实现路线与第三条脆弱性

| 路线 | 做法 | 适用 / 风险 |
| --- | --- | --- |
| `Canny` + `matchTemplate` | 高斯模糊 → `cv2.Canny` → `TM_CCOEFF_NORMED` → `minMaxLoc` 取 `max_loc` | `52pojie-1159541`。★ 对**带干扰线/带纹理**的缺口图仍有效 |
| ★★ **「去白底找缺口」** | 读**灰度图**，收集模板图里**像素值 < 200** 的全部横纵坐标，取最小 `x` | `52pojie-2051638`。★ **实现极简（无 Canny）**，但**只适用于「缺口是高对比度实心块」的站点**；干扰线/拼图类会失效 |
| ★★ **截图裁剪用硬编码偏移** | `left = img.location['x'] + 160`、`top + 60`、`right + 230`、`bottom + 110` 一类**魔数** | `52pojie-1159541`。**页面一改版就全错**，且**不报错**（裁歪了照样能算出「一个」距离）⇒ 稳健做法是像 `52pojie-2051638` 那样**用 xpath 把元素 `save()` 成图片**，不自己做像素裁剪 |

> ★★ **判据**：**识别得到的 `x` ≠ 滑块实际位移**。两条实测映射：
> `x_dis = int(dis) + 135`（`52pojie-1159541`）、`path_plan` 的步进/余数（`52pojie-2051638`）。
> ⇒ **先量出「识别距离 → 实际位移」的映射，再谈拟人化**；映射没量准，拟人做得再好也是错位。

> ⚠️ **这三篇的结论层不要照抄**：它们的 `excludeSwitches: ['enable-automation']` 仍是 2019/2020 口径，
> 现行判定见上一节表格；本节只登记**「假拟人的层级」与「识别/裁剪的脆弱性」**两条可迁移判据。
