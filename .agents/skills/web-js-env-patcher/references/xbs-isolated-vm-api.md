# xbs isolated-vm API 使用说明

> 本文档已根据当前随包 xbsVm API 文档同步更新。原始文档也随包保存于 `assets/runtime-frameworks/xbs-isolated-vm/XBS_API_USAGE.md` 与 `assets/runtime-frameworks/xbs-isolated-vm/XBS_DOM_API_USAGE.md`。

## Skill 使用硬约束

- 只有用户明确选择 `isolated-vm` 补环境框架时，才复制并使用随包魔改 `xbs-isolated-vm/<platform>-<arch>/isolated_vm.node`。
- `window.xbs` / `globalThis.xbs` 只存在于 isolated-vm Context 内；不得桥接旧 `addon.node` 进入 isolated-vm。
- `xbs.dom.createDocument(options)` 是当前 xbs DOM 的唯一公开入口；已删除旧 API 不得继续使用。
- 需要基础 DOM 时优先通过 `xbs.dom.createDocument({ url, html, skeleton, omitApis, features })` 创建 document，再由 runtime 或项目代码决定是否安装到 `window.document`。
- `document.all` 优先使用 `xbs.dom.createDocument()` 自动提供的 `HTMLAllCollection` / HTMLDDA 行为；只有不使用 `createDocument()`、而是手写 document 时，才使用 `xbs.createUndetectable(callback, handlers)` 手动创建，并且必须配合 `HTMLAllCollection.prototype`、handlers 和原型链 fallback。
- 手动创建 `document.all` 时，`length / item / namedItem / constructor` 等已有原型链属性不要定义成 `document.all` 自有属性；更接近浏览器的安装方式是挂到 `Document.prototype` getter，而不是直接作为 document 自有属性。
- 如果当前 xbs API 已能覆盖目标行为但代码没有优先使用或参数错误，应先修正用法，不得标记为 native 能力缺口。
...
3. 对手写 `document.all` 这类特殊对象才优先使用 `createUndetectable(callback, handlers)`；如果需要基础 DOM，应优先使用 `xbs.dom.createDocument()` 自动提供的 `document.all`。
