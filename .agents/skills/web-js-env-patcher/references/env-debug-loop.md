...
- 若加密函数是**从打包产物里抠出来的模块**（webpack），已确认闭包完整且做过端到端数值对拍；
  交接判据（含「`Cannot read property 'call' of undefined` 属闭包问题、不是环境问题」）以
  `node-leakage-and-silent-failure.md` 的「打包产物抠出来的代码：交接约定」一节为准。
