...
| **多写者（N 个 JS 各写一段）** | **同一站十几个 JS 各写一部分 cookie，缺一即「参数错误」**（`52pojie-1906023`） | 按 `## 多写者` 一节：**按 cookie 名反查写入者**建表 + **逐个禁用验证**；**不要假定「一个 JS 写全部」** |
...
## 多写者：一个站有 N 个 JS 各写一部分 cookie（10+ 个 JS 场景）

来源 `52pojie-1906023`（**悬赏求助帖：源文作者自己没跑通**；以下只登记**已观察到的现象与判据**，不是可用方案）。

**现象（源文逐字要点）**：请求首页会**加载十几个 JS**，这些 JS **向浏览器写入 cookie**；
后续请求都要带这些 cookie，**检查不对则提示「参数错误」**。十几个 JS 全是 ob 混淆，
**最大的格式化后九万行**；源文怀疑「有可能有些 js 没有作用只是混淆视听」（⚠️ **源文推测，未复核**）。

**与 `multi-pass-cookie-generation.md` 的区别（务必分清，别混用）**：

| | 多趟自举（multi-pass） | **多写者（本节）** |
|---|---|---|
| 维度 | **时间**：同一份 JS 连跑两次结果不同 | **空间**：**N 份 JS 各写一段** |
| 症状 | 本地结果**短**于浏览器、缺字段 | 缺**某一个 cookie 名**就「参数错误」 |
| 判据 | 同一输入连跑两次输出不同 | **按 cookie 名反查写入者** |
| 处置 | 冷启动 + 热启动两趟 | 定位每段的写入者、只还原必要的那些 |

**取证流程（可执行）**：

1. **按 cookie 名反查写入者**（不要按文件猜参数）：hook `document.cookie` setter，记录
   **每次写入的 cookie 名**与**调用栈（文件 + 行号）**，得到一张「**cookie 名 → 写入者文件**」映射表。
   hook 的写法（含按名条件触发、以及「别把 get 写坏」）见
   `../../web-reverse-hook/references/anti-hook-detection-and-bypass.md` §8；
   「变化点即生成点」的方法论见
   `../../web-reverse-algorithm/references/15-call-site-locating-playbook.md` §0（J1）。
2. **判「哪个 JS 是必要的」**：**逐个禁用候选 JS，看哪一步 cookie 缺失导致后续失败** ——
   缺失的那一个就是必要的；不缺失的优先怀疑是混淆视听。
   （两个判据——正查写入者 / 反查禁用——**互证**，比单看代码可靠。）
3. **按四层链路分别落位**：对每一段 cookie 分别回答 `source / entry / builder / writer`
   （见本文「分析流程」第 3 步），**不要假定它们来自同一个 JS / 同一次响应**。
4. **只还原「后续请求必须带」的那几个名**，其余先跳过（源文最大的文件九万行，全量还原不现实）。

⚠️ **源文的失败点（必须原样登记）**：该帖是**求助帖，作者自己没跑通** ⇒ 本节只给**取证判据**，
**不给「某站到底哪几个 JS 有用」的结论**（单样本只登记现象与判据，不判因）。
...
- **假定「一个 JS 写全部 cookie」**：遇到十几个 JS 各写一段时，缺任一段都表现为「参数错误」——
  按 `## 多写者` 建「cookie 名 → 写入者」映射表，**不要按文件猜它产哪个参数**。
