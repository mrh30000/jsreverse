...
| ★ **网页端根本不提供播放**（只能在 App 内看），但 App 有「投屏」按钮 | **局域网投送层** | 把自己伪装成 DLNA **DMR**，从 `SetAVTransportURI` 的 `CurrentURI` 直接拿到明文地址 → `references/dlna-cast-capture.md` §1 判「投送 vs 镜像」→ §3/§4 起 SSDP+HTTP 两个服务 |
| 每页**多块**图片、块**大小不一**（不规则乱序）、块文件名是一段无规律 hex | **在线文档 · ⑥-子** | 位置从元数据 `{x,y}` 读（**别猜网格**）；资源名按**非标准 CRC32**（与标准差一句取反）复算 → `references/online-document-unlock.md` §8.5 |
