...
### §3C.7 更早的一代：五参数链 + JS 私有进制 `toString(32)`（`52pojie-1305472`，2020）

> 与 §3C.1–§3C.6（2025 那两篇）**同族不同代**：接口从「两跳 + AES 常量」变成
> 「**一次 POST 送五个派生参数**」，且派生用的不是标准库 —— 而是 `Number.prototype.toString(32)`。
> 另有一篇同族样本 `52pojie-1590803`（2022），密钥构造走「乱序重组」，见 §3C.9。

**链路（六步，页面源码里能全看到）**：

| 步 | 动作 | 判据 / 出处 |
| --- | --- | --- |
| ① | `var url = $('#sigu_url').val()` | **入参来自页面 DOM**（`<input id="sigu_url" value="…">`）⇒ 直接读属性，不用算 |
| ② | `token` 藏在 **aaencode** 壳里 | 壳解出来末尾会看到 `var url = $('#sigu_url').val()` ⇒ **token 与 url 同源** |
| ③ | `key  = sigu(明文key)` | `CryptoJS.AES.encrypt(x, Hex.parse(K), {iv: Hex.parse(IV), padding: ZeroPadding}).toString()` |
| ④ | `key2 = sigu2(x)` | `btoa(x)` → 逐字符 `(251 - code).toString(32)` → **拼接** |
| ⑤ | `key3 = sigu3(url + "|" + location.host)` | `btoa` → 逐字符 `(218 - code).toString(32)` → 拼接 |
| ⑥ | `POST /sigu_jx.php`，body 为 `{url,key,key2,key3,token,type:""}` | 响应 `url` 字段是密文；`sigu_decode` 解出「第四跳」地址，再取 m3u8 |

★★★ **本代最值钱的一条：`Number.prototype.toString(32)` 不是标准 base32**

```
btoa 产物字符域: 43..122  ⇒  (251 - code) ∈ [129, 208]
JS 进制字符集 = "0123456789abcdefghijklmnopqrstuvwxyz"  (0-9a-v) —— **不是** RFC 4648 的 A-Z2-7
```

- **误用任何标准 base32 库都必错**（字符集不同）。
- `toString(radix)` **不带前导零 ⇒ 不定宽**；只有当差值域整体落进 `[radix, radix²)` 时才是**恒 2 位**。
  实测 `129..208 ⊂ [32, 1023]` ⇒ 恒 2 位 ⇒ **解码端按「每 2 字符」切分**（源文 `if (i % 2 == 1)`）。
- ★ **可迁移判据**：`encode = (K - charCodeAt).toString(radix)` + 拼接 ⇒ **先算差值域**：
  落在 `[radix, radix²)` ⇒ 定宽 2；落在 `[0, radix)` ⇒ 定宽 1；跨区间 ⇒ **无法按定宽切，必须按 `parseInt` 边界扫描**。
- 解码端：`String.fromCharCode(K2 - parseInt(pair, radix))` → `atob`。
  ⚠️ 本样本里编码用 `251`、解码用 `258`（**两个方向用了不同的常数**）⇒ 别默认「加减同一个 K」。

**其余两点**：

- `padding: CryptoJS.pad.ZeroPadding`（**不是 Pkcs7**）。★ 静默风险：若下游按 PKCS7 剥填充，
  **零填充的产物长度照样合法**，只在明文末字节恰好落在 `0x01..0x10` 时才出错 ⇒ 落地时**按源文口径显式指定**。
- `key` 用 `CryptoJS.enc.Hex.parse("e10adc39…")`（32 hex 字符 → **16 字节原始密钥**）
  ⇒ **不走 KDF、不加 `Salted__` 头**。详见 `../../web-reverse-algorithm/references/16-ciphertext-structure-diagnostics.md` 的
  「CryptoJS key 三形态」（B43 新增）。

### §3C.8 直链的四种形态与「一次性」纪律（`52pojie-1683783` 四站 + `52pojie-1823993`）

同一批文章里一次给了**四个站、四套不同配方** —— 这是本族最好的「形态对照表」：

| 站 | 密文位置 | 配方 | 判据 |
| --- | --- | --- | --- |
| ① | 页面内变量 | `AES-CBC`，key 由 **同页正则**取（`var \w{6}=md5.enc.Utf8.parse("(\w+)");`），iv 同页 | **key/iv 写在同页 ⇒ 用页面里的正则取，不要手抄** |
| ② | 页面内变量 | `AES-CBC`，**key/iv 写死**（`0f84ff0c1f252cba` / `c487eb12e38a0fa0`），密文以 `t9GkQwf` 开头 | 密文**有特征前缀** ⇒ 直接拿前缀去页面搜，一般只剩一处 |
| ③ | 接口返回 | `POST {url,time,key}` → 响应 `url` 字段**直接就是直链** | **先看响应字段名**，不一定要解密 |
| ④ | `player_aaaa` 内 | `urls.slice(8)` → base64 → `[8:-8]` | 即 **W8 双段剥离**（`key-wrapper-families.md` §6.7） |

★★★ **「直链是一次性的」是本节最该背下来的一条纪律**（三两篇源文都踩过）：

- 解出来的地址**几分钟就失效**；而且**访问错一次立刻失效**（源文原话「如果访问错一次就会**立即失效**（别问我怎么知道的!!）」）。
- ⇒ **落地写法：解密后在同一段代码里「立刻下载/立刻转存」，不要先打印再取**；
  调试期要反复看同一个地址时，**每次重新生成**，别复用上一次的串。
- ⇒ 与 §3C.3 的「`key`/`time` 是一次性负载」是**同一条纪律的两种表现**（一个是地址是一次性，一个是参数是一次性）。

**★ ts 切片被伪装成图片**（站④）：下载 ts 得到「一张 1 像素的图」⇒ **用十六进制编辑器去掉图片头即可正常播放**
（源文原话）。同类容器伪装 → `scripts/container_disguise.py`。

**★ 定位动作（源文原话，可直接复用）**：断点要**下在「返回结果」处**，
「因为我们不知道这段解密后的结果是不是想要的，不然分析了半天加密逻辑，解出来的结果不是自己想要的岂不是尴尬」。

**★ 最短路径**：`52pojie-1823993`（2023 网课站）走的是**最朴素**那条 —— 在 `*player.js` 里搜
`decrypt` / `decryptdata` → 断点看 `decryptdata` 里的 **`key`** → 16 字节 hex 直接喂下载器。
⇒ **先试「搜 decrypt + 看 key」，不要一上来就扣算法**。

### §3C.9 环境守卫：让「本地复现值」静默错的三种写法（`52pojie-1305472` + `52pojie-1590803`）

> ★★★ 这是本族（乃至整个「扣代码」路线）**最容易白干一天**的地方，故单列一节。
> **触发信号：扣出来的函数在本地算出的值和服务端不一样，而且不报错。**

| # | 写法 | 现象 | 处置 |
| --- | --- | --- | --- |
| ① | **整段函数定义**包在 `if (location.host 命中白名单) { …defs… } else { top.location.href = "https://www.baidu.com/" }` | 在 Node 里跑 ⇒ **函数根本不存在**（`ReferenceError`，不是静默） | 补 `location.host` / 在浏览器里跑；搜 `top.location.href` 可定位 |
| ② | **函数体内**先判白名单，**非白名单就把入参替换成硬编码假值** | **算法照跑、不报错、结果全错**（本样本假值 `"dwvzv142x454fe54sa"` / `"sa3f13a1c4a561zxsa"` / `"asdsad541sdsa1"`） | 搜「**给形参直接赋字符串常量**」这一形态；或在浏览器里对拍入参 |
| ③ | 「**反格式化**」：检测自己有没有被 pretty-print，命中就进死循环/内存爆破 | 一格式化就卡死、浏览器崩溃 | **不要格式化**（DevTools 的 pretty-print 也算）；改在压缩态下打条件断点/日志断点；或**只扣需要的片段**（源文做法：只提取前三段跑一遍） |

- ★★ 写法② 的判据可以**机械化**：把「`参数名 = "字面量"`」当成一条正则在扣出来的源码里搜一遍；
  命中的函数**一律先在浏览器里对拍一次入参**再采信。
  ⚠️ 这与 §3C.7 的「`btoa` 前给形参赋默认值」**形态相同、意图相反**（那处是兜底，这处是投毒）⇒ **不能只看形状**。
- ⚠️ 写法③是**单源**：源文只说「会检测你是否格式化」，**未给出检测代码**
  ⇒ 本节只登记该现象与处置纪律，**不声称已知其检测机制**。
- ★ 顺带：本代 `52pojie-1305472` 还叠了**每 2 秒一次的无限 `debugger`**
  （`Function("Function(arguments[0]+'bugger')()")("de")` ——「de」+「bugger」拼成 `debugger`）
  ⇒ **搜 `debugger` 字面量搜不到**，要改搜 `bugger` / `setInterval` / `Function.prototype.constructor`。
  处置见 `../../web-reverse-algorithm/references/07-antidebug-and-live-patching.md`（B43 新增两种构造的判据）。

## §3D 不改算法：响应注入式（userscript hook XHR 改写响应，`52pojie-1713067`）

**边界**：前面所有小节都在「**还原地址/密钥**」；本节是**相反的取舍** ——
**完全不动算法**，只 hook 页面的 XHR，把**第三方接口**的结果**塞进页面自己要的 JSON 里**，
让页面自己继续往下跑。「骗过前端」而不是「复现前端」。

**适用判据（满足任两条就该考虑它）**：

- 目标是**自己看/自己用**（用户脚本、油猴），不需要批量采集；
- 前端逻辑是**长链路 + 多次往返**（`detail → 判权限 → url/v1`），逐段还原成本高；
- 真正的数据来自**另一个接口**（本例：第三方音乐接口），页面只是**把关**（VIP 校验）。

**链路骨架（本例）**：

```
hook XMLHttpRequest.prototype.open  →  匹配 args[1] 里的目标接口
  ├ detail 接口：直接改写响应里的 privileges 字段 ⇒ 骗过「有没有播放权限」判定
  └ url/v1 接口：等在 onreadystatechange 里请求第三方接口，拿到地址后再回填
```

★★ **「填字段」这条最容易漏，而且漏了是静默的**（源文专门列了坑）：

- 回填 **不止 `json.data[0].url`**：还需要 `code`（实测填 `200`）、`br`（音质数值）、
  `level`（音质字符串名）、`type`（实测填 `"mp3"`）。
- 漏填的表现是**弹 VIP 提示 / 不播放**，**不报错** ⇒ 与 §3C.9 同属「静默错」，但根因是**契约缺字段**。
- ⇒ 可迁移判据：**改写响应前，先把「目标接口的响应 schema」整份抄下来**，逐字段决定「保留 / 改 / 造」，
  不要只改那个「看起来最像目标」的字段。

**两条取证纪律（与算法无关，但没有它们就白折腾）**：

- ★★ **前端有请求缓存** ⇒ 「点按钮没发出请求」**不等于**「没有这个请求」，
  可能是上一次点击已经请求过并被缓存了。**每次抓包/调试前先刷新页面**（源文原话）。
- ★ **跨域**：第三方返回的地址若不在目标站域名下、又无 CORS 头，`onprogress` 里可能播不了；
  用 `GM_xmlhttpRequest` 取到 **`e.finalUrl`**（跟随 302 后的最终地址）再回填。

> 取舍参照：**能「还原算法」就不要「注入响应」**（注入式依赖页面 DOM/时序，脆弱、不可批量）；
> 但**当授权链路长且自己给自己用时，注入式的性价比远高于逐段还原** —— 这是本族唯一推荐它的场景。
