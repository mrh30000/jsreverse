...
#### 通用：多段链式签名（**同一盐重复插入** + **上一响应字段入签**）

来源 `52pojie-1059382`（2019-11，卓易健康步数接口，PHP）。这是「标准签名题」里**最容易被误判成
复杂算法**的一族：参数全是明文、算法是裸 MD5，**复杂度全在「拼串顺序」与「跨请求状态」上**。

**结构（源文两段，逐字）**：

```php
// 段 1：换 openid
$data  = md5($user . $str . $timeStamp);                       // $str = "8061FD"
POST SendWechat/getWxOpenid   body: accountId={$user}&timeStamp={$timeStamp}&sign={$data}
// 段 2：提交步数（★ 用的是**同一个** $timeStamp，且把**段 1 的响应字段** $response["openid"] 拼进去）
$data1 = md5($user . $str . $walk . $str . $timeStamp . $str . $response["openid"]);
POST SendWechat/stepSubmit    body: accountId={$user}&jibuNuber={$walk}&timeStamp={$timeStamp}&sign=$data1
```
...
| # | 判据 | 为什么重要 |
| --- | --- | --- |
| ① | **同一个盐值在一段串里出现多次**（本例段 2 出现 **3** 次，段 1 出现 1 次） | 「盐只加一次」是思维定式；本批复算：段 1 盐出现 1 次、段 2 出现 3 次 ⇒ **必须数一遍** |
| ② | **两段共用同一个时间戳**（不是各自取一次 `time()`） | 复算/重放时若各自取 `time()`，**第二段必失败**；且窗口极窄（同秒） |
| ③ | **上一响应的字段参与下一段签名**（`openid`） | 这是「链式」的本质：**不能并发、不能乱序**，必须严格两段串行 |
| ④ | **段 1 与段 2 的字段集不同**（段 2 多一个业务字段 `jibuNuber`，且**拼串顺序 ≠ 表单顺序**） | 表单里 `jibuNuber` 排第二，但拼串里它在 `user` 与 `ts` **之间** —— 顺序只能从源码抄，不能从表单推 |

**两条形态坑（源文原样，照抄即可）**：

- **参数名拼写错误是站点常态**：源文表单字段逐字是 **`jibuNuber`**（不是 `jibuNumber`）。
  ⇒ **判据：表单字段名一律从抓包/源码逐字抄，不做「拼写纠正」**；改对了反而 400。
- **业务值可能被站点自定义钳制**：源文附注「`-1` 是 `98800`」——
  本批复算排除了「16 位溢出」这一解释（`(-1) & 0xFFFF == 65535 ≠ 98800`），
  且 `98800 > 65535` ⇒ **这是站点自定义的「最大值哨兵」，不是溢出语义**。
  ⇒ **判据：看到「某个魔数代表最大值」的附注，先算一遍溢出边界排除掉，再登记为「站点约定」。**

**验收**：段 1 的 `sign` 与段 2 的 `sign` 都是 **32 位小写 hex**；
本批用 `artifacts/skill-evolution/tools/b40-probe-anchors.py` 复算了两段的盐出现次数与 hash 形态
（`md5("demo_user8061FD123458061FD15592064608061FDoX9kQ0…")`）——
⚠️ **该 hash 只是「拼法自洽性」的证据，不是服务端对拍向量**（源文没给真实响应），
**登记为未复核**，勿当验收标准用。
...
- **本文只给判据，不给「绕过授权」的成品**。上面所有内容用于**理解算法与做技术验证**；
  源文的附件（注册机）不在本库收录范围。
- ★ **服务端 PHP 的同类题（方法体整段不可读）走 `19-opaque-source-patching.md`**：
  那边是「**整段方法体替换**」（因为没有可读的判断点），本文是「**改一行**」；
  两边的**共同纪律**是「先读调用方，再决定改谁」与「返回值类型由调用方决定」。
