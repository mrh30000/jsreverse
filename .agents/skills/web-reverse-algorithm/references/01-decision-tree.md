...
### A.2 响应解密题的入口捷径：**函数名常常就写在调用点上**（B29）

`response-decrypt` 这一类**不需要**重建 `writer <- builder <- entry` —— 目标是「把响应体还原成明文」，
所以入口在**消费密文的那一行**，而不是产出密文的那一行。

1. 在 Network 里选中返回密文的接口，**搜接口关键词**（URL 片段 / 响应里的字段名，如 `data`）。
2. 命中形如 `let f = JSON.parse(r2(a.data.data))` 的代码 ⇒ **`r2` 就是解密函数**，
   直接把它当答案，不必跟调用栈、也不必先找混淆入口。
3. **三连确认**（少一步都可能认错）：
   - 在这一行下断点，刷新；
   - 看入参是密文串、`r2(...)` 的返回值是**能 JSON.parse 的明文**（这一步同时确认了"解出来是 JSON"）；
   - 全局搜 `r2` 的**定义处**，认算法族（本例是 AES-CBC）。
4. **落地前先用现成工具对一次**：同一密文 + 同一 key/iv，用在线工具/openssl 解出的明文必须一致。
   这一步的价值是把「**算法认错了**」和「**我的代码写错了**」分开 —— 否则后面排错会两件事一起猜。

> 判据的适用边界：只有当调用点**显式命名**了解密函数（`r2` / `decode` / `decrypt` / `xxtea` …）时这条路才成立。
> 若调用点是 `JSON.parse(data)` 这种**内联**写法，说明解密发生在更早（常见是 `fetch` 的响应拦截器或
> `XMLHttpRequest.prototype.responseText` getter 里），回到 §二 的「打包产物」与 §三 的 A 类动作。
...
### G. 极端混淆或高频风控下的免扣兜底路线 (RPC Bridge)

当算法处于重度混淆（大型 JSVMP、每隔几天更新字节码与密钥）或强依赖真实浏览器内部状态（Canvas/WebGL/底层硬件指纹）时，纯算还原与补环境研发成本极高。

优先动作：
1. 评估交付周期与维护成本（研发耗时对比：纯算 2~7 天 vs 补环境 1~2 天 vs RPC 免扣 15 分钟）。
2. 采用轻量浏览器端 WebSocket RPC 桥接方案（参见 `references/14-browser-rpc-bridge.md`），免扣代码直接暴露 window 签名方法供本地 Python / 采集脚本实时调用。
