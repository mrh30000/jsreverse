...
**WS 自定义信封**）。**不要靠肉眼看"像不像长度"**，用脚本的 `--frame auto`：它按"剥完能否完整解析"来选。

★ **长度字段的语义必须用等式反推，不能靠名字想当然**：本仓已实证过一族「**长度字段 = 帧总长 − 6**」
的会话信封（`framing-and-transport.md` §1b）。**连"帧总长 − 6"这种非直觉语义都存在**
⇒ 判据永远是 **`6 + 长度字段 == 帧长`** 这类**等式成立**，而不是「长度字段看着等于 payload 长度」。
★ 同一连接里**可能有多族信封**（本批 20 帧里 5 帧属另一族）⇒ 用 `--frame ws-env` 时它会**主动报错**
而不是静默切错；遇到报错就说明该帧不属本族，**别放宽断言**。
