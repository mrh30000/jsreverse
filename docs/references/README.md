...
> 目录现状（`artifacts/52pojie-fetch19/` 终检，2026-09-23 第十九轮）：
> 文章 `.md` 共 **822 个**（不含本 README）= `52pojie-<id>-<标题>.md` **740 个** + 早期 `<id>-<slug>.md` **51 个**
> + 无 ID 文件 **31 个**（其他平台批量归档 `csdn-*` **25 个**，见第十六节；另 6 个为早期手工文件：`weixin-*.md`、`cloudflare*.md`、`verified.md` 等）。
> 唯一帖子 ID **789** 个；另有 2 个 `.验证报告.md` 与主文共用同一 ID（`2098573`、`2100076`，非重复）。
> 空文件（<400B）**0** 个；本轮新增 14 篇最薄 927B（`1552603` Typora 密钥），最大 23KB（`2084047` Typora 反-反调试）。
> 编码扫描（`repair-encoding.py --scan`）823 文件 **1 命中**（`1985027` 内含站点原帖的混淆代码，不可还原、非抓取问题）。
...
## 十八、补充归档（十五）（14 篇，2026-09-22 第十七轮：新增「全站未归档命中」扫描通道）

**本轮的判断：关键词通道已关闭（第十六轮结论），但「全站未归档命中」是一条被忽略的高产通道。**
历轮的召回入口只有两个——Thread 新帖、关键词搜索。本轮不再造新词，改为把**全站（含历史）匹配
「JS 逆向特征正则」但尚未归档的帖子**列成一张表（`jsrev_scan17.txt`，555 条，带 NEW/REV 标记），
再人工从中挑 120 条抓正文复核：

- 这条通道一次就捞出 **6 篇**「唯一来源」真阳性（`1980009` 宪法小卫士 webpack+国密、`2062977` 心理平台登录、
  `1463377` 百度翻译 sign、`975538` 超星学习通协议、`1985027` 瑞数5 补环境、`1994084` 邮箱登录逆向），
  **占本轮 14 篇的 43%**，且多为「关键词搜不到、Thread 也刷不出」的**存量老帖**；
  另有 4 篇（`1022123`/`1448337`/`2065436`/`2085430`）与该通道的正文抓取重合。
- 另一半来自审计五表复核：`keep17.json`（B ∪ D ∪ E ∪ G ∪ C 合并 475 → 标题级两段门槛 186）抓正文
  → 定稿 8 篇。

⇒ 两条通道合计复核 **291 篇**正文（审计通道 189 + 扩样通道 102），定稿 **14 篇**，
其余 **277 篇**固化进 `BLOCK_EXTRA17.json`（其中 **101 篇**来自本轮扩样，防下轮重复抓取）。
**真阳性形态已收敛为：登录/接口协议、参数/签名算法、webpack 扣取与补环境、JS 侧滑块、响应解密。**

| 主题 | ID | 标题 | 文件 |
| --- | --- | --- | --- |
| 登录 / 接口协议逆向 | [1022123](https://www.52pojie.cn/thread-1022123-1-1.html) | QQ空间协议分析—扫码登录（ptqrtoken 由 Cookie 中 qrsign 计算） | `52pojie-1022123-*.md` |
| ↳ | [1448337](https://www.52pojie.cn/thread-1448337-1-1.html) | 有道云笔记接口分析（Charles 全接口梳理：扫码登录 / 上传 / 文件 CRUD 参数作用） | `52pojie-1448337-*.md` |
| ↳ | [975538](https://www.52pojie.cn/thread-975538-1-1.html) | 超星学习通协议分析（fiddler 抓包 → 直接伪造视频进度上报请求） | `52pojie-975538-*.md` |
| ↳ | [1994084](https://www.52pojie.cn/thread-1994084-1-1.html) | 某某邮箱登录逆向—小白讲解03（定位加密位置 → 分析逻辑 → 改码 → 验证四步法） | `52pojie-1994084-*.md` |
| ↳ | [2062977](https://www.52pojie.cn/thread-2062977-1-1.html) | 记录一次某心理健康平台的登录分析（滑块验证码 base64 图 + 登录参数） | `52pojie-2062977-*.md` |
| 参数 / 签名算法还原 | [1463377](https://www.52pojie.cn/thread-1463377-1-1.html) | 小白 js 逆向—百度翻译（sign 由 JS 生成，execjs 复用） | `52pojie-1463377-*.md` |
| ↳ | [1820059](https://www.52pojie.cn/thread-1820059-1-1.html) | 挖洞遇到验证码那些事（滑块 sign：captcha_signature / phrase / ticket 拼接） | `52pojie-1820059-*.md` |
| ↳ | [2054956](https://www.52pojie.cn/thread-2054956-1-1.html) | 码上爬-题十四：神枪手 请求参数处理（`Object.defineProperty` 代理 window.dd → hash 算法） | `52pojie-2054956-*.md` |
| ↳ | [2065436](https://www.52pojie.cn/thread-2065436-1-1.html) | 某某 Meta 登录逆向（webpack 逐模块抠取 + 大数组自执行重排 + sign 生成） | `52pojie-2065436-*.md` |
| ↳ | [2085430](https://www.52pojie.cn/thread-2085430-1-1.html) | 某站评论获取接口分析（B 站 w_rid：参数定序拼接 + imgKey/subKey 置换表） | `52pojie-2085430-*.md` |
| webpack 扣取 / 补环境 | [1980009](https://www.52pojie.cn/thread-1980009-1-1.html) | 宪法小卫士登录分析（webpack `n("8060")` 抠加载器 + passwordSMEncryption 国密） | `52pojie-1980009-*.md` |
| ↳ | [1985027](https://www.52pojie.cn/thread-1985027-1-1.html) | 瑞数5 补环境（欧冶：完整手动补环境代码模板 document / storage / logger） | `52pojie-1985027-*.md` |
| 滑块验证码（JS 侧） | [1808237](https://www.52pojie.cn/thread-1808237-1-1.html) | 挖洞遇到验证码那些事（自研滑块：randstr 随机串扣代码 + 前端校验绕过） | `52pojie-1808237-*.md` |
| 响应 / 媒体解密 | [2092367](https://www.52pojie.cn/thread-2092367-1-1.html) | 4.0 微信朋友圈媒体解密全解析（`x-Enc` 响应解密：key 来源 + 分类型算法） | `52pojie-2092367-*.md` |

### 本轮口径与审计

- **Thread**：第 1~8 页共 **898 条**（第 9 页空），本轮新增 38 条**全为资源/工具/求助帖**，
  web 逆向 **0**（第 3~17 轮同一结论）。
- **关键词**：本轮**未投新词**（第十六轮已证明复合词命中率≈0），预算全部转投上述两条正文复核通道。
- **筛选门槛（两段）**：`coarse17b.py` 标题级 CORE 命中 + DROP 排除（308 → 186），
  再 `score17.py` 正文级 v2 打分（CORE 硬门槛 + native/工具/爬虫教程负分）→ 158 条入人工复核。
- **噪声 277 条**（`BLOCK_EXTRA17.json`，按正文粗分类）：native / 桌面 / 移动端逆向 **188**、
  加解密库与自研算法及工具 **20**、验证码识别与图像 AI **14**、通用爬虫与自动化取数 **5**、
  纯求助帖（无实质内容）**19**、其余混合型 **31**。
- **`BLOCK_EXTRA17` 的价值**：本轮首次把「已看正文但未入选」**全量**固化，含 101 条本轮新抓的
  `jsrev_scan17` 扩样，避免下轮重复付出 100 次正文抓取成本。
- ⚠️ `52pojie-1985027-瑞数5补环境.md` 内含原帖粘贴的**瑞数混淆代码**，其中一行含少量不可还原字符
  （`repair-encoding.py --scan` 报 1 命中、可还原行 0）——**来自站点原帖而非抓取编码问题**，保留原样。

---

## 十九、补充归档（十六）（22 篇，2026-09-23 第十八轮：全站未归档命中表「全量自动排序」+ 技术词盲区补充）

**本轮的判断：第十七轮只人工挑了 `jsrev_scan17.txt` 555 条命中里的 120 条，剩 419 条是「已发现但没看」的
系统性盲区；而且挑法是人工随手挑，不可复现。** 本轮把这条通道做成可复现的三段流水线：

1. **全量重建命中表**（`jsrev_scan18.py`）：对全池 17756 帖做「标题级信号 − 噪声」自动打分
   （POS = 补环境 / jsvmp / webpack / 反混淆 / 风控盾 / 验证码 / 参数签名 / 接口协议 / 媒体解密…
   NEG = native·移动端·桌面 / 木马样本 / 工具与课程资源 / 纯求助），
   先减掉 `BLOCK_EXTRA*.json ∪ REVIEWED_NOISE ∪ 已归档 ∪ bodies17`，得 **1982 条 score>0 命中**，
   按分数自动排序 —— **不再靠人工随手挑**。
2. **抓正文**（`probe18.py`）：T1 第 17 轮残留（419 中 score≥4 的 95 条）+ T2 全站新面孔（score≥6）
   + T3 标题「系列/隐称」通道，共 **474 条**；另加技术词盲区补充批 **28 条**（jsc / electron / 小程序 / AST）。
   合计抓取 **502 篇**正文（`bodies18/`，ok 492 / 权限受限 9 / 反爬 1）。
3. **正文级 v2 打分 + 逐条复核**（`score18.py` + 人工）：要过 CORE 硬门槛（加密/签名/补环境/AST/
   webpack/jsvmp/风控/滑块/字体/参数加密/接口解密/协议）→ **268 条入评分表** → 逐条看正文定稿 **22 篇**。

| 主题 | ID | 标题 | 文件 |
| --- | --- | --- | --- |
| 小程序 / H5 前端逆向 | [1474964](https://www.52pojie.cn/thread-1474964-1-1.html) | 某商超小程序加密算法解析（wxapkg 解包 → paramsMD5 + 经纬度 base64 加密还原） | `52pojie-1474964-*.md` |
| ↳ | [1484716](https://www.52pojie.cn/thread-1484716-1-1.html) | 某付宝某加油小程序对称加解密（MD5 sign + AES/CBC/pkcs7 置换表，函数逐个抠取） | `52pojie-1484716-*.md` |
| ↳ | [1482566](https://www.52pojie.cn/thread-1482566-1-1.html) | [另类方式破解]支付宝小程序 sign 验签（改小程序代码让它自己吐出签名入参） | `52pojie-1482566-*.md` |
| ↳ | [1724382](https://www.52pojie.cn/thread-1724382-1-1.html) | WX 小程序破解详细教程（明文/加密/签名 四象限判断 → 解包 → 算法还原） | `52pojie-1724382-*.md` |
| ↳ | [1944883](https://www.52pojie.cn/thread-1944883-1-1.html) | php 重写校友邦微信小程序签到加密逻辑（乱序数组 + MD5 + tstr 时间戳） | `52pojie-1944883-*.md` |
| ↳ | [1751688](https://www.52pojie.cn/thread-1751688-1-1.html) | 洗衣小程序逆向（ssid / sign 参数抠函数还原） | `52pojie-1751688-*.md` |
| ↳ | [1764292](https://www.52pojie.cn/thread-1764292-1-1.html) | VX 小程序逆向分析（Frida/Xposed hook WxJsApiBridge 拦截 wx.request 收发） | `52pojie-1764292-*.md` |
| ↳ | [1050690](https://www.52pojie.cn/thread-1050690-1-1.html) | 支付宝小程序抓包与源码获取（nebula 进程定位 + SSL 校验绕过 + 包提取） | `52pojie-1050690-*.md` |
| ↳ | [1479871](https://www.52pojie.cn/thread-1479871-1-1.html) | 某 APP 加密 sign 逆向（Android WebView + uni-app，篡改 hex_md5 反向暴露入参） | `52pojie-1479871-*.md` |
| 参数 / 算法还原（含补环境） | [1827947](https://www.52pojie.cn/thread-1827947-1-1.html) | 超星学习通 enc 参数（requireJS 模块审计 + NodeJS 服务 Socket 与 Python 通信） | `52pojie-1827947-*.md` |
| ↳ | [1985839](https://www.52pojie.cn/thread-1985839-1-1.html) | js 逆向参数解密（青岛政务：JSEncrypt RSA 请求 + AES 响应，含站点原码与 Python 复现） | `52pojie-1985839-*.md` |
| ↳ | [1832611](https://www.52pojie.cn/thread-1832611-1-1.html) | akamai 中 VM2 的检测特征（toString 检测 + VM2 注入字符串识别与源码修正） | `52pojie-1832611-*.md` |
| ↳ | [1981831](https://www.52pojie.cn/thread-1981831-1-1.html) | 瑞数 5 补环境（node 侧 window/document/localStorage 完整补齐模板 + cookie 排查） | `52pojie-1981831-*.md` |
| 验证码 / 滑块（JS 侧） | [1806246](https://www.52pojie.cn/thread-1806246-1-1.html) | 【验证码逆向专栏】某度滑块/点选/旋转 v1·v2（viewlog·getstyle 链路 + fs 参数） | `52pojie-1806246-*.md` |
| ↳ | [2045347](https://www.52pojie.cn/thread-2045347-1-1.html) | 某验 3 底图还原 python 算法改写（Canvas drawImage 断点还原乱序底图） | `52pojie-2045347-*.md` |
| 字体 / 媒体 / jsc | [1465475](https://www.52pojie.cn/thread-1465475-1-1.html) | 起点字体加密学习（woff 下载 + TTFont cmap 映射 → 数字还原） | `52pojie-1465475-*.md` |
| ↳ | [1730445](https://www.52pojie.cn/thread-1730445-1-1.html) | M3U8 KEY 解密探讨（AES-128 + tokenVideoKey 取 key 链路 + 私钥推导） | `52pojie-1730445-*.md` |
| ↳ | [1945555](https://www.52pojie.cn/thread-1945555-1-1.html) | m3u8 aes-128 找不到密钥（完整 player.js 抓包留档 + EXT-X-KEY sign/ms 参数分析） | `52pojie-1945555-*.md` |
| ↳ | [1913191](https://www.52pojie.cn/thread-1913191-1-1.html) | 某 xor + xxtea 加密 jsc 解密记录（文件头签名识别 + 16 位密钥暴破） | `52pojie-1913191-*.md` |
| ↳ | [2054765](https://www.52pojie.cn/thread-2054765-1-1.html) | JSC 字节码反编译初探（改 d8 加 LoadJSC/Disassemble 反编译 electron jsc，Typora 为例） | `52pojie-2054765-*.md` |
| 协议 / 工具 | [1648818](https://www.52pojie.cn/thread-1648818-1-1.html) | 某吧私信 websocket + protobuf + RSA/AES 流程还原（网页端 ws 抓帧 → protobuf 解码） | `52pojie-1648818-*.md` |
| ↳ | [1982161](https://www.52pojie.cn/thread-1982161-1-1.html) | babel 解混淆模板（AST 插件模板，可直接套用） | `52pojie-1982161-*.md` |

### 本轮口径（首次明确「收录判据」，取代历轮隐性标准）

正文（含代码块）**必须包含以下之一**才收，**纯提问无技术内容不收**：

- **(a)** 完整/片段的算法实现或密钥推导；
- **(b)** 补环境 / hook / 反调试 的可复用代码或模板；
- **(c)** 接口链路或参数推导过程的调试记录；
- **(d)** 某逆向方向的系统性方法总结。

不收：native / 移动端（APK、so、Frida、IDA、.NET）为主的帖、工具与资源/课程分享、
纯自动化（selenium 过验证码、打码平台）、通用爬虫教程、加解密库教程、无正文的提问。

> 这条判据让本轮的**求助帖**出现了分野：`1981831`（瑞数5 补环境，带完整补环境代码）、
> `1985839`（青岛政务 RSA+AES，带站点原码与 Python 复现）**收**；
> `1909362`（字节码补环境，代码全在图里）、`2046628`（找大佬抠 zxx 登录参数）**不收**。

### 本轮技术看点

- **小程序 / H5 成为本轮最大主题（9 篇）**：历轮的小程序帖多散落在关键词通道，本轮在全站命中表里
  一次捞出 4 篇「带完整算法还原」的（含 wxapkg 解包 → 参数加密 → 抠函数 → 复现），
  以及 2 篇「另类思路」（改小程序 JS 让签名函数自己吐出入参 / hook `WxJsApiBridge` 拦 `wx.request`）。
- **补环境三条不同路线**：`1981831` 手写 window/document/localStorage（瑞数 5）、
  `1832611` 反查 VM2 注入字符串导致的 toString 不一致、`1827947` 用 Node 起服务 + Socket 给 Python 供算法。
- **媒体/字节码类补齐**：m3u8 取 key 链路 2 篇、`xor+xxtea` jsc 2 篇（含改 d8 反编译 electron jsc）。
- **`1906842`（某数 vmp）被判不可读**：正文 292KB 里 77% 是论坛对粘贴二进制做的转义乱码（`&#240;`/`\x00`），
  仅 60KB 代码块可用 → 本轮**剔除**，并记入 `BLOCK_EXTRA18.json`。

### 本轮审计与统计

- **Thread**：第 1~8 页共 **896 条**（第 9 页空），本轮新增 32 条**全为资源/求助/工具帖**，
  web 逆向 **0**（第 3~18 轮同一结论，Thread 通道可视为关闭）。
- **关键词**：**未投新词**（第 16/17 轮已证明复合词命中率≈0），预算全部给「命中表 + 正文复
  核」。
- **噪声 470 条**固化进 `BLOCK_EXTRA18.json`（防下轮重复抓取），其中正文 <1.5KB 的纯提问/空帖 **296 条**；
  按标题粗分：加解密库·协议理论·其他混合 **248**、纯提问 **90**、native/移动端/桌面 **63**、
  工具·资源·课程 **55**、通用爬虫·自动化 **14**。
- **终检**：文章 `.md` 共 **808 个**（`52pojie-` **726** / 早期 slug **51** / 无 ID **6** / csdn **25**），
  唯一 ID **775**，空文件 **0**，重复仅 2 组已知 `.验证报告.md`；
  `repair-encoding.py --scan` 809 文件 **1 命中**（`1985027` 站点原帖自带，非抓取问题）。

---

## 二十、补充归档（十七）（14 篇，2026-09-23 第十九轮：全站未归档命中表「第二批分层取样」+ NOISE 过滤误杀体检）

**本轮的判断：第十八轮把命中表流水线化后，只吃掉了 T1（score≥4 残留）+ T2（score≥6 新面孔）+ T3，
还剩 1481 条未抓（与 bodies18 零重叠）。本轮就是把这 1481 条按「成本递减、收益递减」拆成三批取完。**

| 批次 | 口径 | 候选 | 抓取 |
| --- | --- | --- | --- |
| T1 | **技术词白名单**（jsc / electron / wasm / 小程序 / AST / sourcemap / protobuf / 字体 / m3u8 / webpack / jsvmp / 补环境…）× 非硬噪声 | 114 | 全部 |
| T2 | score ≥6 且标题含逆向核心词 | 14 | 全部 |
| T4 | 标题「系列 / 专栏 / 隐称」通道 | 12 | 全部 |
| T3 | score 3~5 且标题含逆向核心词 × 非硬噪声 | 477 | 全部 |
| **B 批** | **NOISE 过滤器误杀体检**：score ≥3 + 逆向核心词，但被标题级 NOISE 一刀切掉的（再剔 .NET/native/打码平台） | 173 | 全部 |

合计抓正文 **769 篇**（`bodies19/`，ok 769 / 权限受限 17 / 空正文 1 / 反爬 1）→ v2 打分入表 **297** →
逐条复核按四条收录判据定稿 **14 篇**。

### 本轮两个结论（都已写入 skill）

1. **标题级 NOISE 过滤器没有造成系统性误杀（B 批 173 篇 → 0 篇真阳性）**。
   这是本轮唯一值得单独跑的"过滤器体检"：历轮教训是「口径一刀切会静默丢掉一整类真阳性」，
   所以把被切掉的 173 条全抓了一遍正文 —— 结果是清一色的「小程序源码 / m3u8 播放器求助 /
   验证码识别 / BOSS 直聘反爬求助 / .NET 反混淆」，**0 真阳性** ⇒ 这把快刀可以放心继续用，
   且**下轮不必再花预算复检**。
2. **剩余 875 条未抓项已可判定为噪声**：分层统计后，其中 233 条是「score≥3 + 核心词 + NOISE」（本轮已体检），
   其余为「score 1~2 无核心词」233 条与「score≥3 无核心词」164 条 —— 标题既无逆向核心词、分数又低，
   **命中表通道到此基本吃尽**。

### 本轮主题分布

- **`Electron / asar 逆向` 4 篇**（本轮最大主题）：Typora 一个案例从「密钥直接给出」（`1552603`）
  到「AES-256 密钥推导」（`1983982`）再到「反-反调试 + 激活劫持」（`2084047`）与「Hook 代码整理」（`2085249`），
  构成一条完整的学习梯度。
- **`视频 / m3u8 / DRM` 3 篇**：DRM clearkey 的 SPC/CKC 报文体结构（`2060029`）、
  key 走 `play_licenses` 接口而非 m3u8（`1753800`）、hls.js 补环境 + 密钥 XOR（`1851955`）。
- **`网站 JS 加密与响应解密` 3 篇**：含一个很实用的模式 —— 响应密文**前 16 字符即 key、第 17 位起为密文**（`1926140`）。
- **`字体 / 协议 / 前端反调试` 3 篇**、**`登录 JS` 1 篇**。

### 本轮审计与统计

- **Thread**：逐页拉到第 8 页（868 条，第 8 页空），新增 14 条**全为资源/求助/工具帖**，
  web 逆向 **0**（第 3~19 轮同一结论，Thread 通道关闭）。
- **关键词**：**未投新词**（第 16~18 轮已证明复合词命中率≈0），预算全部给命中表。
- **噪声 755 条**固化进 `BLOCK_EXTRA19.json`（本轮已抓且已复核判废，防下轮重复抓几百篇正文）；
  全量排除集（含历轮 BLOCK_EXTRA 常量抽取 344 + ALLOW 189 + REVIEWED_NOISE + bodies14/16/17/18）共 1945 条。
- **一处编码修复**：`2044857` 抓下来是「UTF-8 被按 GBK 再解」的部分乱码（76 行可还原），
  已用 `repair-encoding.py --fix` 就地修复（西里尔 2018 → 0）；`1985027` 仍是站点原帖自带、
  **可还原行 0**，按历轮结论不修、不丢帖。
- **终检**：文章 `.md` 共 **822 个**（`52pojie-` **740** / 早期 slug **51** / 无 ID **6** / csdn **25**），
  唯一 ID **789**，空文件（<400B）**0**，重复仅 2 组已知 `.验证报告.md`（`2098573`、`2100076`）；
  `repair-encoding.py --scan` 823 文件 **1 命中**（`1985027`，非抓取问题）。
