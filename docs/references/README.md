...
> 目录现状（`artifacts/52pojie-fetch17/` 终检，2026-09-22 第十七轮）：
> 文章 `.md` 共 **786 个**（不含本 README）= `52pojie-<id>-<标题>.md` **704 个** + 早期 `<id>-<slug>.md` **51 个**
> + 无 ID 文件 **31 个**（其他平台批量归档 `csdn-*` **25 个**，见第十六节；另 6 个为早期手工文件：`weixin-*.md`、`cloudflare*.md`、`verified.md` 等）。
> 唯一帖子 ID **753** 个；另有 2 个 `.验证报告.md` 与主文共用同一 ID（`2098573`、`2100076`，非重复）。
> 空文件（<400B）**0** 个；本轮新增 14 篇最薄 4356B（`2065436` 某某 Meta 登录逆向），多为 5~30KB 的实文。重复 ID 0 组。
> 编码扫描（`repair-encoding.py --scan`）787 文件 **1 命中**（`1985027` 内含站点原帖的混淆代码，不可还原、非抓取问题）。
