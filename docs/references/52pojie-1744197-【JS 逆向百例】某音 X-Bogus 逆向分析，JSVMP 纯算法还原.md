...
> **作者**: K哥爬虫 | **发布时间**: 2023-02-10 10:12:00 | **版块**: 『脱壳破解区』 | **查看/回复**: 19798 / 96
> **原文**: [https://www.52pojie.cn/thread-1744197-1-1.html](https://www.52pojie.cn/thread-1744197-1-1.html)

---

*本帖最后由 kspider 于 2023-2-18 10:21 编辑*

![](https://s2.loli.net/2022/10/11/evJuSnNIzDmqKY3.png)
...
JSVMP 的概念最早应该是由西北大学2015级硕士研究生匡开圆，在其2018年的学位论文中提出的，论文标题为：《基于 WebAssembly 的 JavaScript 代码虚拟化保护方法研究与实现》，同年还申请了国家专利，专利名称：《一种基于前端字节码技术的 JavaScript 虚拟化保护方法》，网上可以直接搜到，也可也可[点击此链接](https://pan.baidu.com/share/init?surl=KJWJOgB8JeQx11p6ZJZBUg&pwd=6666)（密码6666），免费获取原版高清无水印的论文和专利。本文就简单介绍一下 JSVMP，想要详细了解，当然还是建议去读一下这篇论文。

![01](https://s2.loli.net/2022/10/12/uH1BVl9oOznTb8M.png)
...
![04](https://s2.loli.net/2022/10/12/rE3cLUk1jV9gt25.png)

![05](https://s2.loli.net/2022/10/12/xnepSC6gYTQM1iK.png)

### 逆向分析

这条请求是 XHR 请求，所以直接下个 XHR 断点，当 URL 中包含 `X-Bogus` 参数时就断下：

![06](https://s2.loli.net/2022/10/12/tuvlEJVHAp73P6r.png)

![07](https://s2.loli.net/2022/10/12/jp1KB4AsoryLfxk.png)

往前跟栈，来到一个叫 webmssdk.js 的 JS 文件，这里就是生成参数的主要 JS 逻辑了，也就是 JSVMP，整体上做了一个混淆，这里可以使用 AST 来解混淆，K哥以前同样也写过 AST 的文章，这里还原混淆不是重点，咱们直接使用 V 佬的插件 [v\_jstools](https://github.com/cilame/v_jstools) 来还原：

![08](https://s2.loli.net/2022/10/12/Edv1OUjbYZWPxm8.png)
...
![11](https://s2.loli.net/2022/10/13/MnxyPuJ3SIfDbhU.png)
...
// 输出：{"key1":"value1","key2":"kspider"}
```

上面的代码中 `JSON.stringify` 传入了一个函数，当 `value` 为 `value2` 的时候就将其替换成字符串 `kspider`，接下来我们演示一下当 `value` 为 `window` 时，会发生什么：

![12](https://s2.loli.net/2022/10/13/BP8xeZyWUucgk7v.png)
...
![13](https://s2.loli.net/2022/10/13/U5MkcLpyY4AlBEi.png)
...
![14](https://s2.loli.net/2022/10/14/seCNW7nyFUYzKuE.png)
...
![15](https://s2.loli.net/2022/10/14/3FgxCHRhK5u6V7N.png)
...
![16](https://s2.loli.net/2022/10/14/YuJMrgfZj8awn9e.png)

刷新网页，断下之后开始单步跟，来到下图所示的地方：

![17](https://s2.loli.net/2022/10/14/aluLsioVmjBzHb7.png)

到这里之后，就不要下一步了，再下一步有可能整个语句就执行完毕了，其中的细节你看不到，所以这里我们在控制台挨个输入看看：

![18](https://s2.loli.net/2022/10/14/LYeRqC93wvuUh8G.png)
...
![19](https://s2.loli.net/2022/10/14/lS9Q5YHbeBqaFIR.png)

现在我们还需要知道这个20是怎么来的，继续往上看，找到20第一次出现的地方，在第8510行，那么我们就要使其在上一步断下，也就是第8509行，如下图所示：

![20](https://s2.loli.net/2022/10/15/mqD4b3V65r2AHEj.png)

第8509行的索引信息为 `位置 2 索引I 47 索引A 730`，同样的下条件断点观察怎么生成的：

![21](https://s2.loli.net/2022/10/15/KmQHDAjI46y7cNt.png)

![22](https://s2.loli.net/2022/10/15/ogVvtzEDWX8rjhB.png)
...
![23](https://s2.loli.net/2022/10/15/gGfpFYXkrQAvRWc.png)

![24](https://s2.loli.net/2022/10/15/Y7JOH6Debv4B59a.png)

![25](https://s2.loli.net/2022/10/15/YCNB3t2HpOIglFW.png)
...
![26](https://s2.loli.net/2022/10/15/c4aEY5JRAnGlBu6.png)
...
"\u0002ÿ-%.*yê^s6eyÇžyœV,”".charCodeAt(15) = 158
q[342] = 16
158 << 16 = 10354688
"\u0002ÿ-%.*yê^s6eyÇžyœV,”".charCodeAt(16) = 253
q[408] = 8
253 << 8 = 64768
10354688 | 64768 = 10419456
"\u0002ÿ-%.*yê^s6eyÇžyœV,”".charCodeAt(17) = 156
...
"\u0002ÿ-%.*yê^s6eyÇžyœV,”".charCodeAt(18) = 86
q[342] = 16
86 << 16 = 5636096
"\u0002ÿ-%.*yê^s6eyÇžyœV,”".charCodeAt(19) = 44
q[408] = 8
44 << 8 = 11264
5636096 | 11264 = 5647360
"\u0002ÿ-%.*yê^s6eyÇžyœV,”".charCodeAt(20) = 148
148 | 5647360 = 5647508
...
![27](https://s2.loli.net/2022/10/15/drcC58mWPlSH4sE.png)
...
_0x46fa4c.apply(null, ["ÿ", "@\u0000\u0001\u000e\t1E?J}cI\u0003ñ \u0000¾d"]) = "\u0002ÿ-%.*yê^s6eyÇžyœV,”"

位置 1 索引I 16 索引A 1812：
_0x2b6720.apply(null, [2, 255, "\u0002ÿ-%.*yê^s6eyÇžyœV,”"]) = "\u0002ÿ-%.*yê^s6eyÇžyœV,”"
```
...
![28](https://s2.loli.net/2022/10/17/nwNzbQcMVdWCDOA.png)

![29](https://s2.loli.net/2022/10/17/9LauzTBHqvkwW7Z.png)
