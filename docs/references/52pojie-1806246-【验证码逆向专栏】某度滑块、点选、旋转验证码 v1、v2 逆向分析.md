...
> **作者**: K哥爬虫 | **发布时间**: 2023-07-08 15:20:00 | **版块**: 『脱壳破解区』 | **查看/回复**: 776 / 8
> **原文**: [https://www.52pojie.cn/thread-1806246-1-1.html](https://www.52pojie.cn/thread-1806246-1-1.html)

---

*本帖最后由 K哥爬虫 于 2023-8-16 14:05 编辑*
...
* 目标：某度滑块验证码、点选验证码、旋转验证码，v1、v2 逆向分析
* v1 旋转验证码：
...
PS：v1、v2 是作者自己为了区分而命名的版本号，主要依据是核心 JS 文件分为 mkd.js 和 mkd\_v2.js 两个版本，如下图所示：

![01](https://s1.ax1x.com/2023/03/20/pptW3If.png)

此外，在界面上也有所区别，v2 版本的旋转、滑块图片有很明显的阴影、线条干扰，如下图所示：

![02](https://s1.ax1x.com/2023/03/20/pptWYRg.png)

上面给的地址中，点选验证码的地址有时候是点选，有时候会变成旋转，估计是异常等级不同导致的，此外，传闻还有一种无感验证，不过作者到处找也没找到个地址，估计逻辑都是差不多的，无感验证如下图所示：

![03](https://s1.ax1x.com/2023/03/20/pptW0Zq.png)

### 抓包分析

以下以 v1 旋转验证码为例（v2 接口名称不一样，但逻辑是一样的），第一次 `viewlog` 接口，请求的 `ak` 是固定值，当然不同场景不同网站是不一样的，`callback` 回调值，`_` 时间戳，返回值 `as`、`tk` 都是后面会用到的。

![04](https://s1.ax1x.com/2023/03/20/pptWBd0.png)

![05](https://s1.ax1x.com/2023/03/20/pptWsiT.png)

然后是一个 `getstyle` 接口，其中的 `tk` 就是前面 `viewlog` 接口返回的，返回值里 `backstr` 后续参数加密会用到，`img` 就是旋转图片地址，`info` 是一些版权信息。

![06](https://s1.ax1x.com/2023/03/20/pptWyJU.png)

![07](https://s1.ax1x.com/2023/03/20/pptW6WF.png)
...
![08](https://s1.ax1x.com/2023/03/20/pptWcz4.png)

![09](https://s1.ax1x.com/2023/03/20/pptW2QJ.png)

上一步验证走完后，并不意味着通过验证了，后续还会有一个 `viewlog/c` 的接口需要进一步验证，其中的 `tk`、`ds` 参数就是上一步返回的，如果验证失败，返回值 `code` 为 1，验证成功，`code` 则为 0。

![10](https://s1.ax1x.com/2023/03/20/pptWWLR.png)

![11](https://s1.ax1x.com/2023/03/20/pptW5o6.png)

![12](https://s1.ax1x.com/2023/03/20/pptWOOA.png)

### 逆向分析 fs

接下来分析主要加密参数 `fs`，跟栈到 mkd.js：

![13](https://s1.ax1x.com/2023/03/20/pptWxTP.png)

![14](https://s1.ax1x.com/2023/03/20/pptfSFf.png)

可以看到 `o` 就是 `fs`，而 `o` 又是 `r.rzData` 经过加密后得到的，输出一下 `r.rzData`，结构如下图所示：

![15](https://s1.ax1x.com/2023/03/20/pptf9fS.png)
...
![16](https://s1.ax1x.com/2023/03/20/ppth1C8.png)

![17](https://s1.ax1x.com/2023/03/20/ppth8gg.png)
...
![18](https://s1.ax1x.com/2023/03/20/ppthGvQ.png)

![19](https://s1.ax1x.com/2023/03/20/ppthtDs.png)
...
![22](https://s1.ax1x.com/2023/03/20/ppthD8U.png)

然后就是 `ac_c` 的计算方法了，首先是旋转验证码，直接搜索 `ac_c`：

![23](https://s1.ax1x.com/2023/03/20/ppthr2F.png)

往上跟栈，有个 percent 的地方，一个三目表达式，e 是固定值 290，`e - 52 = 238`，238 也就是滑动条能够滑动的最大长度：

![24](https://s1.ax1x.com/2023/03/20/ppth6KJ.png)
...
![25](https://s1.ax1x.com/2023/03/20/ppthRV1.png)
...
![26](https://s1.ax1x.com/2023/03/20/ppthf56.png)
...
![27](https://s1.ax1x.com/2023/03/20/ppt42Qg.png)

### 结果验证

![28](https://s1.ax1x.com/2023/03/20/ppth58O.png)

![29](https://s1.ax1x.com/2023/03/20/ppthoxe.png)

![30](https://s1.ax1x.com/2023/03/20/ppthbqA.png)

![31](https://s1.ax1x.com/2023/03/20/ppt49MQ.png)
