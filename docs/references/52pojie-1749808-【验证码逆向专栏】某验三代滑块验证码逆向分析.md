...
> **作者**: K哥爬虫 | **发布时间**: 2023-02-23 15:38:00 | **版块**: 『脱壳破解区』 | **查看/回复**: 7861 / 16
> **原文**: [https://www.52pojie.cn/thread-1749808-1-1.html](https://www.52pojie.cn/thread-1749808-1-1.html)

---

![01](https://s1.ax1x.com/2022/11/25/zY5cgs.png)
...
![02](https://s1.ax1x.com/2022/11/24/zGtIk4.png)
...
![03](https://s1.ax1x.com/2022/11/24/zGtTh9.png)

* gettype.php?gt=xxx&callback=xxx：

获取验证码，HTTP 请求中不同的请求方式和设置不同的 Content-Type 时，参数传递的方式会不一样，一般为 Query String Parameters、Form Data、Request Payload，这里是 Query String Parameters，在 GET 请求时，参数会以 url string 的形式进行传递，即 ? 后的字符串则为其请求参数，并以 & 作为分隔符，这里传递了 gt 参数的值以及 callback，callback 为 geetest\_ + 时间戳：

![04](https://s1.ax1x.com/2022/11/24/zGtj0O.png)

响应预览中返回了一些 js 文件及对应的版本号：

![05](https://s1.ax1x.com/2022/11/24/zGtb11.png)
...
* 第一个 get.php?xxx，url 中传递了一些参数，关键部分如下：
  ...
  + callback：geetest\_ + 时间戳

  ![06](https://s1.ax1x.com/2022/11/24/zGaCOP.png)

  响应内容如下，这里没什么需要注意的，feedback 就是某验的帮助中心：

  ![07](https://s1.ax1x.com/2022/11/24/zGdrD0.png)

2.点击按钮进行验证之后，Network 中抓包到了以下信息：

![08](https://s1.ax1x.com/2022/11/24/zGd6ET.png)
...
* 第一个 ajax.php?xxx，url 中传递了一些参数，关键部分如下：
  ...
  ![09](https://s1.ax1x.com/2022/11/24/zG0KOI.png)

  响应返回验证码模式，滑块验证码为 slide，点选验证码为 click：

  ![10](https://s1.ax1x.com/2022/11/24/zGwZMn.png)
* 第二个 get.php?xxx，url 中传递了一些参数，关键部分如下：
  ...
  ![11](https://s1.ax1x.com/2022/11/24/zGwuZV.png)

  这个响应返回了很多关键内容：

  + bg：被打乱的带缺口背景图，需要还原，372fe236d.webp

  ![12](https://s1.ax1x.com/2022/11/24/zGgdoR.png)

  + fullbg：被打乱的完整背景图，需要还原，7bfaaa72b.webp

  ![13](https://s1.ax1x.com/2022/11/24/zGgsSK.png)

  + slice：滑块图片，不需要还原，372fe236d.png

  ![14](https://s1.ax1x.com/2022/11/24/zGgWTA.png)

  + c：关键参数，与后面 aa 参数的值有关，固定值
  + s：关键参数，与后面 aa 参数的值有关

  ![15](https://s1.ax1x.com/2022/11/24/zGwcLt.png)
* 第二个 ajax.php?xxx，url 中传递了一些参数，关键部分如下：
  ...
  ![16](https://s1.ax1x.com/2022/11/24/zG2l0H.png)

  滑动滑块验证通过即会返回以下内容：
  ![17](https://s1.ax1x.com/2022/11/24/zG2mp6.png)

  失败则会返回：

  ![18](https://s1.ax1x.com/2022/11/24/zG2rAs.png)
...
![19](https://s1.ax1x.com/2022/11/24/zG2jDe.png)

进入后点击左下角 { } 大括号，格式化文件，再 ctrl + f 局部搜索 "\u0077"，只有一个结果，在第 6086 行，在第 6088 行打下断点，滑动滑块即会断住，h + u 即为 w 参数的值：

![20](https://s1.ax1x.com/2022/11/24/zGREDg.png)

2.通过 Initiator 跟栈，跟进到 $\_CId 中：

![21](https://s1.ax1x.com/2022/11/24/zGRYVJ.png)

进去同样格式化后，会跳转到第 4583 行，在该行打下断点：

![22](https://s1.ax1x.com/2022/11/24/zGWmLD.png)

向上跟栈到 $\_CCBv 中同样会找到刚刚的位置：

![23](https://s1.ax1x.com/2022/11/24/zGWmLD.png)
...
![24](https://s1.ax1x.com/2022/11/24/zGfwnO.png)

会跳转到第 6218 行，在 6227 行 return 处打下断点，重新拖动滑块，即会断住：

![25](https://s1.ax1x.com/2022/11/24/zGfqg0.png)
...
![26](https://s1.ax1x.com/2022/11/24/zGhDMV.png)
...
![27](https://s1.ax1x.com/2022/11/24/zGhhxx.png)

跳转到第 6208 行，在第 6214 行 return 处打下断点：

![28](https://s1.ax1x.com/2022/11/24/zG4PoQ.png)

Ot 即 16 为字符串，Ot = rt()，跟进到 rt 函数中，在第 4213 行，于第 4219 行打下断点后会发现，16 位字符串是由四个 t() 方法的结果相加得到的：

![29](https://s1.ax1x.com/2022/11/24/zG4cOf.png)

跟进到 t() 方法的定义位置，在第 4203 行，第 4208 行即为随机字符串算法：

![30](https://s1.ax1x.com/2022/11/24/zG5rEF.png)
...
![31](https://s1.ax1x.com/2022/11/24/zGHW7V.png)

跟进去查看一下：

![32](https://s1.ax1x.com/2022/11/24/zGbHPS.png)

跳转到第 2908 行，在第 2922 行断住后，$\_BACAs(332) 的值为 "Invalid RSA public key"：

![33](https://s1.ax1x.com/2022/11/24/zGLZlQ.png)
...
![34](https://s1.ax1x.com/2022/11/24/zGx0zT.png)
...
![35](https://s1.ax1x.com/2022/11/24/zGzZlT.png)

搜索后可知，其定义在第 128 行：

![36](https://s1.ax1x.com/2022/11/24/zGzT3V.png)
...
![37](https://s1.ax1x.com/2022/11/24/zJSfKO.png)

定义在第 7 行，直接将 QBLnx.$\_Ak 整个扣下来即可，然后会报错提示，window 和 ht 未定义，ht 为 navigator：
![38](https://s1.ax1x.com/2022/11/24/zJpno9.png)
...
![39](https://s1.ax1x.com/2022/11/24/zJpgoj.png)
...
![40](https://s1.ax1x.com/2022/11/24/zJ9e6f.png)

通过搜索，其定义在第 68 行，将 QBLnx.$\_BP 整个扣下来即可，至此，u 参数成功复现：

![41](https://s1.ax1x.com/2022/11/24/zJ90AJ.png)
...
![42](https://s1.ax1x.com/2022/11/24/zJCy5j.png)

将这里写成跟之前一样的固定值， `gt["stringify"](o)` 返回的是 JSON 格式的数据，由 o 参数生成：

![43](https://s1.ax1x.com/2022/11/24/zJPMJs.png)

对比分析以下 o 参数中，哪些是定值，哪些是动态变化的，可以看到箭头所指的值都不一样了：

![44](https://s1.ax1x.com/2022/11/24/zJPI6P.png)
...
![45](https://s1.ax1x.com/2022/11/24/zJi7g1.png)
...
![46](https://s1.ax1x.com/2022/11/24/zJFwKx.png)
...
![47](https://s1.ax1x.com/2022/11/24/zJAJ39.png)
...
![53](https://s1.ax1x.com/2022/11/24/zJJUWn.png)

ct 定义在第 4223 行，扣下来，报错提示，(intermediate value)[$\_BEHAL(...)] is not a function：

![54](https://s1.ax1x.com/2022/11/24/zJJcFJ.png)
...
![55](https://s1.ax1x.com/2022/11/24/zJYRAg.png)
...
![56](https://s1.ax1x.com/2022/11/24/zJtA4H.png)

aa 参数分析完成，接下来分析 rp 参数，定义在第 6076 行：

![57](https://s1.ax1x.com/2022/11/24/zJt62R.png)
...
![63](https://s1.ax1x.com/2022/11/24/zJww4S.png)

l 参数分析完毕，终于只剩下一个 h 了，`m[$_CAIAt(782)](l)` 即将 l 加密后得到的，跟进 `m[$_CAIAt(782)]`，定义在第 1568 行，在第 1575 行打下断点，为 e 中两个 value 值相加：

![64](https://s1.ax1x.com/2022/11/24/zJw7uR.png)

e 定义在第 1574 行，t 为传入的 l 参数，跟进到 `this[$_GFJn(264)]` 中，在第 1523 行，复现如下：

![65](https://s1.ax1x.com/2022/11/24/zJr9KJ.png)

校验结果一致：

![66](https://s1.ax1x.com/2022/11/24/zJ04dP.png)
...
![67](https://s1.ax1x.com/2022/11/25/zYEvBq.png)

点击按钮进行验证即会断住，格式化后跳转到第 295 行，

![68](https://s1.ax1x.com/2022/11/25/zYVGDI.png)
...
![69](https://s1.ax1x.com/2022/11/25/zYeC6J.png)

e["height"] = 160、e["width"] = 260 指还原后的图片比例为 260 x 160，a 的值为 r / 2 即 80，就是将整张图片分为了上下两等分，再将图片纵向切割为了 26 等份，Ut 数组的长度为 52，Ut[*] 即依次取数组中的元素，Ut 数组即为图片还原的顺序，是固定的，`25 < Ut[*] ? a : 0 `判断图片是上半部分还是下半部分，`\_ % 26 \* 10` 表示每个小块取 10 px 像素，正确图片的顺序为：

![70](https://s1.ax1x.com/2022/11/25/zYe0ns.png)
...
![71](https://s1.ax1x.com/2022/11/25/zYNZND.png)
...
![72](https://s1.ax1x.com/2022/11/25/zYU1z9.png)
...
![73](https://s1.ax1x.com/2022/11/28/zaEOb9.jpg)
