...
> **作者**: K哥爬虫 | **发布时间**: 2023-02-08 14:53:00 | **版块**: 『脱壳破解区』 | **查看/回复**: 11991 / 45
> **原文**: [https://www.52pojie.cn/thread-1743428-1-1.html](https://www.52pojie.cn/thread-1743428-1-1.html)
...
![02](https://s2.loli.net/2022/06/25/gVAjGrczmIKQw13.png)

![03](https://s2.loli.net/2022/06/25/xocINyknMGVKCdU.png)

2、瑞数的 JS 混淆代码中，变量、方法名大多类似于 `_$xx`，有众多的 `if-else` 控制流，新版瑞数还可能会有 jsvmp 以及众多三目表达式的情况：

![04](https://s2.loli.net/2022/06/27/CkGUsfSa52tDlKP.png)
...
![05](https://s2.loli.net/2022/06/25/4phou197Ujk3raA.png)

![06](https://s2.loli.net/2022/06/25/vLtr3V6RawlPfnN.png)

![07](https://s2.loli.net/2022/06/25/klX3bwRjaIzKMiW.png)

![08](https://s2.loli.net/2022/06/25/nLHtQY5eGlW8FJV.png)
...
![09](https://s2.loli.net/2022/06/27/Gt1dsXVlBQe7MF5.png)
...
![10](https://s2.loli.net/2022/06/27/c5hJZ6wQo8DI29N.png)

![11](https://s2.loli.net/2022/06/27/C2J1BRpTvPmatde.png)
...
![12](https://s2.loli.net/2022/06/28/OA4fiPCyxDdsNw3.png)

* 4 代：`ret = _$DG.call(_$6a, _$YK);`，`_$DG` 实际上是 `eval`，有关键字 `ret`，`call` 是明文；

![13](https://s2.loli.net/2022/06/28/yfibHmY9PT3nqJK.png)
...
![14](https://s2.loli.net/2022/06/28/CeNv1OQsjrcfwpD.png)

![15](https://s2.loli.net/2022/06/28/BGHLeu2XUNv9Mho.png)

![16](https://s2.loli.net/2022/06/28/r1HPoqDZSpe694l.png)
...
![17](https://s2.loli.net/2022/06/28/dGk57ai6XbpJyPl.png)
...
![18](https://s2.loli.net/2022/06/28/524tBWAPgHYmNdc.png)

仔细观察这两次 Cookie 生成的地方，分别往上跟栈，你就会发现两个 Cookie 分别是经过了两个不同方法得到的，如下图所示：

![19](https://s2.loli.net/2022/06/28/jdqkVcfDU9oCpn1.png)

![20](https://s2.loli.net/2022/06/28/bevGhxN7Rj2dUtn.png)

这里的代码存在于 VM 虚拟机中，且是 IIFE 自执行代码，我们还得往前跟栈看看这些 VM 代码是从哪里加载出来的，跟栈来到首页（202页面）带有 call 的位置：

![21](https://s2.loli.net/2022/06/28/x59TgbvsDEzQJNU.png)
...
![24](https://s2.loli.net/2022/06/29/1MN74jTEUs3zgLQ.png)
...
![25](https://s2.loli.net/2022/06/29/E2NivagLVdIpCbZ.jpg)
...
![25](https://s2.loli.net/2022/06/30/BeRF1rYuKGhqXLz.png)

观察 `$_ts` 的 key 和 value，和浏览器中得到的是一样的：

![26](https://s2.loli.net/2022/06/30/Uz8eSuy27JMIHVc.png)
...
![27](https://s2.loli.net/2022/06/30/4sWMyD3kZJE1rS6.png)
...
![28](https://s2.loli.net/2022/06/30/h4RWubIqy6nB9JG.png)

通过前面我们的分析，已经知道了入口在 call 的地方，快速搜索并下断点：

![29](https://s2.loli.net/2022/06/30/VpSCie6b4D5LGkw.png)

通过前面我们的分析，我们也知道了有两次生成 Cookie 的地方，快速搜索 `(5)`，搜索结果第二个即为入口：

![30](https://s2.loli.net/2022/06/30/egxvfLmXQohK3iT.png)

#### 假 Cookie 生成逻辑

首先单步跟假 Cookie，虽然是假的，但是后续生成真 Cookie 中会用到，在跟的时候你会走到这个逻辑里面：

![31](https://s2.loli.net/2022/06/30/9Mfd2hgTRApY7ct.png)

有一步会调用 `_$8e()` 方法，而 `_$8e = _$Q9`，`_$Q9` 又嵌套在 `_$d0` 里的，搜索一下哪里调用了 `_$d0`，发现是代码开头：

![32](https://s2.loli.net/2022/06/30/PR3JOmejlXprqEU.png)

那么传入的参数 `_$Wn` 是啥呢？单步跟入，是一个方法，作用就是取 202 页面的 content 内容，那么我们在本地就直接删掉这个 `_$Wn` 方法，直接传入 content 的值即可，如下图所示：

![33](https://s2.loli.net/2022/06/30/pMSh49Z5m7CoOjH.png)

另外，我们发现，代码有非常多的在数组里面按索引取值的情况，比如上图中的 `_$PV[68]` 的值，实际上就是字符串 content，很显然我们要把这个数组的来源找到，直接搜索 `_$PV =`，可以找到疑似定义和赋值的地方：

![34](https://s2.loli.net/2022/06/30/M3x5LR6e8XdwWFm.png)

![35](https://s2.loli.net/2022/06/30/qWjcGn45xrwLaiV.png)

所以我们得看看这个 `_$iL` 方法，传入了一个非常长的字符串，打断点进去看看，果然生成了 `_$PV`，是一个 725 位的数组：

![36](https://s2.loli.net/2022/06/30/F9flpiCVXKcHOjJ.png)

接下来在扣代码的过程中，你会经常遇到一个变量，在本文中是 `_$sX`：

![37](https://s2.loli.net/2022/06/30/9VpsxQRHyTkYPn6.png)

有没有很熟悉？这个值就是我们前面拿到的 `$_ts` 变量，在开头就可以看到是将 `window.$_ts` 赋值给了 `_$sX`：

![38](https://s2.loli.net/2022/06/30/mf8tD9bWBQPJXpc.png)

继续走，会走到以下逻辑中：

![39](https://s2.loli.net/2022/06/30/v5XogiWEnwmx31q.png)

这里会遇到六个数组，他们都已经有值了，所以我们得找到他们是咋来的，任意搜索其中一个数组名称，会找到定义和赋值的地方：

![40](https://s2.loli.net/2022/06/30/maJAnI7eld5sT3w.png)

![41](https://s2.loli.net/2022/06/30/pBXaZPHe85yDxiK.png)

赋值明显是调用了 `_$rv` 方法，再搜 `_$rv` 方法，发现是开头就调用了：

![42](https://s2.loli.net/2022/06/30/7qAMQsSBGJ4K9mL.png)

后续没有什么特别的，一直单步，最后有个 `join('')` 操作，就生成了假 Cookie：

![43](https://s2.loli.net/2022/06/30/tDb1kliY63CZXSH.png)
...
![44](https://s2.loli.net/2022/06/30/ngkaizOL6MqKUXY.png)

#### 真 Cookie 生成逻辑

单步跟真 Cookie，在本文中也就是 `_$ZN(768, 1);`，可以看到开始进入了无穷无尽的 `if-else` 控制流：

![45](https://s2.loli.net/2022/06/30/Y9KruCtVHFoBx8l.png)
...
![46](https://s2.loli.net/2022/06/30/fV1Hum7tgGFKPMb.png)

##### 取假 Cookie

单步跟 747 号控制流，会有个进入第 709 号控制流的步骤，会取先前生成的假 Cookie，经过一系列操作之后返回一个数组：

![47](https://s2.loli.net/2022/06/30/EFfQYHNDIKci7go.png)

![48](https://s2.loli.net/2022/06/30/bOtLNQJHjgWmU5n.png)

至此我们在本地同步扣的代码，如果正常的话，返回的数组也应该是一样的（后续的数据就不一样了，有一些时间戳之类的参数参与运算）：

![49](https://s2.loli.net/2022/06/30/SluzHAydLeaVW6C.png)

##### 自动化工具检测

继续跟 747 号控制流，会进入 268 号控制流，接着进入 154 号控制流，这里面会针对自动化工具做一些检测，如下图所示：

![50](https://s2.loli.net/2022/07/01/aiSEHAsfcqNvj7Q.png)

![51](https://s2.loli.net/2022/07/01/8bBTeaFQWvGdnxM.png)

这里定义了一个变量 `_$iL`，检测不通过就是1，后续又把这个变量赋值给了 `_$aW`，所以我们本地保持一致，也为 false 即可（其实我们不用自动化工具的话，这一段检测就不用管直接返回 false 就行）：

![52](https://s2.loli.net/2022/07/01/m45PrkF39LwBG7O.png)
...
![53](https://s2.loli.net/2022/07/01/qmJMSR9ZBEWK6Hc.png)

![54](https://s2.loli.net/2022/07/01/4XOFlf8JeBtixVq.png)

这里不是单纯的取 `$_ts` 里的键值对，你在扣代码的时候，你也许会发现怎么本地到这里取值的时候，取出来的不是数字，而是字符串呢？就像下面这种情况：

![55](https://s2.loli.net/2022/07/01/q3hLJSIyzmGM4wd.png)

实际上我们最开始得到的 `$_ts` 值，是经过了二次处理的，我们以第一个 `_$sX._$Xb` 为例，直接搜索 `_$sX._$Xb`，可以发现这么一个地方：

![56](https://s2.loli.net/2022/07/01/A5sGR6iwa2JCZ3t.png)
...
![57](https://s2.loli.net/2022/07/01/1uLAdx5fgMyKzb7.png)
...
![58](https://s2.loli.net/2022/07/01/3HWaiShXfZ1mI4P.png)

![59](https://s2.loli.net/2022/07/01/8pLnG93rUey7JSI.png)
...
![60](https://s2.loli.net/2022/07/01/DqM4xPGiLKoFYe9.png)

2、再匹配出 VM 代码开头的 20 个赋值的语句，如：`_$sX._$RH = _$wI; _$sX._$i5 = _$n5;` 等；

![61](https://s2.loli.net/2022/07/01/GOXnUKul2C5m9Ik.png)
...
![62](https://s2.loli.net/2022/07/01/uMWteX1CZ2YKAhS.png)

4、我们知道了这四个方法在 20 个赋值语句中的位置，那么我们直接匹配本地对应位置的名称，进行动态替换即可，当然前提是咱们本地已经扣了一套代码出来了：

![63](https://s2.loli.net/2022/07/01/ta8HCN2VbsXpi3c.png)

![64](https://s2.loli.net/2022/07/01/6EFZ85o3GYctPkS.png)
...
![65](https://s2.loli.net/2022/07/01/lTzHNMZecYFr3Kq.png)

#### 注意事项

特别注意 VM 代码开头，会直接调用执行一些方法，某些变量的值就是通过这些方法生成的，当你一步一步跟的时候发现某些参数不对，或者没有，那么就得注意开头这些方法了，可能一开始就已经生成了。

![66](https://s2.loli.net/2022/07/01/fMXko7cYtsQeSxR.png)
...
![67](https://s2.loli.net/2022/07/01/OmKyNM7D1UJWACi.png)

可以看到后传入 `l.open()` 的 URL 还是正常的，断下后到 `l.send()` 就带有后缀了，再看 `l.open()` 其实就是 `xhr.open()`，明显和正常的有区别，同样这个方法也在 VM 代码里，应该是重写了方法，可以和正常的做对比：

![68](https://s2.loli.net/2022/07/01/5a9TnEvsc2JbI8M.png)

跟到 VM 代码里去看看，经过了 `_$sd(arguments[1])` 方法就变成了带有后缀的完整链接了：

![69](https://s2.loli.net/2022/07/01/uReo9BEOqsQrpMY.png)

跟进 `_$sd` 方法，前面都是对 url 做一些处理，后面有个进入第 779 号控制流的流程，实际上就是原来我们生成 Cookie 的步骤，跟一下就行了。

![70](https://s2.loli.net/2022/07/01/ewQMIyKaG2JoNnC.png)

### 善用 Watch 跟踪功能

![71](https://s2.loli.net/2022/07/01/z5nkOXENYcpCVr4.png)
...
![72](https://s2.loli.net/2022/07/01/ypLR4X5sq87DucT.png)

![73](https://s2.loli.net/2022/07/01/l2RzbIPnBU94GA1.png)
