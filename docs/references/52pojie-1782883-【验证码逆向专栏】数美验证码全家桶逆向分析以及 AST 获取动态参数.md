...
> **作者**: K哥爬虫 | **发布时间**: 2023-05-08 17:18:00 | **版块**: 『脱壳破解区』 | **查看/回复**: 9597 / 42
> **原文**: [https://www.52pojie.cn/thread-1782883-1-1.html](https://www.52pojie.cn/thread-1782883-1-1.html)
...
![01](https://s1.ax1x.com/2023/04/06/ppoP2CQ.png)
...
![02](https://s1.ax1x.com/2023/04/06/ppoPc4g.png)

返回结果重点看 `captcha-sdk.min.js` 文件地址，如下图所示有个 `v1.0.4-171`，本文中我们称 `v1.0.4` 为大版本，`171` 为小版本，小版本不定时会更新，版本号不断升高。

![03](https://s1.ax1x.com/2023/04/06/ppoP6US.png)

然后就是 `register` 接口，不同类型，返回的数据都大同小异，其中 `bg` 是背景图片，`fg` 是滑块，文字点选、空间推理中 `order` 是提示信息，`k`、`l`、`rid` 三个参数后续会用到。

![04](https://s1.ax1x.com/2023/04/06/ppoPR3j.png)

![05](https://s1.ax1x.com/2023/04/06/ppoPyE8.png)

![06](https://s1.ax1x.com/2023/04/06/ppoPWgs.png)
...
![07](https://s1.ax1x.com/2023/04/06/ppoPfvn.png)

![08](https://s1.ax1x.com/2023/04/06/ppoP4uq.png)

### 逆向分析

跟栈会发现核心逻辑在 `captcha-sdk.min.js` 里，这个 JS 类似于 OB 混淆（以前的文章介绍过，此处不再细说）：

![09](https://s1.ax1x.com/2023/04/06/ppoP5D0.png)

这里可以自己写 AST 还原一下，为了方便我们直接使用 v\_jstools 解混淆：

![10](https://s1.ax1x.com/2023/04/06/ppoPTET.png)
...
![11](https://s1.ax1x.com/2023/04/06/ppoP7UU.png)

![12](https://s1.ax1x.com/2023/04/06/ppoPH5F.png)

如果你没注意到这个跨域问题，可能会替换之后发现没替换成功，原因是数美的资源有四个域名，其中一个宕了便会启用另一个，你替换其中一个报错了就会自动跳转另一个，所以看起来你并没有替换成功：

![13](https://s1.ax1x.com/2023/04/06/ppoPqC4.png)
...
![14](https://s1.ax1x.com/2023/04/06/ppok5DI.png)

这里的栈并不多，来回跟栈也没发现是哪里生成的，此时可以从初始位置也就是 `embed.html` 初始化验证码的地方开始单步跟：

![15](https://s1.ax1x.com/2023/04/06/ppokWgH.png)

单步跟进去会发现一个 `getCaptchaUuid()` 的方法，将此方法扣出来即可。

![16](https://s1.ax1x.com/2023/04/06/ppokfvd.png)
...
![17](https://s1.ax1x.com/2023/04/06/ppokIbt.png)
...
![21](https://s1.ax1x.com/2023/04/06/ppokH58.png)
...
![24](https://s1.ax1x.com/2023/04/06/ppokO2Q.png)

再往下走还有三个加密参数，待加密值是定值，然后将 s 的值（也就是前面 `register` 接口返回的 k 经过 DES 解密后的值赋值给了 `this._data.__key`）。

![25](https://s1.ax1x.com/2023/04/06/ppokXvj.png)

至此所有加密参数就搞完了。

### 结果验证

![26](https://s1.ax1x.com/2023/04/06/ppoA4WF.png)

![27](https://s1.ax1x.com/2023/04/06/ppoAhJU.png)

![28](https://s1.ax1x.com/2023/04/06/ppoA2d0.png)

![29](https://s1.ax1x.com/2023/04/06/ppoAfiT.png)

![30](https://s1.ax1x.com/2023/04/06/ppoARoV.png)

![31](https://s1.ax1x.com/2023/04/06/ppoA5z4.png)
...
![32](https://s1.ax1x.com/2023/04/06/ppoAoQJ.png)

![33](https://s1.ax1x.com/2023/04/06/ppoA7LR.png)

![34](https://s1.ax1x.com/2023/04/06/ppoAbe1.png)
...
截止本文发布，小版本 `171` 为最新，`v1.0.4` 小版本从 `148` 开始，`v1.0.3`、`v1.0.1` 在 `147` 以前没有混淆，可自行正则匹配，暂未发现其他大版本，如有遇到不能适配的，可联系我瞅瞅。

**PS：此 AST 代码仅实现对动态参数的提取，并非还原所有的混淆，提取出来的结果是有序、未去重的，后续按索引取就行。**

![35](https://s1.ax1x.com/2023/04/06/ppoAqdx.png)
