...
> **作者**: 13955925361 | **发布时间**: 2026-04-28 16:13:00 | **版块**: 『脱壳破解区』 | **查看/回复**: 3186 / 17
> **原文**: [https://www.52pojie.cn/thread-2105256-1-1.html](https://www.52pojie.cn/thread-2105256-1-1.html)

---

*本帖最后由 13955925361 于 2026-4-28 17:41 编辑*

案例有点难，直接上效果图吧。当然直接用rpc调是最方便的，我这只是逆向，就不做过多展示了。直接上代码。

![](https://attach.52pojie.cn/forum/202604/28/161311tvhbbwgncvipbww3.png)

![](https://attach.52pojie.cn/forum/202604/28/155715eebhkuhdhegab2dq.png)

protobuf格式代码:命名为douyin.proto  在当前文件夹目录终端输入protoc  --python\_out=.   douyin.proto    创建一个新的.py文件
...
    int32 service = 3;
    int32 method = 4;
    string payload\_encoding = 6;
    string payload\_type = 7;
    bytes payload = 8;
    string LodIDNew = 9;
}
...
  // 是否需要WRDS存储 - 布尔类型
  bool need\_wrds\_store = 6;

  // WRDS版本 - 64位整型（字符串形式）
  int64 wrds\_version = 7;

  // WRDS子键 - 字符串类型
  string wrds\_sub\_key = 8;

  // 消息额外信息 - 字符串到字符串的映射
  map<string, string> message\_extra = 9;

  // 租户ID - 字符串类型
  string tenant\_id = 10;
}
...
  // 获取间隔 - 64位整型
  int64 fetch\_interval = 3;

  // 当前时间戳 - 64位整型
  int64 now = 4;

  // 内部扩展字段 - 字符串类型
  string internal\_ext = 5;

  // 获取类型 - 32位整型
  int32 fetch\_type = 6;

  // 预留字段7 - 用于未来扩展
  map<string, string> route\_params = 7;

  // 心跳持续时间 - 64位整型
  int64 heartbeat\_duration = 8;

  // 是否需要确认 - 布尔类型
  bool need\_ack = 9;

  // 推送服务器地址 - 字符串类型
  string push\_server = 10;

  // 直播游标 - 字符串类型
  string live\_cursor = 11;

  // 历史记录是否结束 - 布尔类型
  bool history\_no\_more = 12;

  // 代理服务器地址 - 字符串类型
  string proxy\_server = 13;

  // V2推送服务器地址 - 字符串类型
  string push\_server\_v2 = 14;
}
...
function get_sign(md5_value) {
    console.log("[danmu.js] Input MD5:", md5_value);
    ...
    if (typeof window !== 'undefined' && window.byted_acrawler && typeof window.byted_acrawler.frontierSign === 'function') {
        ...
        console.log("[danmu.js] &#10003; 使用 window.byted_acrawler.frontierSign");
    } else if (typeof window !== 'undefined' && typeof window.yuan === 'function') {
        signFunc = window.yuan;
        console.log("[danmu.js] &#10003; 使用 window.yuan");
    } else if (typeof _0x5c2014 === 'function') {
        signFunc = _0x5c2014;
        console.log("[danmu.js] &#10003; 使用全局 _0x5c2014");
    } else {
        console.error("[danmu.js] &#10007; 所有签名函数都未找到");
        console.error("[danmu.js] window 存在:", typeof window !== 'undefined');
        console.error("[danmu.js] window.byted_acrawler 存在:", typeof window !== 'undefined' && window.byted_acrawler ? true : false);
    ...
    try {
        ...
        console.log("[danmu.js] 签名函数返回值:", JSON.stringify(ret));
        console.log("[danmu.js] ret type:", typeof ret);

        // 尝试多种方式提取签名
        if (ret && typeof ret === 'object') {
            // 方式1: X-Bogus 字段
            if (ret["X-Bogus"]) {
                console.log("[danmu.js] &#10003; 通过 X-Bogus 获取签名:", ret["X-Bogus"]);
                return ret["X-Bogus"];
            }
            // 方式2: signature 字段
            if (ret["signature"]) {
                console.log("[danmu.js] &#10003; 通过 signature 获取签名:", ret["signature"]);
                return ret["signature"];
            }
            // 方式3: 返回整个对象的 JSON 字符串
            console.log("[danmu.js] &#9888; 返回完整对象JSON:", JSON.stringify(ret));
            return JSON.stringify(ret);
        } else if (typeof ret === 'string') {
            console.log("[danmu.js] &#10003; 签名生成成功（字符串）:", ret);
            return ret;
        } else {
            console.error("[danmu.js] &#10007; 意外的返回类型:", typeof ret);
            console.error("[danmu.js] &#10007; 返回值:", ret);
            return "ERROR:UNEXPECTED_RETURN_TYPE";
        }
    } catch (e) {
        console.error("[danmu.js] &#10007; 调用签名函数出错:", e.message);
        console.error("[danmu.js] &#10007; 错误堆栈:", e.stack);
...
window = (function(){
  ...
  Object.defineProperties(Screen.prototype, {
    ...
    availHeight: {"enumerable":true,"configurable":true,"get":function(){return 912},"set":function(){return true}},
    width: {"enumerable":true,"configurable":true,"get":function(){return 1707},"set":function(){return true}},
    height: {"enumerable":true,"configurable":true,"get":function(){return 960},"set":function(){return true}},
    colorDepth: {"enumerable":true,"configurable":true,"get":function(){return 32},"set":function(){return true}},
    pixelDepth: {"enumerable":true,"configurable":true,"get":function(){return 32},"set":function(){return true}},
  ...
  Object.defineProperties(Node.prototype, {
    ...
    baseURI: {"enumerable":true,"configurable":true,"get":function(){return "https://live.douyin.com/80017709309?from_search=true"},"set":function(){return true}},
  ...
  Object.defineProperties(Document.prototype, {
    URL: {"enumerable":true,"configurable":true,"get":function(){return "https://live.douyin.com/80017709309?from_search=true"},"set":function(){return true}},
    documentURI: {"enumerable":true,"configurable":true,"get":function(){return "https://live.douyin.com/80017709309?from_search=true"},"set":function(){return true}},
    ...
    domain: {"enumerable":true,"configurable":true,"get":function(){return "live.douyin.com"},"set":function(){return true}},
    referrer: {"enumerable":true,"configurable":true,"get":function(){return "https://ntp.msn.cn/"},"set":function(){return true}},
    lastModified: {"enumerable":true,"configurable":true,"get":function(){return "04/24/2026 13:34:40"},"set":function(){return true}},
    readyState: {"enumerable":true,"configurable":true,"get":function(){return "complete"},"set":function(){return true}},
    title: {"enumerable":true,"configurable":true,"get":function(){return "东方甄选的抖音直播间 - 抖音直播"},"set":function(){return true}},
  ...
  Object.defineProperties(Navigator.prototype, {
    ...
    appVersion: {"enumerable":true,"configurable":true,"get":function(){return "5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/147.0.0.0 Safari/537.36 Edg/147.0.0.0"},"set":function(){return true}},
    ...
    userAgent: {"enumerable":true,"configurable":true,"get":function(){return "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/147.0.0.0 Safari/537.36 Edg/147.0.0.0"},"set":function(){return true}},
    language: {"enumerable":true,"configurable":true,"get":function(){return "zh-CN"},"set":function(){return true}},
    languages: {"enumerable":true,"configurable":true,"get":function(){return ["zh-CN","en","en-GB","en-US"]},"set":function(){return true}},
    ...
    deviceMemory: {"enumerable":true,"configurable":true,"get":function(){return 16},"set":function(){return true}},
  ...
  Object.defineProperties(Performance.prototype, {
    timeOrigin: {"enumerable":true,"configurable":true,"get":function(){return 1777007881150.4},"set":function(){return true}},
    interactionCount: {"enumerable":true,"configurable":true,"get":function(){return 1},"set":function(){return true}},
  ...
  var window = new Proxy(v_new(Window), {
    get(a,b){ if(b=='global'){return}
      ...
      if (typeof b !== 'symbol' && avoid_log.indexOf(b) == -1){
        v_console_log(' [/b][/b][b][b] [window get '+b+'] ==>', r)
      }
      return r
    },
    set(a,b,c){
  ...
  function v_proxy(obj, name, func){
    return new Proxy(obj, {
      get(a,b){ if(b=='global'){return}
        var r = a[b]
        if (typeof b !== 'symbol' && avoid_log.indexOf(b) == -1){
          v_console_log(' [/b][/b][/b][/b][/b][b][b][b] ['+name+' get '+b+'] ==>', r)
        }
      ...
      set(a,b,c){
        if (typeof b !== 'symbol' && avoid_log.indexOf(b) == -1){
          v_console_log(' [/b][/b][/b][b][b][b] ['+name+' set '+b+'] <--', c)
        }
        a[b] = c
        return true
      },
  ...
  Object.prototype.hasOwnProperty = v_saf(function hasOwnProperty(){
    ...
    v_console_log(' [/b][/b][/b][/b][b][b][b] [hasOwnProperty]', this===window?window:this, [].slice.call(arguments), r)
    return r
  })
  ...
  function init_cookie(cookie){
    ...
    Object.defineProperty(Document.prototype, 'cookie', {
      get: function() {
        ...
        v_console_log(' [/b][/b][/b][b][b][b] document -> cookie[get]', r)
        return r
      },
      set: function(c) {
        v_console_log(' [/b][/b][/b][b][b][b] document -> cookie[set]', c)
  ...
  function v_hook_href(obj, name, initurl){
    var r = Object.defineProperty(obj, 'href', {
      get: function(){
        ...
        v_console_log(` [/b][/b][/b][b][b][b] ${name||obj.constructor.name} -> href[get]:`, JSON.stringify(r))
        return r
      },
      set: function(href){
        href = href.trim()
        v_console_log(` [/b][/b][/b][b][b][b] ${name||obj.constructor.name} -> href[set]:`, JSON.stringify(href))
  ...
  function v_hook_storage(){
    Storage.prototype.clear      = v_saf(function(){          v_console_log(` [/b][/b][/b][b][b][b] Storage -> clear[func]:`); var self=this;Object.keys(self).forEach(function (key) { delete self[key]; }); }, 'clear')
    Storage.prototype.getItem    = v_saf(function(key){       v_console_log(` [/b][/b][/b][b][b][b] Storage -> getItem[func]:`, key); var r = (this.hasOwnProperty(key)?String(this[key]):null); return r}, 'getItem')
    Storage.prototype.setItem    = v_saf(function(key, val){  v_console_log(` [/b][/b][/b][b][b][b] Storage -> setItem[func]:`, key, val); this[key] = (val === undefined)?null:String(val) }, 'setItem')
    Storage.prototype.key        = v_saf(function(key){       v_console_log(` [/b][/b][/b][b][b][b] Storage -> key[func]:`, key); return Object.keys(this)[key||0];} , 'key')
    Storage.prototype.removeItem = v_saf(function(key){       v_console_log(` [/b][/b][/b][b][b][b] Storage -> removeItem[func]:`, key); delete this[key];}, 'removeItem')
    ...
    window.sessionStorage = new Proxy(sessionStorage,{ set:function(a,b,c){ v_console_log(` [/b][/b][/b][b][b][b] Storage -> [set]:`, b, c); return a[b]=String(c)}, get:function(a,b){ v_console_log(` [/b][/b][/b][/b][b][b][b] Storage -> [get]:`, b, a[b]); return a[b]},})
    window.localStorage = new Proxy(localStorage,{ set:function(a,b,c){ v_console_log(` [/b][/b][/b][/b][/b][b][b][b] Storage -> [set]:`, b, c); return a[b]=String(c)}, get:function(a,b){ v_console_log(` [/b][/b][/b][/b][b][b][b] Storage -> [get]:`, b, a[b]); return a[b]},})
  }
  function v_init_document(){
    ...
    Document.prototype.getElementById = v_saf(function getElementById(name){ var r = v_getele(name, 'getElementById'); v_console_log(' [/b][/b][/b][/b][/b][b][b][b] Document -> getElementById', name, r); return r })
    Document.prototype.querySelector = v_saf(function querySelector(name){ var r = v_getele(name, 'querySelector'); v_console_log(' [/b][/b][/b][b][b][b] Document -> querySelector', name, r); return r })
    Document.prototype.getElementsByClassName = v_saf(function getElementsByClassName(name){ var r = v_geteles(name, 'getElementsByClassName'); v_console_log(' [/b][/b][/b][b][b][b] Document -> getElementsByClassName', name, r); return r })
    Document.prototype.getElementsByName = v_saf(function getElementsByName(name){ var r = v_geteles(name, 'getElementsByName'); v_console_log(' [/b][/b][/b][b][b][b] Document -> getElementsByName', name, r); return r })
    Document.prototype.getElementsByTagName = v_saf(function getElementsByTagName(name){ var r = v_geteles(name, 'getElementsByTagName'); v_console_log(' [/b][/b][/b][b][b][b] Document -> getElementsByTagName', name, r); return r })
    Document.prototype.getElementsByTagNameNS = v_saf(function getElementsByTagNameNS(name){ var r = v_geteles(name, 'getElementsByTagNameNS'); v_console_log(' [/b][/b][/b][b][b][b] Document -> getElementsByTagNameNS', name, r); return r })
    Document.prototype.querySelectorAll = v_saf(function querySelectorAll(name){ var r = v_geteles(name, 'querySelectorAll'); v_console_log(' [/b][/b][/b][b][b][b] Document -> querySelectorAll', name, r); return r })
    var v_head = v_new(HTMLHeadElement)
    var v_body = v_new(HTMLBodyElement)
    Object.defineProperties(Document.prototype, {
      head: {get(){ v_console_log(" [/b][/b][/b][b][b][b] Document -> head[get]", v_head);return v_proxy(v_head, 'document.head') }},
      body: {get(){ v_console_log(" [/b][/b][/b][b][b][b] Document -> body[get]", v_body);return v_proxy(v_body, 'document.body') }},
    })
  }
  ...
  function v_init_event_target(){
    ...
    window.touch = touch
    EventTarget.prototype.addEventListener = function(){v_console_log(' [/b][/b][/b][b][b][b] EventTarget -> addEventListener[func]', this===window?'[Window]':this===document?'[Document]':this, [].slice.call(arguments)); add_event(this, [].slice.call(arguments)); return null}
    EventTarget.prototype.dispatchEvent = function(){v_console_log(' [/b][/b][/b][b][b][b] EventTarget -> dispatchEvent[func]', this===window?'[Window]':this===document?'[Document]':this, [].slice.call(arguments)); add_event(this, [].slice.call(arguments)); return null}
    EventTarget.prototype.removeEventListener = function(){v_console_log(' [/b][/b][/b][b][b][b] EventTarget -> removeEventListener[func]', this===window?'[Window]':this===document?'[Document]':this, [].slice.call(arguments)); add_event(this, [].slice.call(arguments)); return null}
  }
  function v_init_Element_prototype(){
    Element.prototype.appendChild            = Element.prototype.appendChild            || v_saf(function appendChild(){v_console_log(" [/b][/b][/b][b][b][b] Element -> appendChild[func]", [].slice.call(arguments));})
    Element.prototype.removeChild            = Element.prototype.removeChild            || v_saf(function removeChild(){v_console_log(" [/b][/b][/b][b][b][b] Element -> removeChild[func]", [].slice.call(arguments));})
    Element.prototype.getAnimations          = Element.prototype.getAnimations          || v_saf(function getAnimations(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getAnimations[func]", [].slice.call(arguments));})
    Element.prototype.getAttribute           = Element.prototype.getAttribute           || v_saf(function getAttribute(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getAttribute[func]", [].slice.call(arguments));})
    Element.prototype.getAttributeNS         = Element.prototype.getAttributeNS         || v_saf(function getAttributeNS(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getAttributeNS[func]", [].slice.call(arguments));})
    Element.prototype.getAttributeNames      = Element.prototype.getAttributeNames      || v_saf(function getAttributeNames(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getAttributeNames[func]", [].slice.call(arguments));})
    Element.prototype.getAttributeNode       = Element.prototype.getAttributeNode       || v_saf(function getAttributeNode(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getAttributeNode[func]", [].slice.call(arguments));})
    Element.prototype.getAttributeNodeNS     = Element.prototype.getAttributeNodeNS     || v_saf(function getAttributeNodeNS(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getAttributeNodeNS[func]", [].slice.call(arguments));})
    Element.prototype.getBoundingClientRect  = Element.prototype.getBoundingClientRect  || v_saf(function getBoundingClientRect(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getBoundingClientRect[func]", [].slice.call(arguments));})
    Element.prototype.getClientRects         = Element.prototype.getClientRects         || v_saf(function getClientRects(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getClientRects[func]", [].slice.call(arguments));})
    Element.prototype.getElementsByClassName = Element.prototype.getElementsByClassName || v_saf(function getElementsByClassName(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getElementsByClassName[func]", [].slice.call(arguments));})
    Element.prototype.getElementsByTagName   = Element.prototype.getElementsByTagName   || v_saf(function getElementsByTagName(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getElementsByTagName[func]", [].slice.call(arguments));})
    Element.prototype.getElementsByTagNameNS = Element.prototype.getElementsByTagNameNS || v_saf(function getElementsByTagNameNS(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getElementsByTagNameNS[func]", [].slice.call(arguments));})
    Element.prototype.getInnerHTML           = Element.prototype.getInnerHTML           || v_saf(function getInnerHTML(){v_console_log(" [/b][/b][/b][b][b][b] Element -> getInnerHTML[func]", [].slice.call(arguments));})
    Element.prototype.hasAttribute           = Element.prototype.hasAttribute           || v_saf(function hasAttribute(){v_console_log(" [/b][/b][/b][b][b][b] Element -> hasAttribute[func]", [].slice.call(arguments));})
    Element.prototype.hasAttributeNS         = Element.prototype.hasAttributeNS         || v_saf(function hasAttributeNS(){v_console_log(" [/b][/b][/b][b][b][b] Element -> hasAttributeNS[func]", [].slice.call(arguments));})
    Element.prototype.hasAttributes          = Element.prototype.hasAttributes          || v_saf(function hasAttributes(){v_console_log(" [/b][/b][/b][b][b][b] Element -> hasAttributes[func]", [].slice.call(arguments));})
    Element.prototype.hasPointerCapture      = Element.prototype.hasPointerCapture      || v_saf(function hasPointerCapture(){v_console_log(" [/b][/b][/b][b][b][b] Element -> hasPointerCapture[func]", [].slice.call(arguments));})
    Element.prototype.webkitMatchesSelector  = Element.prototype.webkitMatchesSelector  || v_saf(function webkitMatchesSelector(){v_console_log(" [/b][/b][/b][b][b][b] Element -> webkitMatchesSelector[func]", [].slice.call(arguments));})
  }
  function v_init_HTMLElement(){
    try{
      Object.defineProperties(HTMLElement.prototype, {
        style: {set: undefined, enumerable: true, configurable: true, get: v_saf(function style(){v_console_log(" [/b][/b][/b][b][b][b] HTMLElement -> style[get]", [].slice.call(arguments)); return this.v_style })},
      })
    }catch(e){
      v_console_log(e)
    }
  ...
  function v_init_DOMTokenList_prototype(){
    DOMTokenList.prototype.add = DOMTokenList.prototype.add || v_saf(function add(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> add[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.contains = DOMTokenList.prototype.contains || v_saf(function contains(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> contains[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.entries = DOMTokenList.prototype.entries || v_saf(function entries(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> entries[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.forEach = DOMTokenList.prototype.forEach || v_saf(function forEach(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> forEach[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.item = DOMTokenList.prototype.item || v_saf(function item(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> item[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.keys = DOMTokenList.prototype.keys || v_saf(function keys(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> keys[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.length = DOMTokenList.prototype.length || v_saf(function length(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> length[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.remove = DOMTokenList.prototype.remove || v_saf(function remove(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> remove[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.replace = DOMTokenList.prototype.replace || v_saf(function replace(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> replace[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.supports = DOMTokenList.prototype.supports || v_saf(function supports(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> supports[func]", [].slice.call(arguments));})
    DOMTokenList.prototype.toggle = DOMTokenList.prototype.toggle || v_saf(function toggle(){v_console_log(" [/b][/b][/b][b][b][b] DOMTokenList -> toggle[func]", [].slice.call(arguments));})
  }
  ...
  function v_init_PointerEvent_prototype(){
    PointerEvent.prototype.getCoalescedEvents = v_saf(function getCoalescedEvents(){v_console_log(" [/b][/b][/b][b][b][b] PointerEvent -> getCoalescedEvents[func]", [].slice.call(arguments));})
    PointerEvent.prototype.getPredictedEvents = v_saf(function getPredictedEvents(){v_console_log(" [/b][/b][/b][b][b][b] PointerEvent -> getPredictedEvents[func]", [].slice.call(arguments));})
  }
  function v_init_PerformanceTiming_prototype(){
    try{
      Object.defineProperties(PerformanceTiming.prototype, {
        connectEnd: {set: undefined, enumerable: true, configurable: true, get: v_saf(function connectEnd(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> connectEnd[get]", [].slice.call(arguments));})},
        connectStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function connectStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> connectStart[get]", [].slice.call(arguments));})},
        domComplete: {set: undefined, enumerable: true, configurable: true, get: v_saf(function domComplete(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> domComplete[get]", [].slice.call(arguments));})},
        domContentLoadedEventEnd: {set: undefined, enumerable: true, configurable: true, get: v_saf(function domContentLoadedEventEnd(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> domContentLoadedEventEnd[get]", [].slice.call(arguments));})},
        domContentLoadedEventStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function domContentLoadedEventStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> domContentLoadedEventStart[get]", [].slice.call(arguments));})},
        domInteractive: {set: undefined, enumerable: true, configurable: true, get: v_saf(function domInteractive(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> domInteractive[get]", [].slice.call(arguments));})},
        domLoading: {set: undefined, enumerable: true, configurable: true, get: v_saf(function domLoading(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> domLoading[get]", [].slice.call(arguments));})},
        domainLookupEnd: {set: undefined, enumerable: true, configurable: true, get: v_saf(function domainLookupEnd(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> domainLookupEnd[get]", [].slice.call(arguments));})},
        domainLookupStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function domainLookupStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> domainLookupStart[get]", [].slice.call(arguments));})},
        fetchStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function fetchStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> fetchStart[get]", [].slice.call(arguments));})},
        loadEventEnd: {set: undefined, enumerable: true, configurable: true, get: v_saf(function loadEventEnd(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> loadEventEnd[get]", [].slice.call(arguments));})},
        loadEventStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function loadEventStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> loadEventStart[get]", [].slice.call(arguments));})},
        navigationStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function navigationStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> navigationStart[get]", [].slice.call(arguments));})},
        redirectEnd: {set: undefined, enumerable: true, configurable: true, get: v_saf(function redirectEnd(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> redirectEnd[get]", [].slice.call(arguments));})},
        redirectStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function redirectStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> redirectStart[get]", [].slice.call(arguments));})},
        requestStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function requestStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> requestStart[get]", [].slice.call(arguments));})},
        responseEnd: {set: undefined, enumerable: true, configurable: true, get: v_saf(function responseEnd(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> responseEnd[get]", [].slice.call(arguments));})},
        responseStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function responseStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> responseStart[get]", [].slice.call(arguments));})},
        secureConnectionStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function secureConnectionStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> secureConnectionStart[get]", [].slice.call(arguments));})},
        unloadEventEnd: {set: undefined, enumerable: true, configurable: true, get: v_saf(function unloadEventEnd(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> unloadEventEnd[get]", [].slice.call(arguments));})},
        unloadEventStart: {set: undefined, enumerable: true, configurable: true, get: v_saf(function unloadEventStart(){v_console_log(" [/b][/b][/b][b][b][b] PerformanceTiming -> unloadEventStart[get]", [].slice.call(arguments));})},
      })
    }catch(e){}
  }
  ...
  window.postMessage = v_saf(function(){v_console_log(' [/b][/b][/b][b][b][b] [postMessage]', arguments)}, 'postMessage')
  window.matchMedia = v_saf(function(){v_console_log(' [/b][/b][/b][b][b][b] [matchMedia]', arguments); return v_proxy({}, 'matchMedia{}')}, 'matchMedia')

  init_cookie("SEARCH_RESULT_LIST_TYPE=%22single%22; hevc_supported=true; enter_pc_once=1; UIFID=; SelfTabRedDotControl=%5B%5D; __ac_signature=_02B4Z6wo00f01qVHpAQAAIDBKO8djeOdqkqlZ6CAAMHS90; xgplayer_device_id=69858618082; xgplayer_user_id=224209139808; live_use_vvc=%22false%22; fpk1=U2FsdGVkX18vc3IAj9CGeN43BiNYmiO2pF/RJIUOYmvU0Pe9BczRO4Re504bLB+labI/Lkqla5CNMEb/6UJA0Q==; fpk2=b345ebad146dda9f9a4e55672d01662e; WebUgChannelId=%2230001%22; passport_csrf_token=82722848089e484579fe70c640636cfb; passport_csrf_token_default=82722848089e484579fe70c640636cfb; bd_ticket_guard_client_web_domain=2; passport_assist_user=CkG8SQARKBf_MXiyBuzBmdwqt7F69EOZSiiZlBAj5XZcaNnOvzsYgYSelEBg16KEkHL563YYU6zWDJOSgGI22suLoxpKCjwAAAAAAAAAAAAAUDtzAUsKEAnEOAwI3WTFO3BNVo0wxZeAG4wNNv67IG3IqAW1uvW-55T5J2trl5yffbIQqp6NDhiJr9ZUIAEiAQMburPm; _bd_ticket_crypt_cookie=7e30da1bfcefd38a92a515f38fcf5d4e; __security_mc_1_s_sdk_sign_data_key_web_protect=0acaa851-494c-a3c9; __security_mc_1_s_sdk_cert_key=6aa49e11-4e3a-a427; __security_mc_1_s_sdk_crypt_sdk=589b20d5-42fa-8b38; __security_server_data_status=1; login_time=1774620989395; my_rd=2; is_support_rtm_web_ts=1; publish_badge_show_info=%220%2C0%2C0%2C1776843590435%22; is_dash_user=1; __live_version__=%221.1.5.1418%22; xg_device_score=7.829016546692666; has_avx2=null; device_web_cpu_core=32; device_web_memory_size=16; csrf_session_id=4fd3f74330c30baaf9b9336b8b9b7a09; live_private_user=0; download_guide=%223%2F20260422%2F0%22; FOLLOW_LIVE_POINT_INFO=%22MS4wLjABAAAAj-z-uDIosRzLMenRqyy0vkiV_M1e3PRRvqdhzfe5z9khc254IMOFjLhlRyKQy40F%2F1776960000000%2F0%2F1776876488815%2F0%22; s_v_web_id=verify_mobib4a9_bMtXPx0W_gaJu_4Nxs_86D1_sdkUmoU3KDPy; __druidClientInfo=; stream_player_status_params=%22%7B%5C%22is_auto_play%5C%22%3A0%2C%5C%22is_full_screen%5C%22%3A0%2C%5C%22is_full_webscreen%5C%22%3A0%2C%5C%22is_mute%5C%22%3A0%2C%5C%22is_speed%5C%22%3A1%2C%5C%22is_visible%5C%22%3A0%7D%22; stream_recommend_feed_params=%22%7B%5C%22cookie_enabled%5C%22%3Atrue%2C%5C%22screen_width%5C%22%3A1707%2C%5C%22screen_height%5C%22%3A960%2C%5C%22browser_online%5C%22%3Atrue%2C%5C%22cpu_core_num%5C%22%3A32%2C%5C%22device_memory%5C%22%3A16%2C%5C%22downlink%5C%22%3A10%2C%5C%22effective_type%5C%22%3A%5C%224g%5C%22%2C%5C%22round_trip_time%5C%22%3A100%7D%22; strategyABtestKey=%221776962262.624%22; volume_info=%7B%22isUserMute%22%3Atrue%2C%22isMute%22%3Atrue%2C%22volume%22%3A0.201%7D; home_can_add_dy_2_desktop=%221%22; FOLLOW_NUMBER_YELLOW_POINT_INFO=%22MS4wLjABAAAAj-z-uDIosRzLMenRqyy0vkiV_M1e3PRRvqdhzfe5z9khc254IMOFjLhlRyKQy40F%2F1777046400000%2F0%2F1777006360808%2F0%22; LivePausePop=%22%257B%2522todayCount%2522%253A4%252C%2522closeNum%2522%253A0%252C%2522todayShowRoom%2522%253A%25227631893755536083739%252C7632159621574069055%2522%252C%2522lastTimer%2522%253A1777007120080%257D%22; biz_trace_id=45fea40e; bd_ticket_guard_client_data_v2=%3D%3D; live_can_add_dy_2_desktop=%221%22; live_debug_info=%7B%22roomId%22%3A%227632087951572208418%22%2C%22resolution%22%3A%7B%22width%22%3A0%2C%22height%22%3A0%7D%2C%22fps%22%3A9%2C%22audioDataRate%22%3A0%2C%22droppedFrames%22%3A0%2C%22totalFrames%22%3A0%2C%22videoBuffer%22%3A%5B%5D%2C%22src%22%3A%22https%3A%2F%2Fpull-flv-f11.douyinliving.com%2Ffantasy%2Fstream-695712127744213847.flv%3Farch_hrchy%3Ds2%26expire%3D1777612667%26major_anchor_level%3Dvip%26neq%3D1%26sign%3D0d44004c145060a9bdc68f12e328ee62%26t_id%3D037-20260424131746CAFE63B4CFB96CB41907-dOt6rE%26unique_id%3Dstream-695712127744213847_855_flv%26_session_id%3D037-20260424131746CAFE63B4CFB96CB41907-dOt6rE.1777008875693.16199%26rsi%3D1%26abr_pts%3D-800%22%2C%22linkmicInfo%22%3A%7B%22uiLayout%22%3A0%2C%22playModes%22%3A%5B%5D%2C%22allDevices%22%3A%22%E8%BF%9E%E7%BA%BF%E8%AE%BE%E5%A4%87%EF%BC%9A%E7%94%B3%E8%AF%B7%E8%BF%9E%E7%BA%BF%E5%90%8E%E6%89%8D%E8%8E%B7%E5%8F%96%22%2C%22audioInputs%22%3A%5B%5D%2C%22videoInputs%22%3A%5B%5D%7D%2C%22href%22%3A%22https%3A%2F%2Flive.douyin.com%2F80017709309%3Ffrom_search%3Dtrue%22%7D; IsDouyinActive=true; bd_ticket_guard_client_data=%3D%3D")
  v_hook_href(window.location, 'location', "https://live.douyin.com/80017709309?from_search=true")
  Location.prototype.toString = v_saf(function toString(){ return "https://live.douyin.com/80017709309?from_search=true" })
