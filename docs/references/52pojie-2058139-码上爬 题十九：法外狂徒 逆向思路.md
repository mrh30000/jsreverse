...
> **作者**: ImpJ | **发布时间**: 2025-09-05 11:26:00 | **版块**: 『脱壳破解区』 | **查看/回复**: 2747 / 19
> **原文**: [https://www.52pojie.cn/thread-2058139-1-1.html](https://www.52pojie.cn/thread-2058139-1-1.html)

---

*本帖最后由 ImpJ 于 2025-9-5 11:31 编辑*

最近在自己研究爬虫和逆向，做了几个网站的逆向练习，分享一下自己的经验，顺便希望大佬们指导一下思路是否正确。

分析网址：aHR0cHM6Ly93d3cubWFzaGFuZ3BhLmNvbS9wcm9ibGVtLWRldGFpbC8xOS8=

进入页面直接按照惯例F12 观察一下header和response看看有没有什么特别的地方

![](https://attach.52pojie.cn/forum/202509/05/111711evikkyc40ihrvz41.png)

从网络请求中看到请求头中没有特殊的参数，看到response返回了一个r和一个k，他俩都是乱码，我们先不着急去看他俩的解析逻辑，在回到页面上点击翻页看看网络请求中的请求头是否有变化。
（这道题比较简单，翻页后发现cookie中的数据都一样没有变化。）
综上，我们开始直接开始分析r和k的解析逻辑，打开源代码页面观察一下都有哪些js文件。

![](https://attach.52pojie.cn/forum/202509/05/111719tfzqenefffnhwcwp.png)

**注**：这里我分享一下我的经验，先看下源代码里的js文件，看到pagination19.js和19pro.js这样比较明确的有指向性的名字的js的时候直接先看这样的js，如果这样的js文件分析后没有明确的参数生成或解析的过程再去断点分析生成或解析位置和逻辑，这样的话能节省一点时间。
我们按照正常流程来分析r和k的解析逻辑，我们来到网络请求tab点击网络请求，找到启动器看下堆栈调用，找到是哪个js的方法，这里我们看到是pagination19.js这个文件，我们直接点击进去看看这个方法

![](https://attach.52pojie.cn/forum/202509/05/111721wuxffst141uwnz5n.png)

点进去之后我们来到了pagination19.js，看到js代码如下

![](https://attach.52pojie.cn/forum/202509/05/111723eq2eqtl8xjjeeete.png)

看到这段代码是一个使用 Promise 的 JavaScript 方法

* fetch(...) 发起网络请求并返回一个 Promise
* .then(response => response.json()) 处理第一个 Promise 的结果，返回新的 Promise（解析 JSON 后的结果）
* 第二个 .then(...) 处理解析后的 JSON 数据

从分析中我们可以知道这个js方法的逻辑是通过url获取数据，然后json解析url得到的结果，再去DES3去decrypt，所以我们直接来到第14行打上断点，来看看decrypt方法在哪里

![](https://attach.52pojie.cn/forum/202509/05/111648pgzlawo69a6w1r29.png)

通过断点看到decrypt方法在19pro.js里我们直接点进去看下逻辑。

![](https://attach.52pojie.cn/forum/202509/05/112038l9yopk9poib0g8oo.png)

这里我们有两个方法去解析r和k，一种是扣代码然后暴露出去用python调用，一种是直接把js复制出来让ai还原解密逻辑生成python代码，这里就不再啰嗦了直接上代码
js代码：
...
function main(str,key) {
    return DES3.decrypt(str,key);
}
module.exports = { main };
if (require.main === module) {
    const str = process.argv[2];
    const key = process.argv[3];
    console.log(main(str,key));
}
```
...
cookies = {
    'sessionid': 'xxxxxxxx', #这里是你自己的sessionId
    'v': 'QTBzQUM5NXZxSkd1WlB0eVdTZGRYclZnMnVRdzRGOWkyZlFqRnIxSUp3cmgzR1dhUmJEdnN1bkVzMlRPMTc1NjY4OTUwMzAzMA==',
    'HMACCOUNT': '453CE42CD03164BA',
    '_nano_fp': 'Xpmyn5CJX5EJn5XbnC_CMlPMU1kFffKffta8GRIB'
}
...
def _iv() -> bytes:
    return datetime.now().strftime("%Y%m%d").encode("utf-8")

def decrypt(enc_b64: str, key: str, iv: bytes | None = None) -> str:
    if iv is None:
        iv = _iv()
    enc_bytes = base64.b64decode(enc_b64)
    cipher = DES3.new(key.encode("utf-8"), DES3.MODE_CBC, iv)
    plain = unpad(cipher.decrypt(enc_bytes), DES3.block_size)
    return plain.decode("utf-8")

async def async_h2_request(url):
    global total_sum
    async with httpx.AsyncClient(http2=True, cookies=cookies, headers=header) as client:
        try:
            response = await client.get(url)
            res['r'] = response.json().get('r', [])
            res['k'] = response.json().get('k', [])
            # 使用js方式获取数据
            # output_text = subprocess.run(['node', './Q19.js', res['r'], res['k']], capture_output=True, text=True,
            #                              encoding='utf-8').stdout

            number_arr = json.loads(decrypt(res['r'], res['k']))
            total_sum += sum(number_arr['current_array'])
        except Exception as e:
            print(f"异步请求出错: {e}")

for i in range(1, 21):
    test_url = f"https://www.mashangpa.com/api/problem-detail/19/data/?page={i}"
    asyncio.run(async_h2_request(test_url))

print(f"最终和为：{total_sum}")
```

至此，是我对这道题的分析过程，如果有不足和错误的地方希望大佬指正，新人第一次发帖还希望大家多多包涵，谢谢。
