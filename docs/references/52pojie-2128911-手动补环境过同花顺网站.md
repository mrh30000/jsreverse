...
> **作者**: 13955925361 | **发布时间**: 2026-09-19 18:02:00 | **版块**: 『脱壳破解区』 | **查看/回复**: 157 / 3
> **原文**: [https://www.52pojie.cn/thread-2128911-1-1.html](https://www.52pojie.cn/thread-2128911-1-1.html)

---

*本帖最后由 13955925361 于 2026-9-19 18:04 编辑*

ai那一版过不了的看这个了，代码更少更精简

![](https://attach.52pojie.cn//forum/202609/19/175612fuujugl108p8861u.png?l)![](https://attach.52pojie.cn//forum/202609/19/175615b3d3cs2cxbhcze8e.png?l)
运行调用代码：
...
# -*- coding: utf-8 -*-
"""
同花顺 hexin-v 手动补环境 —— 行情数据抓取
------------------------------------------------------------
流程：
  1) 调 node env.js 现算一个新鲜的 hexin-v(cookie 里的 v 值) + 对应 UA；
  2) 带 token 请求行情页，用 BeautifulSoup 解析 <table class="m-table"> 表格；
  3) 支持翻页，合并所有行，存 CSV 并打印预览。

关键坑：
  - hexin-v 内部编码了 strhash(navigator.userAgent)，请求头 UA 必须与 env.js 里的
    navigator.userAgent 完全一致；本脚本直接从 node 输出解析 UA，天然一致。
  - token 有时效性，现算现用；某页被判拦截时会自动重算 token 再重试一次。
"""
import csv
import os
import re
...
NODE_EXE = r"C:\Program Files\nodejs\node.exe"      # 若已加入 PATH 可改成 "node"
ENV_JS = os.path.join(BASE_DIR, "env.js")

# 行情页 URL 模板：{page} 为页码
URL_TMPL = "https://q.10jqka.com.cn/index/index/board/all/field/zdf/order/desc/page/{page}/ajax/1/"
START_PAGE = 1        # 起始页
TOTAL_PAGES = 3       # 抓取页数（从 START_PAGE 开始）
PAGE_DELAY = 1.0      # 每页间隔秒数，礼貌抓取
CSV_PATH = os.path.join(BASE_DIR, f"同花顺行情_{datetime.now():%Y%m%d_%H%M%S}.csv")

# ==================== 1. 调 env.js 拿 token ====================
def get_hexin_v():
    """运行 env.js，返回 (v值, userAgent)。"""
    node = NODE_EXE if os.path.exists(NODE_EXE) else "node"
    proc = subprocess.run(
        [node, ENV_JS], cwd=BASE_DIR,
        capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=90,
    )
    out = proc.stdout or ""
    mv = re.search(r"__HEXIN_V__=(\S+)", out)
    mu = re.search(r"__HEXIN_UA__=(.+)", out)
    v = mv.group(1).strip() if mv else ""
    ua = mu.group(1).strip() if mu else ""
    if not v:
        print("[!] 未能解析 hexin-v，node 输出末尾：\n", out[-1200:])
        print("[!] node stderr：\n", (proc.stderr or "")[-1200:])
    return v, ua

# ==================== 2. 请求 + 解析 ====================
def build_sec_ch_ua(ua: str) -> str:
    m = re.search(r"Edg/(\d+)", ua) or re.search(r"Chrome/(\d+)", ua)
    ver = m.group(1) if m else "153"
    return f'"Microsoft Edge";v="{ver}", "Not_A Brand";v="8", "Chromium";v="{ver}"'

def make_headers(ua: str, v: str, referer: str) -> dict:
    return {
...
def parse_table(html: str):
    """解析行情表格，返回 (headers, rows)。rows 为 list[dict]。"""
    ...
    for tr in table.select("tbody tr"):
        ...
        if headers and len(tds) == len(headers):
            ...
            rows.append({f"col{i}": val for i, val in enumerate(tds)})
    return headers, rows

def fetch_page(session, page, ua, v):
    url = URL_TMPL.format(page=page)
    resp = session.get(url, cookies={"v": v}, headers=make_headers(ua, v, url), timeout=20)
    return resp, url

def is_real_data(resp) -> bool:
    t = resp.text or ""
    return resp.status_code == 200 and len(t) > 2000 and "m-table" in t

# ==================== 3. 主流程 ====================
def main():
    print("=" * 64)
    print("步骤 1：调 node env.js 现算 hexin-v ...")
    v, ua = get_hexin_v()
    if not v:
        sys.exit("补环境未产出 token，请先排查 env.js / ma.js。")
    if not ua:
    ...
    print("=" * 64)
    print(f"步骤 2：抓取第 {START_PAGE} ~ {START_PAGE + TOTAL_PAGES - 1} 页 ...")
    for idx, page in enumerate(range(START_PAGE, START_PAGE + TOTAL_PAGES)):
        resp, url = fetch_page(session, page, ua, v)
        if not is_real_data(resp):
            # token 可能失效，重算一次再试
            print(f"[!] 第 {page} 页疑似被拦截(status={resp.status_code}, len={len(resp.text or '')})，重算 token 重试 ...")
            v, ua2 = get_hexin_v()
            if ua2:
                ua = ua2
            if not v:
                print(f"[!] 重算 token 失败，跳过第 {page} 页。")
                continue
            resp, url = fetch_page(session, page, ua, v)
            if not is_real_data(resp):
                print(f"[!] 第 {page} 页仍失败，跳过。")
                continue

        page_headers, rows = parse_table(resp.text)
        if page_headers and not headers:
            headers = page_headers
        all_rows.extend(rows)
        print(f"[+] 第 {page} 页：解析到 {len(rows)} 条 (累计 {len(all_rows)})")
    ...
    # 存 CSV（utf-8-sig 方便 Excel 打开不乱码）
    fieldnames = headers or list(all_rows[0].keys())
    ...
    print("=" * 64)
    print(f"[+] 共 {len(all_rows)} 条，已保存到：{CSV_PATH}")
    print("-" * 64)
    print("数据预览（前 8 条）：")
    for r in all_rows[:8]:
        print("  " + " | ".join(f"{k}:{r.get(k, '')}" for k in fieldnames[:6]))
    print("=" * 64)
...
function watch(Object,WatchName){
    const handler = {
        ...
        get(target, property, receiver){
            if (property == 'process'){return undefined}
            if (property == 'globalThis'){return undefined}
...
window.addEventListener=function addEventListener(){
    console.log("方法","window.addEventListener","参数:",arguments);
...
document={
    head:watch({tagname:"HEAD"},"dom.head"),
    createElement:function createElement(){
    console.log("方法","document.createElement","参数:",arguments)
    if (arguments[0] == "div" ){
        return watch({
            tagName:"DIV",
            onwheel:null
    ...
    if (arguments[0] == "canvas") {
        return watch({
            ...
            getContext :function getContext(){
                console.log("方法","document.createElement.getContext","参数:",arguments)
                if (arguments[0] == 'webgl2') {
                    return watch({},"dom.canvas.getContext")
                }
            }
        },"dom.canvas")
    }
    ...
    getElementsByTagName : function getElementsByTagName() {
        console.log("方法", "document.getElementsByTagName", "参数:", arguments)
        if (arguments[0] == "base'") {
            return []
        }
    }
...
Navigator.prototype = {
    ...
    plugins:watch({length:0},"navigator.plugins"),
...
location={
    "ancestorOrigins": {},
    "href": "https://q.10jqka.com.cn/index/index/board/all/field/zdf/order/desc/page/5/ajax/2",
    "origin": "https://q.10jqka.com.cn",
    "protocol": "https:",
    "host": "q.10jqka.com.cn",
    "hostname": "q.10jqka.com.cn",
    "port": "",
    "pathname": "/index/index/board/all/field/zdf/order/desc/page/5/ajax/2",
    "search": "",
    "hash": ""
}
location.toString= function toString(){this.href}
location.__proto__ = Location.prototype;
Storage_sql={}
Storage = function Storage(){throw new TypeError('Illegal constructor');}
Storage.toString = function toString(){return 'function Storage() { [native code] }';}
Storage.prototype = {
    toString: function toString() {
        return '[object Storage]';
    },
    getItem: function getItem() {
        console.log('方法', 'Storage.getItem', '参数:', arguments)
        if (Storage_sql[arguments[0]]) {
            return Storage_sql[arguments[0]]
        } else {
            return null
        }
    },
    setItem: function setItem() {
        console.log('方法', 'Storage.setItem', '参数:', arguments)
        Storage_sql[arguments[0]] = arguments[1]
    },
    removeItem: function removeItem() {
        console.log('方法', 'Storage.removeItem', '参数:', arguments)
        delete Storage_sql[arguments[0]]
    },
    clear: function clear() {
        console.log('方法', 'Storage.clear', '参数:', arguments)
        Storage_sql = {}
    }
}
...
XMLHttpRequest.prototype = {
    open: function open() {
        console.log('方法', 'XMLHttpRequest.open', '参数:', arguments)
    },
    send: function send() {
        console.log('方法', 'XMLHttpRequest.send', '参数:', arguments)
    },
    onload: function onload() {
        console.log('方法', 'XMLHttpRequest.onload', '参数:', arguments)
    },
    onerror: function onerror() {
        console.log('方法', 'XMLHttpRequest.onerror', '参数:', arguments)
    },
    getAllResponseHeaders : function getAllResponseHeaders() {
        console.log('方法', 'XMLHttpRequest.getAllResponseHeaders', '参数:', arguments)
    },
    getResponseHeader : function getResponseHeader() {
        console.log('方法', 'XMLHttpRequest.getResponseHeader', '参数:', arguments)
    },
}

// Element：chameleon 的 D() 会 hook Element.prototype.appendChild / insertBefore
...
Element.prototype = {
    ...
    appendChild:function appendChild(c){ return c; },
    insertBefore:function insertBefore(c){ return c; },
    removeChild:function removeChild(c){ return c; },
...
XMLHttpRequest.prototype = watch(XMLHttpRequest.prototype, "XMLHttpRequest.prototype");

// chameleon 初始化末尾会 setInterval(getServerTime, 1200000)，
// 这个 20 分钟的定时器会让 Node 进程常驻不退出；unref 后就不阻塞退出。
const _setInterval = setInterval;
setInterval = function setInterval(fn, ms){
    const t = _setInterval(fn, ms);
    if (t && t.unref) t.unref();
    return t;
};

const hexin_v = require_('./ma.js');

console.log("\n================= 结果 =================");
console.log("hexin-v (v值) :", hexin_v || globalThis.__hexin_v || "(未生成，检查初始化是否报错)");
console.log("=======================================\n");
// 机器可解析标记（供 Python 提取，避开中文与 watch 日志干扰）
console.log("__HEXIN_V__=" + (hexin_v || globalThis.__hexin_v || ""));
console.log("__HEXIN_UA__=" + Navigator.prototype.userAgent);
...
var TOKEN_SERVER_TIME = 1789789000.668;
!function(n, t, r, e) {
    !function() {
        var a = r[0]
          , u = n[0]
...
// ===== 导出 chameleon 生成的 v 值(hexin-v) =====
// 此时上面的 IIFE 已同步执行完 Init→W()，token 已写入 document.cookie(name=v) 与 localStorage(hexin-v)。
;(function exportHexinV() {
    var v = null;
    try {
        // 1) 优先从 cookie 解析：chameleon 写入形如 "v=<token>; domain=...; path=/; expires=..."
        if (typeof document !== 'undefined' && document.cookie) {
            var m = String(document.cookie).match(/(?:^|;\s*)v=([^;]+)/);
            if (m) v = m[1];
        }
        // 2) 兜底从 localStorage 读
        if (!v && typeof localStorage !== 'undefined' && localStorage.getItem) {
            v = localStorage.getItem('hexin-v') || localStorage.getItem('v');
        }
    } catch (e) {}
    try { globalThis.__hexin_v = v; } catch (e) {}
    // 通过 CommonJS 导出，供 env.js 的 require 返回值直接拿到
