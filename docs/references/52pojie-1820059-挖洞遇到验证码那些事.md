...
> **作者**: 中二 | **发布时间**: 2023-08-11 16:40:00 | **版块**: 『脱壳破解区』 | **查看/回复**: 16666 / 158
> **原文**: [https://www.52pojie.cn/thread-1820059-1-1.html](https://www.52pojie.cn/thread-1820059-1-1.html)

---

*本帖最后由 wangguang 于 2023-8-11 17:12 编辑*
...
captcha\_signature在刷新验证码发下来的数据包里面的sign参数。

captcha\_phrase和captcha\_ticket在getTicket接口从服务器返回回来的，captcha\_phrase是缺口，如果缺口对齐失败的话captcha\_ticket是获取不到的。所以得先把getTicket接口的参数补齐拿到ticket。
...
cn函数有一个三元表达式，TOKEN\_SERVER\_TIME是数值，x + T + C是"undefined"，TOKEN\_SERVER\_TIME == x + T + C 是false

所以执行(time = parseInt(TOKEN\_SERVER\_TIME),time)

这段函数的整体意思是将TOKEN\_SERVER\_TIME取整赋值给time，再把time值返回。

![](https://attach.52pojie.cn/forum/202308/11/162705qwbp3pp6lnglg3rn.png)

E[i] = Gn.serverTimeNow()直接改写成E[i] = parseInt(TOKEN\_SERVER\_TIME)

运行代码报错TOKEN\_SERVER\_TIME未定义，在浏览器搜索TOKEN\_SERVER\_TIME。

![](https://attach.52pojie.cn/forum/202308/11/162730d49psps04kugp737.png)

在第一行就可以看到TOKEN\_SERVER\_TIME。直接扣下来
...
Gn.serverTimeNow()—>parseInt(TOKEN\_SERVER\_TIME)
