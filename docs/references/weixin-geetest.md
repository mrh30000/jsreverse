---
title: "某验4五子棋逆向py纯算（AST解混淆/自动化提取）"
source: "https://mp.weixin.qq.com/s/P9bWMSC3Yvf19Coe_noTkg"
author:
  - "[[王平]]"
published:
created: 2026-09-07
...
| 接口 | 路径 | 作用 |
| --- | --- | --- |
| Load接口 | `/load` | 加载验证码资源，返回五子棋盘/滑块背景图及关键参数 |
| Verify接口 | `/verify` | 提交验证，核心参数为 w |

### 1.2 Load接口分析

**初次请求参数：**

- • `callback` ：geetest\_ + 13位时间戳拼接
- • `captcha_id` ：每个网站固定不变
- • `challenge` ：UUID动态生成
...
| 特征 | 说明 |
| --- | --- |
| 变量名混淆 | 使用 Unicode 稀有字符（CJK 兼容区）作为变量名，如 `_ᕺᖘᖄᖘ` |
| 字符串加密（核心） | 所有字符串常量被加密为整数参数，运行时通过解密函数还原 |
| 解密函数句柄传递 | 解密函数从全局对象属性取出，经数组包装后赋给混淆别名 |
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MTq2yvJe9cvqNnOib7iaibhDqGiaoMq8f66Uy0t6ePIzJqibEmRXUqiaaDUjlX4ahibJOl86x9aT1bxWCcFia6GSGgorOMYbZRO3YRpWEg/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=4)

### 2.7 解密校验

将 `output.js` 替换网页中的 `gcaptcha4.js` ，刷新验证码，确认能正常加载与校验通过。

![图片](https://mmbiz.qpic.cn/mmbiz_gif/yGOpnM3p9MRna2m0WX124utibVvT19iaL7pK2Z3PI1NZ4I3yGg2lYsPsE1UeZ8bTbvWMVcxDVdwzlqADYtibcoaOic4loM3YIaasIlcS2CHkCZA/640?wx_fmt=gif&from=appmsg#imgIndex=5)

---

## 三、w值逆向

![图片](https://mmbiz.qpic.cn/mmbiz_png/yGOpnM3p9MQ5tXEj1PAwysO3ib64sTbtWb1sDNjicjSdtenQ10R1A9Dq1IOfpWib2fic5hqt97X1I90WxEUbeNOL4kDLNuiak1T7KWBXo7XhOHr8/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=6)
...
| 参数名 | 说明 | 来源 |
| --- | --- | --- |
| `7822f0` | `{"a4b2": "2f0ab495"}`  变化值，键值均取自 lot\_number，切片位置在 `gcaptcha4.js` 固定位置提取 | 动态 提取（见 3.4.2） |
| `biht` | 固定值"1426265548"\` |  |
| `dQFB` | `BoHp`  变化值，在 `gcaptcha4.js` 中固定位置提取 | 见 3.4.2 |
| `device_id` | 固定空串 | — |
| `em` | 固定对象 | 见 3.4.1 |
| `ep` | 固定值 `"123"` | — |
| `gee_guard` | 固定对象 | 见 3.4.1 |
| `geetest` | 固定 `"captcha"` | — |
| `lang` | 固定 `"zh"` | — |
| `lot_number` | load 接口返回 | — |
| `passtime` | 验证耗时，随机 500~3000即可，没有硬性要求 | 随机生成 |
| `pow_msg` | 需逆向 | 见 3.3.2 |
| `pow_sign` | 需逆向 | 见 3.3.2 |
| `userresponse` | 五子棋棋子移动坐标 | 见 3.3.3 |

#### 3.3.2 pow\_msg 与 pow\_sign 逆向
...
1. 1\. `pow_msg = _powFunc(options, captchaId, hashfunc, version, bits, datetime, '')`
	...
	![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MQT80NcicgO3HrYmGgEmtdajRtdhrk0vV78iawyaK7Kcibia0pIhNotZ3ib6R9FiblNcaLNVwRn99yAQNau8Cia1UiaCibFNKMtPicvWicu7c/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=7)
	```
...
6. 4\. **校验：** `pow_sign` 必须以 `00` 开头才有效，否则换随机串重试。![图片](https://mmbiz.qpic.cn/mmbiz_png/yGOpnM3p9MRwGVQa6rxuaYcJ6OY0YKIer1XdDN84rhyTzDQ1Sb00mIldPBFbxbbR4nLqC4lOyCD0CNAM7icnIlESYy9mLpSVbibkdicMDSQfTw/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=8)
...
def find_move(ques):
    ...
    return None
```
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MTwG5dhR98tQAxnmwvjtaUrMJpYVHTkVqk3lVDV4fGciaLz5LLibjaN3CtJ4hZocP5nlRIETB8X9op0phcW6pbdAUpsZ9kRVC9wo/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=9)
...
```
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MQr05MNUNRmyrBYCSc4UYNlxn02fcqdlvGw3OcZRqdJjrdxeicYpGRzibBjP0ATH8qMiaRHjWymVzNCaJAicKoJGmib2EdRljQWIZwQ/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=10)

其中![图片](https://mmbiz.qpic.cn/mmbiz_png/yGOpnM3p9MS2YOeomqQhiaP8Tcu1ibCtjibNlNYLVYWibCdgT1uU1kAtnodH8o9od9Z7sHN2r46a4oSIFmow22DiaMfZXcERhjZpQQkcku7aClAM/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=11)
...
var i = (0, _ᕴᖗᖗᖆ['getStringByIndexes'])(_ᖃᕹᖙᕶ['lot'], _ᖃᕹᖙᕶ['lotNumber']),
    ...
    o = i['split']('.'), a = {};
```
![图片](https://mmbiz.qpic.cn/mmbiz_png/yGOpnM3p9MRjvZH78N3jc32SwwE8SyqEnUtyVB0HNo0WuepIEBPe6W1aSwtTBfEgh4uZQHZuPiau8lfYbuLIWzUv19EmJvMDf6NY28eLvTyg/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=12)

1. 1\. `getStringByIndexes` 分析，先获取切片字符串，后面通过split和正则进行拆分匹配。![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MRgInZf7icPRR2td0TJnibjB8N5BXxahqx52z5JgpkFQic7UBbZy2yqXlBhwo9yZhVJVXpXohp3VCzR4ia7CHNHlKqL0gsJ8xibIPss/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=13)
2. 2\. 全局搜索切片值，能够直接定位
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MSGwwOcIoAM70ibBOVWjDnXUqIw9PTkRFQpQKqAS9WBNfpJ9ElUNV2JWJTtX2ofIhfPwxnb89H63UmmiauIibIpOt4rOUWsohnhyA/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=14)

3.打上日志断点，和实际值对比

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MQzzqm9hNb8NZtFsEgW3o7AyZMxChLXj9nk39YExRM1WPoeldugpicngavqI3JX1QNGMuUs0jRL9T7DpJpnjsPJ3Z9ibiap72l0ibk/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=15)

日志输出和后面a参数的切片值是一致的，验证通过

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MTCZ6khRSqZ94fX8icxW9mgLrWkmbop3NJn7icicFrCic341zyLDicEGQ6JEY7I1WeEmiaic5L8ho9hbYWjnCmaOnDcyGpQgfpf7NZSKs/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=16)
...
| 符号 | 名称 | 作用 |
| --- | --- | --- |
| `+` | 拼接符 | 将同一层级内的多个切片结果 **横向拼接** ，组成一个完整的字符串（键或值）。 |
| `.` | **层级分隔符（核心）** | 表示 JSON 结构的 **嵌套层级** 。每遇到一个点号，就“ **往内新开一层对象** ”，点号左侧的内容作为当前层的键，右侧内容进入下一层作为其值。 |
| `:` | 赋值符 | 冒号右侧的内容（切片结果）作为当前 **最内层键** 对应的 **最终值** 。 |
| `n[m:n]` | 切片指令 | 对目标字符串的截取用 `n[m:n]` 表示（含尾索引，即取第 m 到第 n 个字符），其底层实际执行等价于 str.slice(m, n+1)（左闭右开 `[m, n+1)）` 。 |

###### 3\. 所有切片操作的明细表（按执行顺序）

根据补充的 `slice` 调用逻辑，实际切片边界与结果如下（ **左闭右开** ）：

| 编号 | 表达式中的写法 | 实际切片操作 | 截取索引范围 | 提取结果 | 用途说明 |
| --- | --- | --- | --- | --- | --- |
| ① | `n[5:7]` | `slice(5, 8)` | 5, 6, 7 | `"b14"` | 拼接为 **最外层键** 的左半部分 |
| ② | `n[7:9]` | `slice(7, 10)` | 7, 8, 9 | `"4e5"` | 拼接为 **最外层键** 的右半部分 |
| ③ | `n[20:27]` | `slice(20, 28)` | 20 ~ 27 | `"e1ce149f"` | 直接作为 **第二层键** |
| ④ | `n[10:10]` | `slice(10, 11)` | 10 | `"c"` | 拼接为 **最内层键** 的第1个字符 |
| ⑤ | `n[12:12]` | `slice(12, 13)` | 12 | `"4"` | 拼接为 **最内层键** 的第2个字符 |
| ⑥ | `n[3:3]` | `slice(3, 4)` | 3 | `"9"` | 拼接为 **最内层键** 的第3个字符 |
| ⑦ | `n[7:7]` | `slice(7, 8)` | 7 | `"4"` | 拼接为 **最内层键** 的第4个字符 |
| ⑧ | `n[7:14]`  （冒号右侧） | `slice(7, 15)` | 7 ~ 14 | `"4e5cb46a"` | 直接作为 **最内层键对应的值** |
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/yGOpnM3p9MQScxFNUCiaKnE8KPUO4F9YyOlbIZ3zDAibcgGiag2ziaCoxwqQCibst0slld0QiaaIzwraxpW1XT7s9CsQ5vicVicpHJHzJmGasjJJW9k/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=17)
...
```
![图片](https://mmbiz.qpic.cn/mmbiz_png/yGOpnM3p9MTDALicdP2dtbjKVtvl00qq7WWWicVSYd2VBs8YHJhjo47sFwFU9nZlyNMJxkKGNEADlbQNYw6g10XypWRkyO54aoEfjIEdwRicrs/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=18)
- • RSA 加密，公钥与指数固定
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/yGOpnM3p9MRguVibe60gQcdlkunzEibKbicvu0MjVfVZm4wXBBvn02FojuiccZyfSlSL9dXibksbfyg7WEO1Q7HMKAaTnqN3aQSoELPSP9bqia924/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=19)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/yGOpnM3p9MSReOQ55XoLjYcw6C61E1bRDJFNIIwq0KmsPBpqeoibd22HjydE60Jfq7pcugQyxVUs0bOam0iaCv4vG0x8eaaSbZhVZ6wplECj0/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=20)

### 滑块：成功率偏低，只用了ddddocr识别模块，追求高成功率的话，需要对接自己训练模型或接平台

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MQs1Hl0dHvyY4X8Iokca7HzgG7IkFlspe10G52N4baaR6fAuBQfE7dtCCkOVezw8CWicW56bict8Kg7MDXibeNbEq2LWHia4hqS5s8/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=21)

### 消消乐：

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MTiaL6ibI19ddcT4VCJInGR9WS4GjSqicbt7cZGCpcjm5rxGXqeZicFyxHa6FzcpCr5lVbr9o6nsGIaWhHN0MAKZy2jJhZLboxTKPc/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=22)

### 文字点选：

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MQpibxKQZUVn2lib26rpzryNZNgptOCRRIsZkkHBnXkWicEWCnXdQkicm8ibicicLRxYuMMVPEMSBtD761HSoibvaKvV61R8JHwaZ9WhA0/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=23)

## 六、业务中实际应用

目标地址： `aHR0cHM6Ly9hY2NvdW50Lndwcy5jbi8j` ，选择手机登录

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MSGbAgeiabkJM3xqJCAO13IJSfib7TDHL4gSI2zqzlGsB022gjgxDWEw0tr5vGXhgDVNdBAmdrwSwJ6gbxicxx45mGIFGN87UQlBM/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=24)

抓包或者html提取，获取到该网站某验验证码的captcha\_id =703298677ada65c569195d173645cde7

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/yGOpnM3p9MS7OEIql26vfJ33g4o0kc6NicVvxZ9zhWthNibibBnKmyNgxM239ZFhufSYwU8R24PAiaZQ2tTYB3CEZSibHricYjJ4IErvL4c8Fl8UQ/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=25)

调用 `sms` 接口，发送短信，其中圈起来的部分，就是滑块验证码通过后返回的值，获取到之后直接发起 `sms` 请求

![图片](https://mmbiz.qpic.cn/mmbiz_png/yGOpnM3p9MRQIE7zUjFbl3icO0baFCgD68Q5ZHuicqEQlsQM7miaAsBJILUC9FJXqL4fYXYmzHnlY71dWjRnkx5ibOPm5zAVo3IRj95PTKUYOkA/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=26)
