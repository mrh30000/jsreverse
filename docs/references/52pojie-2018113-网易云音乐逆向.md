...
> **作者**: 13955925361 | **发布时间**: 2025-03-24 14:44:00 | **版块**: 『编程语言区』 | **查看/回复**: 4736 / 35
> **原文**: [https://www.52pojie.cn/thread-2018113-1-1.html](https://www.52pojie.cn/thread-2018113-1-1.html)

---

前面是python代码，后面有Js代码，将Js代码保存为网易云.js文件，安装PyExecJS库，在控制台输入网易云歌单ID后面的数字就可以做到快捷下载歌单里歌曲的功能了。
...
class Spider(object):
    ...
    def __init__(self):
        user_put=input("请输入内容：")
        self.start_url='https://music.163.com/playlist?id={}'.format(user_put)
    ...
    def parse_url(self):
        ...
        song_idlist=html.xpath('//ul[@class="f-hide"]/li/a/@href')
        song_namelist=html.xpath('//ul[@class="f-hide"]/li/a/text()')
        for song_id,song_name in zip(song_idlist, song_namelist):
            mp3_id=song_id.split('=')[-1]

            i1x = {
                "ids": f"[{mp3_id}]",
                "level": "standard",
                "encodeType": "aac",
                "csrf_token": "cf6f1e0f21a7257b5d795c6fd71664c4"
            };
            with open('网易云.js', 'r', encoding='utf-8') as f:
                ctx = execjs.compile(f.read())
            json_data = ctx.call('get_data', i1x)
            self.parse_data(json_data,song_name)

    def parse_data(self, json_data, song_name):
        headers = {
            'accept': '*/*',
...
function  get_data (i1x){
    var result = d(JSON.stringify(i1x), "010001", n, "0CoJUm6Qyw8W8jud")
    json_data={'params':result.encText,
