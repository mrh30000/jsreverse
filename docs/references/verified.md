...
## 批次 B40 · 2026-09-25（第四十一次执行）

**取材口径**：**归档线第 40 轮新收 8 篇全取（8/8 命中）** —— 承接上一批「归档线第 40 轮」的优先级 ①。
该轮归档自述「**作者维度单轮新高**；其余五条通道继续为 0」，供给 8 篇，**如实按 8 篇蒸馏**。
簇结构：**媒体链路 2 篇**（HLS AES-128 手工链路 / B 站 playurl DASH 分流）、
**接口解析链路 2 篇**（抖音去水印 PHP + Java，同作者相隔 3.5 个月互为交叉验证）、
**接口签名 1 篇**（两段链式 MD5）、**服务端授权逆向 1 篇**（WP 主题 Z5Encrypt 混淆）、
**前端反检测 2 篇**（selenium webdriver 同系列）。

**产出**：**0 新建技能**（能力缺口评估 = 0，逐簇给了「为什么不是新建」的判据），
**演化 3 个技能、新建 1 个参考文件 + 1 个脚本**：
`stream-drm-reverse`（`hls-and-ts-structure.md` 新增 §1.3 实例 + §5.0 命令行兜底；
`playback-address-interfaces.md` 新增 §2.5 B 站 + §3B.4 抖音；`SKILL.md` 分流表 +2 行）、
`web-reverse-algorithm`（**新** `references/19-opaque-source-patching.md` 约 300 行 +
**新** `scripts/php_opaque_patch.js`（`--selftest` **40 项**）；`02-algorithm-families.md` §一 新增「多段链式签名」、
§七 补同族指针）、`web-js-env-patcher`（`browser-acquisition.md` 新增「自动化框架自曝指纹」小节 + 原生基线表）。

### 本批的「结论级发现」（三条，均由本库实测/复算抓出）

1. ★★★ **CBC「错 IV」的破坏面是「两 IV 的差异位置」，不是「整个首块」**。
   源文 `52pojie-977455` 明写「偏移量没写，默认为 0」并**用同一个全零 IV 解了全部分片**、还贴了「播放正常」的截图。
   本库用**两个独立实现**（openssl 3.5.7 与 3.0.13；真机 Chrome WebCrypto）把它量出来：
   分片 1 的正确 IV 是 `00…01`，与全零**只差最后 1 字节** ⇒ 解出来的明文**只坏 1 个字节（下标 15）**，
   第 16 字节起逐字节完好，**首字节 `0x47`（TS 同步字节）存活** ⇒ 落在 TS 首包载荷里，**肉眼与 `ffmpeg -v error` 都不报错**。
   ⇒ 判据升级：**遇到「作者说用零 IV 解全片而且能播」的帖子，既不要照抄，也不要用它反推「HLS 默认 IV 是 0」**——
   那是**破坏面太小所以没暴露**；正确取值仍是**媒体序列号**。
2. ★★ **源文之间可以用「同一作者的时间差」做版本 delta，且 delta 本身就是判据**。
   抖音两篇（`52pojie-968123` 2019-05 / `52pojie-1022592` 2019-09）给同一族接口串：
   **`_rticket` 从有到无**、`version_code` 从写死到模板化、`manifest_version_code` 出现**与 `version_code` 不等**（650/660）。
   最值钱的一条：**`ts`（秒）与 `_rticket`（毫秒）是同一次请求的同一时刻**（`floor(_rticket/1000) == ts+1`），
   ⇒ 复算/重放时**不能各自取一次时间**。另：**`as`/`cp` 是设备指纹对**（同篇内等长、跨篇 18 vs 22 位）⇒ 不可复算、不可抄常量。
3. ★★ **「源文的 Markdown 转义会泄漏进代码块」是一族可执行的陷阱**（本批回源门禁抓出）。
   `52pojie-2116200` 里方法名逐字是 **`is\_local` / `zib\_init\_add\_action` / `json\_encode`**，
   连 **`function is\_local(){`** 都是转义的 ⇒ 直接拿源文当输入时，**按名字做替换会全部落空且不报错**。
   本库因此给工具加了 `normalizeMdEscapes`（把 `\_` 还原成 `_` 并**回报归一化处数**），
   自检第 7 组用「未归一化 ⇒ 找不到方法」+「归一化计数 = 4」+「幂等」三对断言钉死。
   （同族：B38 的「HTML 命名实体残留族」，本条是 **Markdown 转义**那一支。）

### 本批登记的口径边界与「单源/未复核」项（勿当结论用）

- `52pojie-1177888` 同时给出 `BV1Yk4y1r7jM` 与 `185221286`，而**公开 av↔BV 算法**（本批用 3 组公开对照对
  **双向自证**：`av170001 ↔ BV17x411w7KC` 等；Node 与真机 Chrome 双实现同值）算出
  `bv2av("BV1Yk4y1r7jM") = 752883075` ⇒ **两者不互指**。`185221286` 不是 avid（很可能是 cid 之类的内部 id）。
  **单样本，不判因**；`upgcxcode/<n%100>/<n//100%100>/…` 这条路径规律**只在源文的两个 URL 上成立**。
- `52pojie-1177888` 的「响应里搜 4K 找到的那条不是 4K」：源文只给现象与截图，**没给接口名与字段名**
  ⇒ 本库只登记「`qn` 码段 `30080=1080P` / `30120=4K`」这一层，**具体字段名不替它补**。
- B 站 `upsig` 的**算式与密钥源文未给** ⇒ 登记为未复核（只登记「`uparams` 白名单不变量」这一条结构判据）。
- `52pojie-1022592` 的 Java 截断写法在**无 query 的 URL** 上会抛异常 —— 该结论基于 **Java 语言规范**
  （本批机器可验证的部分只有「`lastIndexOf('?')` 返回 `-1`」），**未在 JVM 上实跑**。
- `52pojie-968123` 的 cookie 被作者**自己脱敏**成 `##` ⇒ 本库**保留脱敏形态，不代为还原**（纪律：源文自己打码的，落点必须保留打码）。
- `52pojie-2116200` 存在**口径张力**：源文一边说「`require.php` 不能修改」，一边把 `new_aut.php` 的
  `zib_init_add_action()` 替换掉 ⇒ **如实登记，不替源文裁决**；源文结尾亦自陈「**不排除是否还有自毁机制没改到**」。
- `52pojie-896034` / `897646` 是 **2019 年**路线 ⇒ 「`excludeSwitches` 已不再充分」这一判定来自
  **本批真机基线表**，不是对源文的反驳（源文当年的环境里它可能确实够用）。

### 本批自曝缺陷（全部在本轮内修完）

1. ★★★ **「注入到错的镜像里」= 第四类假绿**：`check_skill_integrity.js` 的基目录是 **`.claude/skills`**，
   而 `b40-verify-docs.js` 是 **`.agents/skills`** ⇒ 故障注入只写一边时，**另一个检查器根本看不到注入**，
   表现为「这条门禁不会红」——其实**是我的夹具没送到**。⇒ 纪律：**故障注入必须同时作用于两个镜像**（已写进脚本注释）。
2. ★★ **本批首版 `b40-hls-cli-check.sh` 有 4 条断言是「我的错」**（不是代码的错）：
   ① 以为 `-nosalt` 对 `-K` 也有影响（实测**裸 key 不走 KDF ⇒ 无影响**）；
   ② 以为错 IV 会坏满 16 字节（实测**只坏 IV 差异位置**）；
   ③ 以为首字节 `0x47` 一定会被破坏（实测**序列号差时首字节完好**）；
   ④ glob 断言写成了「`index10` 紧跟 `index2`」（实际中间还有 `index11`）。
   ⇒ 纪律重申：**断言失败先怀疑断言**；且**断言要覆盖「写法边界」而不只是「语义正确」**。
3. ★★ **`python` 不在 subprocess 的 PATH 上**（交互 shell 有、`subprocess` 里没有）⇒ 12 条断言全部**空输出**，
   看起来像「脚本坏了」。已改为**逐个探测 `python3`/`python`/`py` 并把解释器与 openssl 版本打印出来**；
   顺带发现本机**两个 openssl（3.5.7 / 3.0.13）**，两套环境下 11/11 均绿 ⇒ **跨版本稳健性顺带被证实**。
4. ★ **工具夹具本身不合法**：首版夹具把源文里被论坛转义过的 `&#229;` 抄了进去，
   而我的扫描器把 `#` 当 PHP 行注释 ⇒ `is_local` 整行被判成字符串。
   ⇒ 已把夹具改为**合法 PHP**，并把这条登记为「实体残留是搬运产物、不是源码」的实例。
5. ★ **`ensureStatic` 的「先补后清」顺序坑**：第一版自检直接把这个 bug 抓出来了
   （先补 ⇒ 残留行被判为「已有 static」⇒ 不补；再清 ⇒ 残留被删 ⇒ **两头都没了**）。已改为**先清后补**并断言幂等。
6. ★ **文档门禁扩到「被引 Top-60」后抓出 1 处存量缺陷**：`wsam-reverse/references/wasm-runtime-reproduction.md`
   里 `env-native-protection.md` 是**裸名跨技能引用**（缺 `../../web-js-env-patcher/references/` 前缀，两处，一处在正文一处被同句的另一条正确引用衬出来）。
   ⇒ **实证**：该缺陷在 `--topn=0` 下**完全不可见**（本批专门做了对照实验）⇒ **「被引 Top-N」这一档仍有独立价值，尚未收敛**。

6. ★★ **语料里出现了「文件名含反引号」的源文，而台账口径无法表达它**：
   `52pojie-2122819-从 RegisterNatives 到逐字节复现：一次 Android \`ttEncrypt\` 封包逆向.md`
   的文件名**自带一对反引号**，而 `scan-pending.js` 的登记行正则要求「**反引号包裹的 `.md`**」
   （`([^\`]+\.md)`）⇒ **这一行永远无法被识别**，表现为「台账 778 / 待处理 484」而不是 779 / 483
   —— **即该文会被无限重复处理**（正是 `check_skill_integrity.js` 规则 8 想防的「永久漏做」）。
   ⇒ **附带修复**：把该文件**重命名为不含反引号的等价名**
   （`…一次 Android ttEncrypt 封包逆向.md`，**内容零改动 ⇒ md5 不变**），并同步台账行。
   实测：台账 778 → **779**、待处理 484 → **483**。
   ★ **口径纪律（写进下一批的取材动作）**：**语料文件名不得含反引号**；
   归档线新增文件时应加一条「文件名不得含 \` / | / \」的机械校验（否则台账无法登记）。

### 本批的验收（全部实跑）

- `check_skill_integrity.js`：**0 阻断 0 告警**（台账 722 → **730** / 候选 1213 / 待处理 491 → **483**）。
- `b40-verify-docs.js`：**0 阻断 0 告警**（改动集 9 + 一跳 82 + 二跳 33 + **被引 Top-60 并入 58** = **155 文件**；
  路径引用 479 处 0 悬空 / `§` 引用 80 处 0 悬空 / 小节号 0 异常 / 裸名跨技能 **0** / **跨技能 `SKILL.md` 71 处 0 悬空（本批新增判据）** /
  蓝图裸名 0 / 带引文标题引用 9 处 0 漂移）。
- `b40-verify-numbers.py`：**53/53**（含 av↔BV 双向自证 6 项、`uparams` 不变量 9 项、`ts`/`_rticket` 配对、
  盐出现次数 1/3、两段 md5 复算、`-1` 溢出排除、两条子脚本复跑）。
- `b40-verify-sources.py`：**88/88**（正面针 + **负面针**，覆盖 8 篇源文；负面针用于钉死「源文确实没有 X」）。
- `b40-fault-injection.py`：**12/12**（7 组注入，4 组带阴性对照；★ 全部**双镜像**注入）。
- `b40-hls-cli-check.sh`：**11/11**（且在两套 openssl 3.5.7 / 3.0.13 下均绿）。
- `php_opaque_patch.js --selftest`：**40/40**。
- ★ **browsercli 真机 Chrome**：**30/30**（A/B/C/D/E 五组：原生指纹基线 12 项 / 实例级补丁可检测性 5 项 /
  av↔BV 跨实现 5 项 / 滑动公式零熵 2 项 / **WebCrypto 与 openssl 跨实现对拍 6 项**）。
- 双镜像 `.claude/skills` ↔ `.agents/skills`：逐字节一致。
...
| # | 文件 | md5 | 处理时间 | 关联技能 | 变更类型 | 核心萃取 |
| --- | --- | --- | --- | --- | --- | --- |
| 723 | `52pojie-977455-ts加密视频文件下载、解密、合并.md` | `181bf009e89e92acf0418964aa11eb0c` | 2026-09-25 | `stream-drm-reverse` | evolve | HLS AES-128 手工链路：`EXT-X-KEY` + key 独立 URL + **无 IV**；`openssl enc -aes-128-cbc -d -nosalt -K <hex>` （★ `-K` 裸 key vs `-k` 口令 KDF 是两回事，源文正文写 `-k` 是笔误）+ `copy /b *.ts` 合并（★ 通配符按**文件名字典序**，`index10` 排在 `index2` 前 ⇒ 零填充或显式列顺序）；★ 源文「偏移量没写默认为 0」是**静默错误**：本库实测**错 IV 的破坏位置恰为两 IV 的差异位置**，仅序列号差 ⇒ **只坏 1 字节（下标 15）**，落在 TS 首包载荷里、首字节 `0x47` 完好 ⇒ 肉眼与 ffmpeg 都不报错 |
| 724 | `52pojie-1177888-bilibili哔哩哔哩视频下载第一课（原理）.md` | `ed891d92b0aa0e6414c570f6ccea6cca` | 2026-09-25 | `stream-drm-reverse` | evolve | B 站 `playurl` → DASH `.m4s` 直链（仅需 `Referer`）：★ 判据一 **`-1-` 视频 / `-2-` 音频分流** ⇒ 源文结尾自陈的「下载了没有声音」的真因；★ 判据二 **「响应里搜到 4K ≠ 拿到 4K」**（可用码流由**账号权益**决定，`30080` 实测 1080P / `30120` 实测 4K）；★ 判据三 同站并存两套签名方案（`expires+ssig` vs `e+upsig+uparams`），**`uparams` 是签名白名单，`upsig`/`uparams`/`mid`/`logo` 不在其中**（本批 2/2 复算成立）；路径段规律 `upgcxcode/<n%100>/<n//100%100>/<n>/…`；⚠️ 登记一处未解不一致：公开 av↔BV 算法（3 组对照对双向自证）给出 `bv2av("BV1Yk4y1r7jM") = 752883075 ≠ 185221286` ⇒ 后者不是 avid |
| 725 | `52pojie-968123-这才是某音解析最新源码,别拿第三方的来糊弄人.md` | `a9405851b32df762f6adf7ddf29b2e28` | 2026-09-25 | `stream-drm-reverse` | evolve | 抖音 APP 协议去水印（PHP 版）：分享文本 → `itemId: "` → `api-hl.amemv.com/aweme/v1/aweme/detail/` → `play_addr.url_list`（4 条）；★ 本批复算 **`ts`（秒）与 `_rticket`（毫秒）是同一次请求的同一时刻**（`floor(_rticket/1000) == ts+1`）；★ `as`/`cp` 是**设备指纹对**（同篇内等长 22/22，跨篇 18 位）⇒ **不可复算、不可抄常量**；响应字段全集（封面 / 音频 / 作者 / 描述）；⚠️ 源文 cookie 被作者自己脱敏成 `##` ⇒ **本库保留脱敏形态，不代为还原** |
| 726 | `52pojie-1022592-某音无水印最新算法.md` | `eaa8284904d26f429954a10b27e94982` | 2026-09-25 | `stream-drm-reverse` | evolve | 同族 Java 版（同一作者相隔 3.5 个月）⇒ 与 PHP 版互为交叉验证：给出版本迭代 delta（`_rticket` 有/无、`version_code` 写死 vs 模板、`manifest_version_code` **可与 version_code 不一致** 650 vs 660）；取地址两条分支 **先 `long_video[0]` 再回落 `video`**；⚠️ 源文缺陷登记：`url.substring(indexOf("http"), lastIndexOf("?"))` 在 URL 无 `?` 时 `substring(0,-1)` **抛异常**（Java 语义） |
| 727 | `52pojie-1059382-卓易健康PHP步数接口.md` | `1b2bf300eb2003bb763614ea589a8edf` | 2026-09-25 | `web-reverse-algorithm` | evolve | ★ 多段链式签名判据集（标准签名题里最易被误判成复杂算法的一族）：① **同一盐值在一段串里出现多次**（本批复算段 1 出现 1 次、段 2 出现 **3** 次）；② **两段共用同一个时间戳**；③ **上一响应字段（`openid`）入下一段签名**；④ 表单字段顺序 ≠ 拼串顺序；★ 两条形态坑：参数名逐字是 **`jibuNuber`**（源文如此拼写，改对反而 400）、`-1 是 98800` **不是 16 位溢出**（本批复算 `(-1)&0xFFFF == 65535 ≠ 98800`，且 `98800 > 65535`） |
| 728 | `52pojie-2116200-分析某wp主题9.0逆向授权过程.md` | `44238cb4fc642557844d656bfc12a275` | 2026-09-25 | `web-reverse-algorithm` | evolve | ★ 新参考文件 `references/19-opaque-source-patching.md` + `scripts/php_opaque_patch.js`（`--selftest` 40 项）：**不透明源码的定向补丁**（Z5Encrypt 族，不解密、只整段替换方法体）—— 锚点 **`);$_SERVER;}`**、解密器在**文件头 gzinflate 全局块**、变量名**每次都变的 latin1 乱码**（别按变量名匹配）；★ 「改哪个方法由**调用方**决定」（`is_aut` 是总开关是从 `options-module.php` 反推的）；★ **返回值类型由「调用方怎么用」决定**（远程通信给 `""` 而非 `false`；会被 `json_encode` 的给数组；会被 hash 比较的给合法 md5）；★ **哪些绝不能动**（`eval` 里定义业务函数 / 注册钩子的，动了全局崩）；★ **自毁方法必须与授权方法一起拦**；★ 补 `public static` 必须**先清残留再按缺补**；★ 替换后必须过**括号配平** |
| 729 | `52pojie-896034-selenium跳过webdriver检测并模拟登录淘宝.md` | `1d044e7434cefbe02823e71893a8aeba` | 2026-09-25 | `web-js-env-patcher` | evolve | 自动化框架自曝指纹：**2019 年口径 vs 现行判定**对照表（`navigator.webdriver` / `excludeSwitches:['enable-automation']` 已**不再充分** / ★ **「不加载图片」本身就是一个自动化特征**（页面 JS 读不到，但可被动观测「声明了 N 张图、图片请求 0」））；★★ **原生状态基线表**（真机实测：`navigator.webdriver === false`、定义在 **`Navigator.prototype`**、getter 是 native、实例**没有** own property）与「**不要 patch `navigator.webdriver`**」的**四重可检测性**（多出 own property / getter 非 native / 原型链变两处 / `configurable` ⇒ 可被 `delete` 掉再看）；★ 源文真正值钱的是**半自动化的分工**（人工过登录 → 复用登录态 → 机器取数） |
| 730 | `52pojie-897646-selenium跳过webdriver检测并爬取淘宝我已购买的宝贝数据.md` | `cb480fde7c21c3a6625e9a98ba0b028d` | 2026-09-25 | `web-js-env-patcher` | evolve | 同系列代码版 ⇒ 补出「**位移是确定公式、只有延时随机**」这条假拟人判据：源文滑动 JS 是 `scrollTop = 300+400*i` / `200*i`，本批**真机复算**序列 `300,200,1100,600,1900,1000,2700,1400` 完全由下标决定（**零熵**），而 `sleep` 用了 `random.randint(1,3)` ⇒ **只随机延时 = 最常见的假拟人** |

---

## 批次 B41 · 2026-09-25（第四十二次执行）

**取材口径**：**归档线第 41 轮新收 49 篇全取（49/49 命中）** —— 承接上一批「归档线第 41 轮」的优先级 ①。
该轮归档自述：「G 表体检揭出「口径过时」的系统性误杀（**早期「只收 web」把 App 端接口签名/响应加解密整类打成噪声**）」
⇒ 本轮供给 49 篇、**单轮新高**，且**首次出现 App / native 簇占绝对多数**（**38/49 ≈ 78%**）。
簇：客户端签名与封包（38）· 网盘直链（2）· WS 封包（1）· 二进制协议（1）· 前端反检测/滑块（3）· 页面定位法（1）· 小程序调试（1）· 片段帖不落盘（2）。

### 本批次登记表

| # | 文件 | md5 | 处理时间 | 关联技能 | 变更类型 | 核心萃取 |
| --- | --- | --- | --- | --- | --- | --- |
| 731 | `52pojie-574587-记录某书中的包内容签名值.md` | `232b4eee193367af22dbfe6cc08c300b` | 2026-09-25 | `android-app-reverse` | evolve | ★★ `X-Auth-1 = MD5(f() + 秒级ts)`、`X-AUTH-2 = MD5(mobile_token + 秒级ts)`（**同一时间戳复用于两个 header**）；`f()` → native `encryptAppKey()` → 简单字符串拼接得固定 `18ce9f021f1840fb93108ed2a899fb3a`；★★ **IDA F5 不正常时的标准动作：流程图模式 → `Remove function tail` → `Force BL call` ⇒ 再 F5 就正常** |
| 732 | `52pojie-583158-APP签名算法提取之曲线救国.md` | `40125a0a5877feda6cd01bc6d1df25d6` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **「曲线救国」范式（不可还原 ⇒ 借 App 生成）**：so 里 `EncodeMD5` 是**魔改 MD5**（不是 `MD5(数据+key)`）⇒ **自建 Android 工程**：`jniLibs/<abi>/` 放 so + **新建与 so 注册完全相同的包名路径** + 照搬 native 方法声明 + `System.loadLibrary` + `onCreate` 调用打印 ⇒ 用真机当签名机；★ 签名串 = `a:…c:…FuncTag:…ir:…phoneNum:…platform:…smsType:…userId:` 按序拼接；★ `System.loadLibrary("encode")` 定 so 名；★ 源文自陈局限「效率低、模拟器兼容差 ⇒ 下下策」 |
| 733 | `52pojie-676712-某快药签名算法分析.md` | `07f81de4d417176f4eb6ece42fb8e3f3` | 2026-09-25 | `android-app-reverse` | evolve | ★ 定位手法：**搜 `sign` 命中太多 ⇒ 改从业务入口（`login` 目录 → `LoginActivity` → 登录函数）顺流而下**；签名 = `MD5(method + 排序后 key+value 拼接)`（`Collections.sort` + 自定义比较器，`sign` 自身排除），结果 `toUpperCase()`；so = `libNoodleMD5.so`，导出 `nativeEncryptByMD5(str1, str2)`；★ 判据「**c 代码不可全信，具体执行还要跟 smali**」 |
| 734 | `52pojie-817122-某APP的API签名破解记录.md` | `00b886ad9d01512f86761607a6e996f3` | 2026-09-25 | `android-app-reverse` | evolve | ★★ iOS 侧手法（砸壳 `.ipa` + `class-dump` 头文件 + MonkeyDev/CaptainHook `CHDeclareClass`/`CHClassMethod1`）；`XYAPIEncodeSign` 三方法（`md5:useLower:` / `URLencodeSpacetoPlus:` / `signWithQueryItems:`）；★★ 算法 = `URLEncode(key 排序拼接)` → **逐字符 XOR（deviceId 作循环 key）** → ★★ **把 XOR 结果拼成「十进制数字串」**（不是 hex/字节）→ `MD5(该串 + deviceId)`；★ 判据：hook 返回值与抓包对比一致即确认；★ 「输出形态可读时优先看输出而非读实现」 |
| 735 | `52pojie-877390-Python实现京东自动登录（自动完成滑块验证）.md` | `a9da703983540ce1b29e46b4000573d2` | 2026-09-25 | `web-js-env-patcher` | evolve | 2019 年 selenium + cv2 滑块常规样本 ⇒ 并入新增的「滑块拟人化第二层」判据组；★ 结论层（`excludeSwitches`）为旧口径，**不要照抄** |
| 736 | `52pojie-1137911-记一次逆向某大学app寻找签名算法.md` | `7c706dbf7c9fbc28568d3e04ca419a51` | 2026-09-25 | `android-app-reverse` | evolve | 签名串形态 ①：`value1|value2|…|SALT` → MD5，`Arrays.sort` 后按 key 取 value；★★ **判据：「开发者自己写的 MD5」实测就是标准 MD5 32 位**（别因代码里有自写 `getHexString` 就以为魔改）；调用链 `creator → generateReq → getSignString` |
| 737 | `52pojie-1159541-python模拟哔哩哔哩滑块登入验证.md` | `0f269a9769e5157f9a36396720ed632c` | 2026-09-25 | `web-js-env-patcher` | evolve | ★ 滑块识别路线 A：`GaussianBlur` → `Canny(100,500)` → `matchTemplate(TM_CCOEFF_NORMED)` → `minMaxLoc` 取 `max_loc`；★★ **截图裁剪用硬编码魔数偏移（`+160/+60/+230/+110`）**、识别 x 与位移有固定映射 `x_dis = int(dis)+135`、**「变速拖动」= 把速度换成另一组分段常数**（位移-时间曲线仍处处直线）；★ `excludeSwitches` 旧口径 |
| 738 | `52pojie-1160287-js逆向技巧分享.md` | `64b8655c5c2238b8e7c37e466c026e69` | 2026-09-25 | `web-reverse-algorithm` | evolve | ★★ **可引用的覆盖率口径**：定位加密参数三法（全局搜索 / DOM 事件 / XHR 断点）约覆盖 **60%**，**剩下 40% 必须上 hook**；★ 三法边界：全局搜索「容易搜不到」、DOM 事件「定位靠前（明文入口）」、XHR 断点「只能覆盖 XHR，js 发包/表单发包抓不到」；★★ 新增动作：**全局搜索要带分隔符精确化**（`password=` / `password =` / `password:` / `password :`） |
| 739 | `52pojie-1243816-某狐资讯app的sign加密参数的分析和HOOK.md` | `88beee638c97060d245e6529fcfd2f3e` | 2026-09-25 | `android-app-reverse` | evolve | ★ **交叉定位判据**：搜 `pid` 与搜 `sign` 最终走到**同一个方法**；IDA 静态注册导出表 → 找 MD5 关键点 → **hook `update` 偏移 + hexdump 验证**；`pid` 随机 / `mid` 类 deviceID 固定 |
| 740 | `52pojie-1262453-豆瓣app 网络请求签名算法分析与解密.md` | `96e5faf3609eecab85c1fa64377577cf` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **「HMAC 密钥被公钥包裹」范式**：`_sig = Base64(HMAC-SHA1(key, msg))`，而 **key = AES(密文常量 `c`, 密钥=APK 签名)**；★★ 关键手法：**不改包也能拿到 APK 签名**（另一个 App 填包名即可读已安装 App 的签名）⇒ 密钥可预计算（`bf7dddc7c9cfe6f7`）；`Base64.encodeToString(..., 2)` 的 **2 = NO_WRAP**；★ **UA 必须一致**；★ 模拟器必须 **Android 5.1**（6.0+ 不信任用户证书） |
| 741 | `52pojie-1332557-某种草电商App签名算法解析(一).md` | `318f7b6f3dd648490021109074064035` | 2026-09-25 | `android-app-reverse` | evolve | ★ 判据：**签名在 Java 层搜不到 + base64 形态 ⇒ 转 native**；★★ 手法：`frida_hook_libart` 的 `hook_art.js` hook **`NewStringUTF`**，**按「字符串长度 > 50」过滤再打堆栈** ⇒ 定位 `libshield.so+0x93fa8`；★ `sub_939D8` 在 IDA 里按 **`X`** 找调用者 ⇒ Java `intercept`；★★ 结论：**参数拼接+签名+发包全在 so 里 ⇒ 拿不到干净接口**，只能 hook 拼接过程改参数 + hook 返回值 |
| 742 | `52pojie-1388335-某咖啡app网络请求签名算法分析与解密.md` | `91616b7c9a0900a17c8166ed464270f2` | 2026-09-25 | `android-app-reverse` | evolve | ★ 判据：全局搜 `sign` + hook 无功而返 ⇒ **改搜 `System.loadLibrary`** 定 so；★ so 名被**索引化**（`StubApp.getString2("17534")`）⇒ hook `getString2` 还原；★ `md5_crypt` 入参形态 `key=value` 用 `=` 拼、`;` 组合（含 cookie 的 uid）；★★ so 导出 `doMD5sign`：`md5(query+salt)` → digest → **分 4 组各 4 字节 `int.from_bytes`，小于零则 ×-1** |
| 743 | `52pojie-1445251-东-sign签名获取.md` | `ddbe56596c004b8983153fccbbed409b` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **frida RPC 范式（本批路线一）**：直接调 native `BitmapkitUtils.getSignFromJni(context, functionId, bodyString, uuid, 'android', version)`；★ 三参数缺一不可（`body_string` 传错 sign 必失败）；★ `sign` 每次不同；返回 `st=…&sign=…&sv=102`；★ **高版本检测 frida、低版本功能不全 ⇒ 选 9.2.0**；⚠️ 源文含 `&#8232;`（U+2028）实体残留 ⇒ 又一处论坛搬运污染实例 |
| 744 | `52pojie-1476924-某生鲜电商sign签名算法分析.md` | `067390214e971897e8ce9de852ba2889` | 2026-09-25 | `android-app-reverse` | evolve | ★★ **「时间戳是否参与签名」的零成本实验**：① 去掉 signature →「认证过期」⇒ 确认它是加密参数；② 只传 signature 不传 timestamp → 仍失败 ⇒ **证明 timestamp 参与签名**；签名 = `MD5(固定盐 + timestamp + token)`；★ 32 位数字+小写字母 ⇒ 先猜 MD5；★ 向上/向下双向追溯（赋值关系反推语义）；★ 文本 diff 工具对比多次 cURL |
| 745 | `52pojie-1546549-最右 5.7.3 app sign aes 参数分析.md` | `b7f6ac41f2928b05617f4445ad35581b` | 2026-09-25 | `android-app-reverse` | evolve | ★★ **`iv` 拼接到密文前**（`密文 = iv + cipher`）—— 与「`ivlen+input_len`」判据同族，解密时 `iv = 参数一前 16 字节`；★ 判据：so 调 **openssl `EVP_EncryptInit_ex`，其后两个参数分别是 key / iv**；★ 三函数 `encodeAES` / `sign`(变形 md5) / `decodeAES`；★ **AES 解密后还有一步处理**（非明文）；★ **动态 key/iv 对 unidbg 调试的影响 ⇒ 先在生成点 `mem_write` 固定** |
| 746 | `52pojie-1557273-dewu得某APP newSign签名算法的python实现.md` | `89ad842996fdeea65ce4ed2f9b9f73b3` | 2026-09-25 | `android-app-reverse` | evolve | 得物 `newSign` 的 **Python 复现**（`AES_128_ECB_PKCS5Padding`）：★ 与既有 `02-algorithm-families.md` §一「得物 newSign（so 层 AES-ECB）」同题 ⇒ **补齐了「纯 Python 复现」这一侧**；⚠️ 源文只贴代码、未给逐步推导 ⇒ **本库只登记形态，不据它修订既有得物口径** |
| 747 | `52pojie-1565766-JS逆向赛尔号启航手游逆向.md` | `6992b1cec925b85f0edaa695a71e457b` | 2026-09-25 | `web-reverse-algorithm` | evolve | ★★ 阻碍一：入口 JS 名带时间戳（`bootstrap+<ts>.js` 再 `document.appendHead` 链式加载）⇒ **DevTools 本地替换固定文件名**；★★★ 阻碍二：**VM 内脚本无法本地替换 ⇒ 在「加载它的非 VM JS」的 HTTP 响应末尾追加 `//@sourceURL=<名字>.js`**，再执行原 `appendHead` ⇒ **把「不可替换」变成「可替换」**；★ 搜 `sendmessage`/`receivemessage` 定发包收包；★★ **引擎类站点业务对象常挂全局 ⇒ 直接调 API 跳过模拟点击** |
| 748 | `52pojie-1572670-某应用sign签名算法还原.md` | `06f2cf7ed4a0d4954d3b2c1a5ee36244` | 2026-09-25 | `android-app-reverse` | evolve | ★ 多 so 交叉定位（`c-lib`/`sos`/`crypto` 三个 `loadLibrary` 分别 hook 动态注册）；★★ **unidbg 版本敏感性**：API 23 报错（`ContextWrapper`/`GetMethodID`/`free` 失效）⇒ **换 API 19 直接成功**（「死马当活马医换版本」是有效动作）；★ `strcat` hook 补出未解参数；★ **`DES_ede3_cbc_encrypt`（3DES-CBC）**，key1/2/3 来自 `0xb88`、iv = `01234567`；★★ **Thumb 指令 `+1`**（`hookZz.wrap(base + 0xb88 + 1)`）；★★ **应用改过的 base64：输出中间插了两个空格**，且 **jadx 反编译该函数不正确 ⇒ 换 Procyon 引擎**才拿到正确逻辑；两次 md5（请求参数一次、签名一次） |
| 749 | `52pojie-1580675-某flutter app参数分析与进阶.md` | `0e7a69253a00563b10063023183fe7c5` | 2026-09-25 | `android-app-reverse` | evolve | ★ **Doldrums**（`rscloura/Doldrums`）处理 `libapp.so`：`python3 main.py libapp.so output` ⇒ **在输出里搜 `md5`/`update`** 定位算法 ⇒ frida hook 拿结果；★ 判据：抓包发现 `libflutter.so` ⇒ Flutter 题 |
| 750 | `52pojie-1619961-手把手分析蓝奏直链.md` | `9f506283b9a735c453bb8606c69d502a` | 2026-09-25 | `cloud-drive-direct-link` | evolve | ★ 单文件手工链：`<iframe>` 取 `src` 与原站 host 拼新 URL → 页面异步 **POST `/ajaxm.php`**（form）→ 响应 JSON 的 `dom` + `url` 拼接 → **302 响应头 `Location` = 真实直链**；★ 陷阱「地址超时，请刷新」**不是失败，是要手快** |
| 751 | `52pojie-1737837-【洞见者】sign参数加密算法逆向.md` | `25a32f3624813e284a214ffc5ce36318` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 形态 ⑤（罕见）：`sign = MD5( (盐 + base64(json)).split('').sort().join('') )` —— **盐 + base64 后「逐字符排序」再 MD5**；★ 定位：32 位字母数字混合 ⇒ 先猜 md5，再搜 hash 值定位函数名 `getSignBody`；★ `Inspeckage` 免脱壳可看到加密前原文（但**全是乱码**）；★ 梆梆企业版用 **DITOR 通用模式直接脱掉**；★ 反面教材：**「消息长度 header 会影响签名」是论坛错误说法（作者称无关）** |
| 752 | `52pojie-1749556-某星充电APP signature参数分析.md` | `39748c37476660c13cb9e8c564516bb4` | 2026-09-25 | `android-app-reverse` | evolve | ★ **双签名**：header `X-Ca-Signature` = 参数 + 固定 key 拼接后 MD5；站点详情 body 的 `Signature` = **H5 加载 js 生成**（随机 uid + 时间戳）；★ 防抓包 ⇒ 用**算法助手**（frida 一键）正常抓包 |
| 753 | `52pojie-1784353-coco2djs手游封包协议分析jsc文件解密[客户虐我千百次,我待客户如初恋].md` | `5f7471ddb435e095659d34f54206c3f3` | 2026-09-25 | `websocket-reverse` | evolve | ★★★ **本批封包分层最完整的一篇**：① 判据「游戏一般不加固」（频繁更新）；② **charles 抓不到 ⇒ 换 httpcanary**（WebSocket）；③ **大段乱码且数据量大 ⇒ 压缩传输**（不是加密）；④ ★★ **jsc 解密 key 定位**：hex 打开 `cocos2djs.so` 搜 ASCII `main.js`，旁边 `0ed68e2d-8b2f-4a…` 即 key；⑤ ★★★ **解码顺序**：4 字节 `headerFlag`（判压缩→`inflate`）→ 4 字节长度（**字节集反转=大小端/网络序**）→ 1 字节 `moduleEnum` → 1 字节 `cmdEnum` → 2 字节 `statusCode` → 余下 `readBytes`；⑥ ★★ **发送包必须带 `HashCode`**，否则「前多两字节、后丢两字节」；⑦ 源文自陈边界：**取巧路径依赖「未混淆」，混淆后须回退 `ida` 定位 xxtea + frida hook 提 key** |
| 754 | `52pojie-1889762-酷狗概念版的签名验证Bypass+签名算法.md` | `d7764b5ccd90dcda3a18a2d04bac767c` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 签名校验 Bypass 链：行为分析（启动跳网页/结束进程）→ 定位 C 层 `libj.so`（字串表搜网页 URL → 交叉引用 → `Intent` 开 `HttpsActivity`）→ 010 Editor 删字段 → **替换后「点击登录又跳」⇒ 说明有第二个触发点**（`DetectUtils`）→ **`DetectUtils.a` 写死 1 绕过**；★ 签名校验实现 = `PackageManager` 取签名 → JNI 判断 ⇒ **PMS 代理工具**绕过；★★ 「改一处后还有第二处 ⇒ 必须找全所有触发点」 |
| 755 | `52pojie-1897636-某APP的sDjcSign加密.md` | `25be2d3adae0c83e87ae589a0f7ba22d` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **形态 A 最典型**：`sDjcSign = b(RSA(AES(sb2, key)))`，AES 明文 `sb2 = openid + sDeviceID + timestamp + "148"`；★ 判据：**每次结果不同 ⇒ 先怀疑 RSA**；★ 手法：frida 逐层 hook（AES / RSA / byte2string）打印明密文；★ 判据：**`'[B'` 入参 + `string` 返回 ⇒ byte2string**；★ RSA 公钥**从文件读**（`djc_rsa_public_key_new.der`）⇒ 复现也用 Java；★★ **「长度一致 ≠ 正确」，必须发包验证**；★ `djcRequestId = sDeviceID-timestamp-random`；★ 有效期约 3 个月的参数可手工抓一次 |
| 756 | `52pojie-1900055-[Android 原创] 某app目录接口逆向解密分析过程.md` | `1a861c5ab6fbdf0a80b17eb85e610fda` | 2026-09-25 | `android-app-reverse` | evolve | ★ 响应 AES-CBC 形态 ⇒ jadx 定位 `AesUtils.decrypt()`（标准库）⇒ frida hook `requestPost` 拦明文；★ 短文，作为「响应加密先查标准库工具类」的实例 |
| 757 | `52pojie-1915954-某练通登录sign算法逆向.md` | `d7177ca3a3bf34df43403507288282b1` | 2026-09-25 | `android-app-reverse` | evolve | ★★ **换靶实证**：搜 `sign` 无结果 ⇒ **换搜业务名 `UserLogin`** 命中（与 B37「四级关键词表」同源）；★ 判据：**jadx 看到目录类很少 ⇒ 判断加壳**，查壳报「百度壳」但 **`mt` 一看是「伪百度」⇒ 黑盒脱壳**；★ 签名 = `SIGN_KEY + concat(参数值)`（14 个参数按序拼）；★★ **`Pass = MD5(URLDecoder(URLEncoder(MD5(ps)+id,"GB2312"),"GB2312"))`** —— URL 编码往返（GB2312）是一种「伪加密」形态；★ timestamp = GMT+8 格式化再 parse 的秒级；★ **LoginID 来自上一接口 `UserTipForChangePass` 的响应**（链式依赖） |
| 758 | `52pojie-1964051-记一次使用 Xposed RPC 和 BurpGuard 应对金融APP参数签名及加密的详细过程.md` | `c1f478b1970718c1d166b641963c969c` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **RPC 化路线二（frida 被检测时选 Xposed）+ 完整工程范式**：App 内起 `NanoHTTPD`（`/encrypt` `/decrypt`）+ `adb forward tcp:50000`；★ 原话「**Xposed 没有检测，但 frida 有检测**」；★ `BurpGuard`：mitmproxy 双 handler（`ClientProxyHandler` 解密 / `BurpProxyHandler` 加密），**模拟器代理 8081 → Burp 上游 8082 ⇒ Burp 里看到明文可改包**；★ **白盒 SM4（`WbSm4().encode(len, bytes)` 不传密钥）** ⇒ 密钥难提取 ⇒ **直接 RPC**；★ 签名 `AppSign = generateAppSign(url, uuid, body)`；★ 爱加密企业版「算法助手 Plus + frida 脱壳**脱不完整**」（代码被抽到 native） |
| 759 | `52pojie-1982524-微信小程序逆向分析：美味buyong等.md` | `e4689ea3d9adb8cbc6eef4a65dba8d9e` | 2026-09-25 | `miniprogram-reverse` | evolve | ★ **未完结求助帖**（作者自陈困惑）⇒ 只取一条可迁移动作：`mAuthToken` 每次变（时间戳/随机）⇒ 追 `getStore` 发现取值是**闭包私有对象 `t[e]`**，且 **`setStore` 断不到、`clearStore` 能断** ⇒ **改搜「哪里调用了 `setStore`」而不是死磕断点** |
| 760 | `52pojie-2005690-python实现蓝奏云文件夹内文件获取和文件直链获取.md` | `b88f7353600ad01a7385e7d4bf6fe873` | 2026-09-25 | `cloud-drive-direct-link` | evolve | ★★★ **速率硬约束（本族唯一「后果外溢」的门禁）**：**访问过频蓝奏会把「所有人」ban 掉** ⇒ 每页延时 ≥1 秒；★★ 但**列表接口与直链接口的限速不是同一套**（`GetFileListByData()` 没做限速 ⇒ 更危险）；★ 三函数契约 `GetFileListByUrl(url,pwd,pg=1)` / `GetAllFileListByUrl(url,pwd)` / `Get_final_link(_id)`；★ 四条边界：**嵌套文件夹不支持 / 无密码文件夹不支持 / HTTP 401 = 文件夹暂时不可访问（抛异常）/ 接口会变需跟改**（源文自己 2025.2.27 修第 2 页、2025.4.10 修 api 变更） |
| 761 | `52pojie-2022671-web逆向，遇到加密js。.md` | `a3eced20362a8eaa26647391b9977493` | 2026-09-25 | `web-reverse-algorithm` | skip | 已精读。内容为**片段帖**：仅指出「请求参数被拼成 `a=&b=&c=&` 再由 `b.send(m)` 前的加密处理」与混淆结构 `(function(_$cT,_$kd){…})($_ts.scj, $_ts.aebi)`。**`$_ts` 族已在 `web-js-env-patcher/references/ruishu-botgate.md` 等四份文档完整登记**（含 VMP 结构与 keynames 脚本）⇒ **不新增技能、不重复落盘**，按「避免冗余」登记为已处理 |
| 762 | `52pojie-2031450-《某游快爆》APP sign值逆向.md` | `17e9887409a819416ba541c494e0c26d` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 定位：抓包看到字段名带**业务语义**（`c=applaunch`）⇒ **直接搜该字符串**；★ native `Java_com_xmcy_hykb_data_Token_getToken(env, obj, ctx, jKeyArray, jValueArray)` —— ★★ **入参是两个平行数组（key[] / value[]）**；★★ 算法：把**加密的 key（`qlftg}`）/ value（`a8276l17g<d?2>=16j0?c=a>3h=2:?…`）先 XOR 解密**（`decodeChar(c,key)=c^key`，key 数组循环 4 个）⇒ 加入数组 ⇒ **按 key 的 ASCII 排序** ⇒ 用 `|` 拼 value ⇒ MD5；★ 明文为 `secret` / `c1714e41e5a907874c59a4d81a8486ea` |
| 763 | `52pojie-2047651-某海外市场运营商app登录协议逆向.md` | `58b212771a7e33941320ae73ae87ff4d` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **形态 B：会话密钥 + 公钥包裹** —— ① `substring = UUID.substring(0,24)` 会话密钥 ② `encrypt = 3DES(substring, payload)` ③ `encrypt2 = 3DES(substring, 毫秒ts)` ④ `key = Base64(RSA(pubKey, substring))` ⑤ 组包 `data=&key=&timestamp=&dataContentType=`；★★ 判据：**响应不加密（明文 `errorCode`）而请求全加密** ⇒ 只需单向还原；★ 手法：`r0trace.py` hook 堆栈定位；★ `objection` 调 `SplashScreenActivity` 跳过闪屏；★ `xapk` 改 zip 解压取 `com.*.apk`；★ `Base64.encodeToString(...,2)` = NO_WRAP |
| 764 | `52pojie-2051638-某东滑块验证逆向分析.md` | `078f65e78122f300cb63b8e91e9ca913` | 2026-09-25 | `web-js-env-patcher` | evolve | ★★ **滑块「假拟人」第二层：连延时都不随机** —— `path_plan(40,x)` 步进 40 + 末段余数（`[40,40,7]`）+ `move(x,10,1)` + `time.sleep(1)` ⇒ **走一步停一秒**；★★ **识别路线 B「去白底找缺口」**：读灰度图、取模板图**像素 < 200** 的横纵坐标最小 x（无 Canny，实现极简，但只适用高对比实心块）；★ 稳健做法：**用 xpath 把元素 `save()` 成图片**，不自己做像素裁剪（对比 B 站那篇的硬编码魔数） |
| 765 | `52pojie-2054564-多姿势某瓣app参数逆向分析.md` | `9a094f7c17c4698987d705ddf6fce481` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 与 `52pojie-1262453` **同一目标（豆瓣 `_sig`）的交叉验证对**：本文走**动态 hook**（`HMACHash1.a(str, str2)` 打印参数 + 返回值与抓包对比）⇒ 得 `key = bf7dddc7c9cfe6f7`，**与 1262453 的静态推导（APK 签名 → AES → key）完全一致** ⇒ ★★ **同一目标两种姿势（静态推导 vs 动态 hook）互为验收**；★ 判据：**`_sig` 末尾有 `=` ⇒ 先 base64 解码试**；★ 手法：**算法助手（LSPosed）日志里搜抓包参数值 → 取调用堆栈** |
| 766 | `52pojie-2060153-某8旗下安某某App登录参数分析.md` | `fae9cd86b3747e4c28dc82dd5f60671d` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **本批「多层混合 + 非标变体」最密的一篇**：① ★★ **控制字符前缀陷阱**：明文最前面有 **`U+0001`（SOH）**，**直接 hook `doEncrypt` 拿明文时看不到** ⇒ 排查很久；② 明文 = `json + 设备指纹模板串`（`%nettype/%deviceid/%thirdinfo/%preMobileMaskMd5/currentTimeMillis/randomUUID` 占位符 Replace 生成，分隔符 `f34988b='#'`）；③ so = `libcom_wuba_uc_rsa.so`，动态注册 → `JNI_OnLoad` → `off_C004` 即 `JNINativeMethod` 数组；④ ★★★ **RSA 分块加密**：每块 117 字节（=128-11，PKCS#1 v1.5 明文上限）、末尾拼 `0x01`、再 base64；★ `byte_len <= 585`（5×117）；⑤ ★★ **每块内「先 RSA 再 RC4」**，且 **RC4 是非标（S 盒长度 128，标准为 256）**；⑥ ★★ **base64 变体：`+→-`、`/→_`、`=`→`B`**；⑦ 随机数种子用时间戳 |
| 767 | `52pojie-2070898-某消费金融算法分析.md` | `491a1c7bcd98a29b44c17424de1b357c` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **形态 C + 全套工程手法**：请求体三段 `part1|part2|part3` = `RSA(key)` | `AES-CBC(data)` | `Base64(MD5(data))`；★★★ **判据「key/iv 随机 ⇒ 它必然要上传」**：`iv = key 的前 16 字节`（iv 是 key 的一半），因为 key 已经通过 part1 上传所以 iv 无需单独传 ⇒ **「发现 key/iv 随机，先找它的传递方式」**；★★ 手法集：① **so 加固视觉判据 = 正常 so 代码段在 IDA 里是蓝色、目标不是** ⇒ `dump_so.py` 一把梭；② `RegisterNatives` 定位；③ **IDA 反编译不理想 ⇒ 换 Binary Ninja**（BN 基址 `0x400000`：`bn 00446aa0` = `ida 46AA0`）；④ **主动调用**代替点击；⑤ `android_dlopen_ext` hook 控时机；⑥ ★★ **「RSA hook 根本不触发」的真因 = 结果被缓存**（`data_532b00` 保存启动后结果，用 flag 决定「重新生成 / 直接取」）⇒ **必须 hook 首次启动**；★ 响应体是 Java 层（先易后难）；★ `10001` = RSA 公钥指数，模数 = 主动调用传入的 module 参数 |
| 768 | `52pojie-2073103-某对app算法分析.md` | `852f7f8a35b9c18bdc99d03e1892b369` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **魔改 DES 全表识别 + unidbg 工程化**：① ★★ **判断 ECB vs CBC 的零成本实验**：改明文入参，看**相同明文块是否得到相同密文块**（一致=ECB）；② ★★ **魔改 DES 逐表判据**：`PC1` 表**长度 56 但数据完全不同** / `SHIFT` 表正常 / `PC2` 表被改 / ★★ **S 盒是标准的** / ★★★ **字节↔位转换「低位在前」（位序反转）**；③ ★★ **unidbg 补环境**：`clock_gettime` 需自行补、`lrand48`/`srand48` hook 固定随机数；④ ★★ **arm32 调用约定**：参数 R0-R3，超出压栈（小端），函数入口常把参数全存栈，返回值经 R0；⑤ ★★ **结构体返回**：`DesResult{ size, size_copy, data* }` ⇒ **前两个字段是长度、第三个才是数据指针**；⑥ ★ `sub_2010` 从固定串按下标取字符（`a1[i] = s[i*v3[i]-i+4]`，`v3=[0,0,2,0,5]`）；⑦ ★ 时机纪律：hook 没输出先怀疑**时机不够早**；清数据/重装看**调用顺序**（`init→setToken→getSign`）；⑧ ★ 重装清数据后仍固定的值 ⇒ **与设备有关，统一当已知数据**；⑨ ★ **反编译不好 ⇒ 换 IDA 版本**（换版本才看到交叉引用） |
| 769 | `52pojie-2073532-魔改md5样本算法分析.md` | `30b32f26bd3c910f2401417aaed0454b` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **「标准算法魔改」的逐层检查清单（可照抄）**：① ★★★ **初始化常量「数值没变、赋值顺序被换」**（ida `a=0x67452301,b=0x10325476,c=0x98BADCFE,d=0xEFCDAB89`）—— **最隐蔽的一类，只比数值会误判成「没魔改」**；② ★★ **按 F 函数的结构形状反推变量映射**（`(v9&v14|v12&~v9)` ≡ `((b&c)|(~b&d))` ⇒ `v9=b,v14=c,v12=d`，剩下是 a）；③ k 表被改（`0xd76aa478` → `0x8A51407D`）；④ ★★ **`FF` 宏末尾加数从 `b` 换成 `d`**（`md5_a = md5_d + v15`）—— 第一轮对不上的**真因**；⑤ ★★ **`__ROR4__(x,25)` ≡ `ROTATE_LEFT(x,7)`**（C 无循环左移，编译成右移 32-7）⇒ **别因为看到 `ROR` 就以为移位被改了**；★ 动作顺序：unidbg 跑通（本样本**不需要补环境**）→ 备一份标准源码逐轮 diff → 每定位一处就在本地同步改并**与 unidbg hook 值对拍**（实测改完 k 表仍为 `559a829a` vs 本地 `75ff2b87` ⇒ 还有魔改点）；★ 读指针按类型换算字节偏移（`*(v10-3)` ⇒ 减 12 字节） |
| 770 | `52pojie-2073893-de到app算法分析.md` | `498c3c9d65e8ec97265e0cf7a1a40a02` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **HMAC-SHA1 的内存指纹判据**：内存里出现**大片 `0x36`（ipad）与 `0x5C`（opad）** ⇒ 先往 HMAC 上靠，再用 SHA1 常量搜索佐证（★ 另一路 `FindCrypt` 插件）；★★ 手法：**`hook memcpy` 是「分析前的好习惯」**，用 `emulator.traceWrite(addr, addr+0x20)` 做内存写监控追溯来源，读日志**小端、4 字节一组、从右往左**；★★ **`std::string` 布局判据**（`0x31` 标志 + `0x28` 大小 + 地址 ⇒ **读第 3 个字段**）；★★ **初始化依赖**：不调 `initImp(ctx, str)` 结果就是不对；★★ **key 派生**：`key = table[(~byte) & 0xFF]`（**按位取反后查 256 字节固定表**）；★ 定位：**hook `HashMap.put` 打堆栈**（与 B37 换靶同族）；★ HMAC 拼接：参数按序拼接、**`secret` 拼在最后**，空串参数要**保留换行**；★ 判据：**jadx 版本是变量**（1.4 反编译失败且定位到错算法 ⇒ 1.5+ 才对） |
| 771 | `52pojie-2074694-他x星qiu算法分析.md` | `6840e96382acfafcd13e45a9807477bd` | 2026-09-25 | `android-app-reverse` | evolve | ★ **同一 App 并存两套算法**（不同接口不同 native 方法：`drj1`/`drjbpsign`/`drjcf1`/`drp0`/`drp1`/`ck`）；★ 定位：**hook `HashMap.put` 打堆栈** → `AppGlobalHttpRequestManager.b`；★★ 实证：**同一 jadx 搜到的匹配是「另一个算法」的位置**，`b` 方法反编译失败 ⇒ **多备几个 jadx 版本**（与 2073103 换 IDA 版本、1572670 换 Procyon 引擎同族）；★ `DefenseTower.epj1(str, str2)` 每次结果不同 ⇒ 有随机数参与 |
| 772 | `52pojie-2094241-flutter逆向-某影视app协议sign加密.md` | `8f1fc1b075b0d7a82e842c6ffee6072d` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **split APK 判据**：Google Play 下发的 App 是分包，`base.apk`（核心代码/资源）+ `split_config.arm64_v8a.apk`（**只含对应 abi 的 so**）+ `split_config.en.apk` + `split_config.xxhdpi.apk` ⇒ **「`lib/` 里一个 so 都没有」的真因是分包**，从 `split_config.<abi>.apk` 取 so；★ `blutter.py` 反编译 Flutter（`asm/` 出 dart 代码）+ **用 blutter 生成的 IDA 脚本解析 so 才能看到函数名**；★ 搜到 `RSA` ⇒ hook 拿密钥 |
| 773 | `52pojie-2100363-某卡Flutter请求响应加密分析.md` | `aecd88b87723dea7e6e98620a5676e6e` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **「Flutter 静态反编译失败」的兜底范式（本批最值钱的手法之一）**：blutter 与 reFlutter **都失败** ⇒ ① **密文长度特征判据表**（AES 16 倍数 / DES 8 倍数 / RSA 固定=密钥长度；★ **16 字节 AES 与 DES 都可能，不能只看 16 字节**；★★ **同时满足两者时优先 AES**，现代接口约 95% 用 AES；Base64 长度表 16→24/32→44/48→64/64→88、128→172/256→344/512→684）；② ★★★ **「同类型算法样本 + 机器码特征码比对」**：找一个**能反编译的、用同一套加密库的 Flutter App 作替身**，hook 它的加密函数提取**机器码序列**，再回目标 `libapp.so` 里 `Memory.scan` ⇒ **绕过函数名混淆定位入口**（提取动作：IDA `Options→General→Number of opcode bytes (non-graph) = 4`）；★ Flutter **必须同时 hook SSL**；★★ 判据：**同一函数第一次调用=加密请求载荷（有明文入参、返回空）、第二次调用=解密响应（有返回是明文）**，靠一个模式参数（`arg5`）分支；★ key/iv 硬编码在内存可读；★ 日志出现 `PaddedBlockCipher.AES/CBC/PKCS7` 特征串可确认算法族 |
| 774 | `52pojie-2107248-[原创]某视频APP七神签名之X-Gorgon生成逻辑逆向分析——古法逆向与AI辅助结合.md` | `883b877bb10b8d7f7fa79177d24dd2b2` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **「逐字节溯源」范式的完整演示（七神签名）**：① ★★ **从出口倒着走**：hook 落地点 `memcpy` 拿源地址 → `emulator.traceWrite(addr, addr+len)` → 从写入日志反推「谁写了哪几个字节」→ 定位写指令 → 逐层套娃；② ★★★ **「逐字节不是一次性写入」是常态**：26 字节 = 前 4(`84 04 40 CA`) + 2(`00 00`) + 后 20；20 字节 = 8 + 12；前 8 = `MD5(url)[0:4]` + 请求头 `x-ss-stub[0:4]`；后 12 = 4(固定 0) + 4(**初始化 `strdup` 写进去的版本号**) + 4(**秒级时间戳**，`1747031282` → `68 21 94 F2`)；③ ★★ **跨 so 间接调用要读寄存器**：`libsscronet.so` 的 `v73` 是 `BLR X23` ⇒ hook 该指令读 `this.context.x23` 才知目标是 `libmetasec_ml.so!sub_14DBF4`；④ ★ **判定层**：`sub_109FB8` 是**最终 hex 构造层**（解释「写一个字节就写一个 0」）、`sub_77C90` 是 byte→hex 编码层 ⇒ **都不是算法层，要继续往上**；⑤ ★★ **魔改 RC4**：标准 RC4 加解密对称，但这里**对输出字节再做一轮变换** ⇒ 标准 RC4 解不出，必须复现第二轮变换做**反变换**；⑥ ★ 固定输出 `840440ca0000900e2e6316640c563d305547aba28ae11cf6f18e`；★ 七神中**除美杜莎外全固定**；★ 花指令可由 AI 识别；⑦ ★★ **把 1w+ 行 trace 日志丢给 AI（IDA Pro MCP + codex）分析** |
| 775 | `52pojie-2107752-某饰品交易平台订单创建协议逆向.md` | `d462b6a51f5fdc1bb23b20477fc67cd1` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **形态 D + RPC 化路线三**：① `sk = RSA(服务端公钥, 随机 AES key)` 放请求头，body = `AES(明文, 该 key)`，响应 = `AES(RSA⁻¹(sk), 密文)` ⇒ ★★ **「加密用服务端公钥、解密用客户端私钥」**（`sg.a.h()`=公钥、`sg.a.g()`=客户端私钥，`MIIBIj` 开头是 X.509 公钥固定标识；`cipher.init(1,…)`=加密、`2`=解密）；② ★★ **白名单 + 远程开关**：`bVar.d().contains(path)`（敏感接口才加密）+ 远程配置 `k_api_use_encrypt`；③ ★ `requestTag = MD5(deviceId:毫秒ts).upper()`；④ ★★★ **`signature` 用「排序后的 body」而加密的 body 未排序** ⇒ 服务端解密后还要**验签**（6 参数：deviceId/系统版本/平台/path/排序后 body/秒级 ts）；⑤ ★★ **native 混淆 + 运行时动态解密，AI dump 无成果 ⇒ unidbg 挂成 HTTP 服务供 Python 调用**（与 1964051 Xposed RPC 同族）；⑥ ★ 定位：**算法助手搜密文 → 直接看到调用堆栈**；★ 抓 4 次包做「同异对比」：固定值硬编码、变化值逆逻辑；★ frida 可注入 ⇒ 直接 hook AES 函数拿明文 body |
| 776 | `52pojie-2120549-极域 V6.0 连续投屏与远程操控协议逆向.md` | `0f30b898fb8241cb5c0930fedf039662` | 2026-09-25 | `protocol-reverse` | evolve | ★★★ **协议层三条判据**：① ★★★ **FourCC 在反汇编里是「小端立即数」**：wire `53 48 43 4F`=`SHCO`，IDA 里是 `0x4F434853` ⇒ **只在 Strings 窗口搜 `SHCO` 很可能什么也找不到，必须同时搜立即数再看交叉引用**（锚点表：`0x4F434853`SHCO / `0x43504B54`TKPC / `0x46524848`HHRF(H.264) / `0x46524A48`HJRF(JPEG) / `0x2321347C` 输入分发器）；② ★★ **流缓冲区重组：不要依赖 TCP segment 边界**（一次 `recv` 可能半个/多个包 ⇒ 攒够 `12 + payload_len` 才取一包；UMSP 头 = 版本 + magic + payload length = 12 字节）；③ ★★ **单变量差分定字段 + 「字段命名按证据逐步升级」**（`unknown_08 → changes_with_x → normalized_x → normalized_x_u16_range_in_u32`），★★ **不要因为值等于像素就命名 `pixel_x` —— 实际是 `0..65535` 归一化**；★ 三层通道（`5512/UDP` MESS feature=8 → `4806/TCP` SHCO channel 10 + TKPC → `COMD`/`MCMD` mode=2）；★ **必须跟到系统调用边界**（看到 `recvfrom` ≠ 控制生效，要确认 `INPUT`/`SendInput`）；★ 陷阱：参数块里的端口 ≠ 学生端服务端口（混用会构造「长度正确但状态错误」的请求）；★ 陷阱：从 view 切 control 重启 H.264 解码器易永久黑屏（新解码器没收到 SPS/PPS/IDR） |
| 777 | `52pojie-2121591-一次 Android 逆向记录(DY).md` | `6bf3e990ffaebdd4a11cc7f18c8d9b57` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **本批方法论最重的一篇**：① ★★★ **第一轮静态只做三件事**（Manifest + DEX 清单 + Native 清单）——「第一次导出完整 JADX 后搜 AES/RSA/MD5 出来一片结果，那次搜索给了我很多文件，几乎没给我结论」；② ★★★ **库级哈希差分做版本 delta**：脚本只读 APK 内 `lib/arm64-v8a/*.so` 算 sha256 对比 ⇒ **216 相同 / 58 变化 / 无增删** ⇒ 相同的库**旧版结论可低成本回归**；★★ **体积/哈希变了的库（`libmetasec_ml.so` 5,657,800→4,505,440）旧函数地址继续 Hook 没有意义，甚至可能打到完全无关的代码** ⇒ 标 `rebaseline_required`，**不因为旧版做完就顺手写「已验证」**；③ ★★★ **「字段名 ≠ 结论」**：`user_count`/`getAudienceCount()`/`total_user`/`display_value`/`display_type` 一堆相似字段 ⇒ 结论必须来自**消费点**（搜 getter 用法 + UI 文案）；★★ **反例比相等记录更有用**（`当前在线 28 / 累计用户 5998` 直接证明 `total_user` 不是在线人数的拷贝）；`display_type` 六值语义表；★ `room_auth` 129 个具名字段规则不统一 ⇒ **只写有消费点的 29 条，不强行翻译**；④ ★★★ **Presence 记录解决「Java 默认值误判」**（`false/0` 可能是服务端下发、也可能是 Gson 默认值）⇒ 四态：**响应确认 / 对象默认值 / 跨场景补齐 / 本次未下发**；同实体多场景按 id 对齐做**字段级并集**，冲突保留来源不静默覆盖；⑤ ★★ **探针先脱敏**（敏感值在探针里就替换为 `{kind,length,sha256_12}`；★ 短指纹**只用于判断是否同一形态，不能重放**；URL 只留结构、query value 不进轨迹；反射遍历上限 4~6 层/集合 ≤80/对象 ≤80~180 字段）；⑥ ★★ **日志先定格式再采**（一行一个 JSON 事件 + `seq` + `at_ms` + `schema`）；★ 房间对象有缓存副本 ⇒ **采样只承认与前台 `current_room_id` 对齐的当前对象**；⑦ ★★ **Native 分析进度阶梯**：`库存在 → 进程加载 → Java/JNI 入口 → 真实调用命中 → 输入输出形态 → 中间阶段 → 离线固定向量`；★★ 「看到 AES/RSA/SHA 字符串 ≠ 发现自研算法」「so 出现在 maps ≠ 目标函数执行过」；★ 拆阶段 hook + 离线回归**只放合成数据**；⑧ ★★ **「客户端能读 ≠ 客户端能生成」**（播放授权/临时 URL Token 由服务端生成） |
| 778 | `52pojie-2122819-从 RegisterNatives 到逐字节复现：一次 Android ttEncrypt 封包逆向.md` | `2113aa52fd7a2b4484e82e7ef4f87b41` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **本批「native 还原」最完整的一篇，四条判据全部可复跑**：① ★★★ **先看输出形状，不要先读算法**：喂 1/3/15/16/17/31/32/1024 字节看输出（118/118/118/134/134/134/150/1142）⇒ **每跨 16 字节边界 +16 ⇒ 像带 PKCS#7 的分组密码**；★ 但**固定开销不是简单 16 字节 IV** ⇒ 并排对比得结构 `6 字节固定(74 63 05 10 00 00) + 32 字节随机材料 + 16 对齐密文`，`cipher_len = out - 6 - 32`；② ★★★ **「连续输出相同」= 缓存 还是 随机数？两个对照实验**：㈠ **同一秒内调用不同输入** ⇒ 32 字节区域相同、密文不同 ⇒ **排除完整结果缓存**（只是复用了同一批随机材料）；㈡ 固定输入**间隔 1150ms** ⇒ 32 字节区域全不同 ⇒ 归因 `srand(time(NULL))` + 32 次 `rand() & 0xff`，**种子只有秒级精度** ⇒ ★★ 金句「真正让我警觉的不是 `rand()` 本身，而是它**每次调用都按当前秒重新播种**」；③ ★★★ **「分段 Hook 代替整函数硬读」**：只 hook **三类边界**（随机字节生成 / SHA-512 / 分组加密封装）⇒ 一次调用打印出**主链骨架** `RANDOM 32 → SHA512 32 → SHA512 128 → SHA512 3 → CIPHER key16 plaintext67 cipher80 → FINAL 118`，**6 行日志暴露整个算法**；④ ★★★ **完整封包（逐字节可复现）**：`R=PRNG(32)`；`H1=SHA512(R)`；`H2=SHA512(H1‖CONST64)`；`KEY=H2[0:16]`、`IV=H2[16:32]`；`P=SHA512(M)‖M`（**摘要拼在消息前面**）；`C=AES-128-CBC-PKCS7(KEY,IV,P)`；输出 = 固定 6 字节 + R(32) + C；⑤ ★★★ **「不是看到 AES 字符串，而是三种证据互相吻合」**：Key 长度+轮数符合 AES-128 / 静态分组关系符合 CBC / **OpenSSL 独立计算逐字节相同**（`openssl enc -aes-128-cbc -K … -iv … -nosalt`，**不加 `-nopad`** 让它 PKCS#7）；⑥ ★★ **导出表找不到 `Java_..._ttEncrypt` ⇒ 两种可能（名字被隐藏 / 动态注册）**，从 `JNI_OnLoad` 找到**定长 XOR 循环（0x30 字节 × `0x73`）**解出类名 ⇒ 再读注册表得 `name=ttEncrypt, signature=([BI)[B`；★★ 「从 `RegisterNatives` 反推入口，比在大段 ARM64 里猜哪个函数像加密可靠得多」；⑦ ★ 三次 SHA-512 分工明确（随机材料预处理 / 派生 Key·IV / 消息完整性摘要）；★ 请求体路径有 102,400 字节上限 |
| 779 | `52pojie-2127692-某知名海外社交 Android 签名链AI逆向分析全流程.md` | `23363cace9afea0fa3ca366b9c147728` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **「修正 / 自我否定」纪律（本批最值钱的元方法）**：① ★★★ **「重要修正」**：`frameSign()` 是明确存在的帧签名 API，但**没有证据证明普通 `/aweme/v1/search/item/` HTTP 请求直接调用它** ⇒ **不要把 `frameSign()` 等同于 HTTP 的 `x-gorgon/x-argus/x-ladon` 生成入口**；② ★★★ **Cronet OpaqueData 修正**：导出了 `Cronet_ClientOpaqueData_do_sign_set/get`，但 `ClientOpaqueData` 同时含 host list / certificate / private key / `do_sign` callback ⇒ **更符合 TLS 客户端私钥签名回调，不应认定为 HTTP 签名通道**；且 `getClientOpaqueData()` 返回 `null` ⇒ **不能当作已验证的 MetaSec 注册路径**；③ ★★ **`.rodata` 字符串 → 虚拟地址换算**（`.rodata` 起点 `0x76e90` + 偏移 ⇒ `x-argus 0x7e2f9` 等），★★ **这些地址严格绑定当前版本，升级后不得复用**；④ ★★ **请求头职责表**：`x-ss-stub`=请求体摘要 / `x-khronos`=秒级时间 / `x-ss-req-ticket`=毫秒级 / `x-gorgon`=URL+body摘要+时间+会话签名 / `x-argus`=设备+App+环境+完整性 token / `x-ladon`=时间+App+设备上下文；★★ **不要假设只更新 `x-khronos` 就能重放修改后的请求 —— 签名字段之间存在关联**；⑤ ★★ **Frida 负结论落盘**：`17.9.1`/`17.6.1` × spawn/attach × 空脚本**全部 `SIGABRT`**（`Unsupported Android linker`/`libnpth.so`）⇒ **不要在相同设备与版本上重复 Frida 方案，除非环境已经变化**；⑥ ★★ **Java 签名接口链**（`ISecApi.frameSign → SecApiImpl → DmtSec → C11W3 → ms.bd.o.g2.frameSign → ms.bd.o.k.a(…)`），★ native 调用形态 `k.a(33554442, mode, nativeHandle, canonicalRequest, null)`；★★ **返回值是「交替排列的字符串数组」`[headerName0, headerValue0, …]` 再转 Map**；⑦ ★★ **RPC 化前置条件清单**（`Context` 已建立 / 正确 `ClassLoader` / so 已加载 / native handle 已初始化 / deviceID·installID·settings·会话已同步）；★★ **伦理边界：不应实现「接受任意 URL/body 返回签名的通用 Oracle」**；★ 证据：`x-metasec-bypass-ttnet-features` / `-mssdk` / `-api-log` 是「签名位于 TTNet 流水线」的直接证据 |

---

## 批次 B42 · 2026-09-25（第四十三次执行）

**取材口径**：**归档线第 42 轮新收 48 篇全取（48/48 命中）** —— 承接上一批「归档线第 42 轮」的优先级 ①。
该轮归档自述：「六条向内通道全部结项，唯一在产 = 对全量池做**口径重判**」；R41 扩了"类目"，本轮补"词形"（同义表达盲区：
加密算法分析 / 协议加密 / 接口加解密 / 参数校验 / 风控 JS）⇒ 供给 48 篇，**App / 接口层签名与参数 36 + 协议链路 10 + Web/JS 2**。
簇：客户端签名与参数（36，其中 **iOS 4 篇 / 双端 1 篇**）· 二进制与自定义协议（8）· WS / 服务端审计（2）· 前端 JS（2）。

### 本批次登记表

| # | 文件 | md5 | 处理时间 | 关联技能 | 变更类型 | 核心萃取 |
| --- | --- | --- | --- | --- | --- | --- |
| 780 | `52pojie-1003401-记录一次androidStudio动态调试破解加密参数的过程！！.md` | `2a21b78dbac5e276fb0be8716fb7b041` | 2026-09-25 | `android-app-reverse` | evolve | ★ `requestId` + `rqid` 双参数：先搜字符串对照 java，再**只把加密相关 smali 拷进 Android Studio 工程**做动态断点调试（`adb shell am start -D -n 包名/入口`）；★★ **判据：同一字符串在多个 smali 里出现 ⇒ 每个位置都打断点，看程序实际走哪一个**；★ 「反编译读不懂就单步看寄存器」是可迁移的兜底动作 ⇒ 已并入 `01-recon-and-carriers.md` §1.3 |
| 781 | `52pojie-1010585-某企业办公APP逆向TCP协议分析.md` | `b7a041227d153332404bf2bb8f281a9c` | 2026-09-25 | `protocol-reverse` | evolve | ★ 自定义 TCP 包结构 `长度[2] + cmd[2] + 固定字段… + 长度[2] + hex 载荷`；★★ **「先看日志」**（App 残留日志直接给出 `LoginOp(genLogin) Cipher rsa DECRYPT_MODE begin` 这类函数名与阶段）；★ 定位链：日志 → 类 `CCProtocolHandler` / `com.duoyiCC2.protocol.dp` → JNI 类 `CCJNI`（5 个 native）= 加解密关键处；★ `KeyPairGenerator(512)` 生成密钥对，私钥解服务端下发的会话 key ⇒ 后续走置换加密 |
| 782 | `52pojie-1104122-数某风控JS算法分析.md` | `8012f633664f446a1298939e8fb60b74` | 2026-09-25 | `ast-deobfuscation` | evolve | ★★★ **评分型风控 JS（数美 `fpv2.js`）的完整结构**：① 字符串数组元素是 **base64**；② **数组移位步长 = 传入参数 + 1**（`_0x1f958f(++_0xb5)` ⇒ 下标整体错位 182）；③ ★★★ **第三段改写了原生 `atob`**（自写 base64 + `decodeURIComponent`）⇒ 「自己解出来对、和浏览器不一致」的真因；④ `switch` 平坦化的 **`runLine` 字符串就是执行顺序**；⑤ **运算符字典**（value 可为函数、且**可嵌套**）⇒ 用 esprima 递归替换、复现时操作数要加括号、**多行函数体直接跳过**；⑥ ★★★ **`arguments` 被原地反转**（先逐串反转、再整数组反转）⇒ **参数顺序不能按声明读，只能在改写后断点抄值**；⑦ 异常流控（终值在 `catch` 里拼）、**执行耗时检测**、关键数据**多处缓存**（`smidV2`）。⇒ 新增 `references/patterns/shumei.md` + `ob-variant-taxonomy.md` §十 |
| 783 | `52pojie-1138081-APP网络请求参数校验,逆向分析分享.md` | `ec9be869c6728c33b49e932ea482c9a4` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **`keystr` 实名认证签名（本批「参数预处理」最全的一例）**：`姓首字母 + 手机号按位查表(两张不同表) + "_" + (pid+6) + 手机号按位查表2 + 身份证三段子串 + 姓名首字母` → `MD5` → 拼 `pjjsdsb` → **再 MD5 一次**；★★ **中文首字母是 `GB2312` 双字节 → 27 项区间阈值表查 A..Z**（不是 Unicode 映射），且那张 `int[27]` **必须程序抽取**；★ 载体：ReactNative+NodeJS ⇒ `assets/index.android.bundle` 改后缀当 js 读；★ `HeHe.getKey` 在 JS 里搜不到 ⇒ 去 Java 层找同名全局对象 |
| 784 | `52pojie-1139448-APP网络请求参数校验Sign,逆向分析分享,第2饭.md` | `485816761d4c23da377c654adcbcbb6b` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 签名形态 8：`ParamsSortUtils.formatUrlMap(map, 参数2, 参数3)` 排序拼接（**尾部多一个 `&` 要删**）→ 拼 `v2020global` → MD5；★★ **两个开关的语义**：参数2 = 是否对值做 URL 编码（并把 `+`→`%20`、`%3A`→`:`），参数3 = key 是否转小写；★ 已登录时自动追加 `token` / `userSn` 参与签名 ⇒ 已并入 `03` §1 形态 8 与 §1.1 |
| 785 | `52pojie-1140214-APP逆向分析分享,参数加密,第3饭.md` | `1c9b321c3e3b474ee36e48e43a9983c4` | 2026-09-25 | `android-app-reverse` | evolve | ★★ **「数据藏在协议头里」+「分段加密」两个反常规形态**：① POST `Content-Length: 0`，真正的数据在 **header 的 `token` 字段**（值是**数组形态** `[内容,内容]`）；② `fenduan(n)` **每 80 位切一段**分别加密后 push 进数组 ⇒ 复现时必须按段拼；★ 载体同 1138081（RN + `index.android.bundle`）；★ 本地同时存在 **公钥与私钥** ⇒ **有私钥就能自己解密**（判据：正常只需公钥，见私钥 ⇒ 直接用来解） |
| 786 | `52pojie-1163182-对某猫安卓端数据接口加密方式的简单分析.md` | `70de4e0a76fd0f31f4503ace19cea716` | 2026-09-25 | `android-app-reverse` | evolve | ★ 形态 17：`AES/CBC/PKCS5Padding` + `toHexString`；★ 手法：AndroidKiller 搜**接口方法名**（`getData`）→ 进赋值处 → 找 `AesEncryptionUtil` → key/iv 以两个字符串常量直传；★ 判据：**网页端与 App 端加密方式不同**（拿网页的解密工具解 App 拿到的数据失败 ⇒ 要另外定位）⇒ 已并入 `03` §1 形态 17 |
| 787 | `52pojie-1315172-极略三国请求签名算法解析.md` | `b1c41f1f2a35e7753917094fec104e92` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **「请求与响应各一个盐」实证**：`sign` = 按 key 字典序排序 → `urlEncodedString` → 拼 secret → MD5；**response 段用另一个不同的 secret**（源文只给出一半）；★ iOS 侧定位（伪码里能看到 `objc_msgSend` + `countByEnumeratingWithState` 的字典遍历）；★ 源文自陈「生成的码与原本的码基本不一样，求大佬解惑」⇒ **未完成帖**，本库只取「双盐」这一条可迁移判据（已并入 `06-ios-app-reverse.md` §7） |
| 788 | `52pojie-1330727-关于xx瑜伽的sign加密破解.md` | `6eb412888c75085bc2dffba2c6c90e06` | 2026-09-25 | `android-app-reverse` | evolve | ★ 给出了**完整的签名拦截器源码**（`CustomSignInterceptor`：`orderHashMapByOldMap` 按 key 排序 → `urlEncodedString` → 拼 secret → MD5；另有 `formatOffset` / `getTimeZone` 参与某参数）；★ 判据：**`SIGNKEY` / `SIGNKEYVALUE` 成对常量 ⇒ 搜 `SIGNKEY` 可直接命中拦截器**；★★ 源文还提到 **response 用另一个盐**（同族「双盐」）；⚠️ **未完成帖**（作者自陈「生成的码与原本的码基本不一样，请问有大佬可以解惑吗」）⇒ 本库只取「排序拼接 + 成对常量 + 双盐」三条可迁移判据，**不据它给出可用公式** |
| 789 | `52pojie-1352135-实战某东APP submit 过程中的 jwData 加密 并附java简单算法实现.md` | `e385828aa18bd8dfc5317961f307bbe6` | 2026-09-25 | `android-app-reverse` | evolve | ★ 形态 1.4：`DESede/CBC/PKCS5Padding` + `IV_BYTES = {0×8}`（全零 IV）；★★ **明文拼法先定死**：`lng + "_" + lat + "_" + networkType + "_null"` ⇒ 算法对但明文拼错会一直以为算法错；★ 附可直接复现的 Java `encrypt()` 骨架（`DESedeKeySpec` + `IvParameterSpec`） |
| 790 | `52pojie-1396064-关于某仙网抢茅台的协议分析和源代码.md` | `421f9c9ba8fc0b307acf13ee122aaca5` | 2026-09-25 | `android-app-reverse` | evolve | ★ 形态 9：一次性 token `reservationToken = MD5("ANDROID" + appKey + DEVICE_ID)`，★ 硬编码串 `"ANDROID"`；★★ **iOS 与 Android 共用同一套算法**（源文原话）；★ 接口链路：登录（返 token）→ settlement（加购物车）→ addorder（提交）；★ 判据：`appKey` / `deviceIdentify` **抓到啥就用啥**（不必自己推） |
| 791 | `52pojie-1397031-关于某仙网抢茅台的协议分析和源代码（续）.md` | `59d7167f9f375028068c12a3065c4d47` | 2026-09-25 | `android-app-reverse` | evolve | ★★ **「抢购类协议的时序纪律」**：把「登录」安排在开抢前几秒内做（`deadDate - now > 20s` 就 sleep），再**循环轮询 settlement** 直到 `unableProducts` 为空，然后**并发风暴提交 order**；★ 判据：`(settlementUrl + token).replace("APPKEYXXX", …).replace("PUSHTOKENXXX", …)` ⇒ **URL 是模板 + 运行时替换**，不是逐个拼参数 |
| 792 | `52pojie-1452996-[白嫖大法]某影视邀请协议分析.md` | `d27884fb0d73cb4e34c7932bba0c764b` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 形态 10：`MD5(固定串 + deviceId + 毫秒ts)` → `.upper()`；★★★ **那个「固定串」是 DES 解密出来的产物**（`e.decode(ConstantUtils.a)`）⇒ **在算法助手里直接搜它搜得到但会误判成「加密结果」**，正解是**注入打印验证它是常量**；★ 工作流：黄鸟抓包 → 算法助手搜 sign 值在**堆栈**里定位 `AppUtils.md5` → MT 注入验证 → 写协议；★ `deviceId` 可用 16 位随机 hex 代替 |
| 793 | `52pojie-1471424-某生鲜电商mfsig加密算法解析.md` | `905777fbb454ba84eb49c3871d9f816d` | 2026-09-25 | `miniprogram-reverse` | evolve | ★★★ **「参数名与值前缀都搜不到」⇒ 该函数被针对性混淆，立刻转动态调试**（源文原话「算是一个安全性比较好的 case」）；★ 动态路线：小程序开发者工具勾「不校验合法域名/web-view/TLS/HTTPS」→ 编译修 Console 报错 → 目标接口断点 → 代码挤在一行要按 `{}` 格式化 → **用纸记录流程** → 翻译并**拿真实请求体对拍**；★★ 算法性质是**索引编码游戏**（字符串↔数组来回转），**不是「标准算法+盐」**；★ 分包反编译顺序：**先主包，子包用 `-s=<主包目录>` 合并**（单包 4M 上限 ⇒ 大工程必然分包）⇒ 已并入 `miniprogram-reverse/references/request-crypto-and-sign.md` §6B |
| 794 | `52pojie-1512064-快手_NS_sig3参数算法分析研究学习过程.md` | `bae4ed6ff8b1d6c42034ff42e13a7175` | 2026-09-25 | `reverse-knowledge` | evolve | ★ **新建蓝图 `kuaishou-ns-sig3`（status=partial）**：入口 `JNI_OnLoad → RegisterNatives(sub_88F4) → doCommandNative = sub_c060`，调用链 `sub_c060 → sub_26BB8 → sub_3AE54 → sub_3AAF8 → sub_3F920 → sub_3E550`；★★ `sub_3E550` 是 **sha256 的变形**（常量表 `dword_9e338`）且**被调用两次**（一次是被加密内容、一次是 base64 值）；★★ 尾部经 `sub_3FDA4` 后 **base64 两次**；★ 前半段与**当前时间戳异或一个值**后加密（`sub_24404`）；★★★ **两处常量「读取图片数据后加密获得」，同版本内不变 ⇒ 直接 dump，不追生成链**；★ 手法：ollvm 下**逐层 frida hook 收敛** + **unicorn trace 比 ida trace 好读** |
| 795 | `52pojie-1530981-某社区登录协议分析.md` | `990f5b4bc1e81f79277cc2474c72e389` | 2026-09-25 | `android-app-reverse` | evolve | ★★ **XXTEA 完整实例（本库首次）**：`params = base64(XXTEA(json, key))`；判据常量 `DELTA = -1640531527`、`UByte.MAX_VALUE = -1`、轮数 `52/(n+1)+6`、`MX(...)`；`fixKey` 把 key **补/截到 16 字节**；★★ **密文长度 = `4*(ceil(明文/4)+1)`** ⇒ 形状「4 的倍数但**不是** 8 的倍数」（24→28、32→36、51→56）已机械化进 `app_cipher_shape.py`；★ 手法：Frida hook `HttpUtils.post(a,map,c,d)` **遍历 map 打印键值** ⇒ 一次拿到全部入参 |
| 796 | `52pojie-1537322-ppp买菜IOS版设备风控浅析与算法还原.md` | `49e1b7da194749cb510951cb52c1aaf6` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **本批最重的一篇（iOS 设备风控 SDK 全链路）**：① 混淆三种（反 F5 模板 `BL sub_xxx` 动态写 X30、字符串 XOR 解密、`ADR+LDRSW+BR` 跳转表）；② ★★★ **加密资源 PIC 当「密钥字典」**：`HMAC-SHA256(bundleId + 常量串 + Info.plist ss)` → 取前 16 字节 → ★★ **逐字节查表并清最低位**（`& 0xFE`，不是密码算法）→ `AES-CBC(IV=0102030405060708)` → zlib → JSON 里的 `k1..k6` 各管一段；③ `a0` 字段要**解两次**（先用 appkey 解，再分两组）；④ 请求体 = `POST <path> <query>` + 签名 JSON 拼接后再算 hmac；⑤ 风控采集面 `m1..m313`（**结果里再嵌一层逐项结果表**）；⑥ 判据 ★★★「**不校验值对不对，而校验某方法的实现来自哪个镜像**」⇒ 已新增 `06-ios-app-reverse.md` 全套（§1/§2/§3/§5/§6/§7/§9） |
| 797 | `52pojie-1547120-某大厂生鲜超市加密协议分析.md` | `9684d6c2d5b804c7c29c1d493d5f5760` | 2026-09-25 | `android-app-reverse` | evolve | ★ 形态 15：`sign` 是 64 位 hex ⇒ 先猜 SHA256，jadx 搜 `HmacSha256` 命中 ⇒ 实为 **HMAC-SHA256**；★ 同站另有 `eu` / `fv` 两个加密参数需一并还原；★★ 接口有 **5 分钟时效**（服务端检测 `t`），过期返回 `invalid signature`；★ 全部接口共用同一 URL，靠 body 里的 `functionId` 分流 ⇒ 定位要按 `functionId` 而不是按 URL |
| 798 | `52pojie-1555004-加菲猫 邀请协议-算法分析.md` | `48394ae0297ca01da0aa2da4213478a7` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 形态 13：`ns = MD5(包名 + <APK 签名 SHA256> + "&z4Y!s!2br" + 时间戳)`；★★ **`<APK 签名 SHA256>` 静态分析不出来 ⇒ 动态断点读寄存器**；★ 判据更正：`old_key` **不是固定值**而是**阿里 UTDID**（每设备不同，实现在 `com.ta.utdid2.device.c.h`）；★ 响应/请求的 `request_key`/`response_key` 是 `AES-CBC + HEX 编码`的 JSON（源文用 AES/CBC/PKCS7 亦可解）；★ `token`/`token_id` 来自登录响应 |
| 799 | `52pojie-1622354-某APP登录算法分析.md` | `7968990036ba9df63f8ac93e86437f5d` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **形态 1.4：`RSA/NoPadding` 的可跑 Python 复现骨架**（`zfill 到模长` → `bytes2int` → `encrypt_int` → `int2bytes(kLen)` → **hex 大写**；★ **`PKCS1_v1_5` 在这里不适用**）；★★ **`pk`（native `cc::d`）与 `t2`（`cc::e`）都是 `AES-256-CBC`**，且 **key/iv 硬编码在 so 的字符串常量里**（`bdeaed243193ce11ac913bbd48d340a4` / iv `ac913bbd48d340a4`）；★★ **key 参数是「多个常量 + 时间戳」派生**：`MD5(AppId + AppKey + 版本 + str(clienttime_ms).toLowerCase())`；★ `params` 的 AES key 由 `KeyGenerator(「AES」).init(128|64)` 生成后 **MD5 再前 32/16..32 切分**（key 与 iv 来自同一个 32 位串的两个切片）；★ 手法：`objection hook` 打印**入参 + 原生堆栈**一次拿全调用链 |
| 800 | `52pojie-1644568-某听书请求签名分析.md` | `1d5c1ea5ef5314f430e84da299e594dd` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 形态 11：`sc = MD5(path + "?" + Uri.decode(排序后 rawQuery) + 盐)`（`TreeMap` 重排）；★★★ **最高性价比动作：一开始就 hook `java.security.MessageDigest` 的全部 `update`/`digest` 重载 + 打堆栈**（源文原话「可以一开始就 hook md5 就可以秒杀了」），**再加挂 Base64/AES/RSA/SHA1/SHA256 更保险**；★ 定位三跳：搜参数名（命中太多）→ 按长度猜 MD5 → 搜 `MessageDigest`/`md5` → 搜用例 |
| 801 | `52pojie-1658810-某拉雅的登录协议复现.md` | `05b0fedffbb7329a694b9b7b2632d763` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 形态 12：`signature = SHA1( 全大写("ACCOUNT=..&FDSOTP=..&NONCE=..&PASSWORD=..&MOBILE-V1-PRODUCT-<64hex>") )`，且结果**按 76 字节分段、段间插 `\n`**；★★ **链路 5 步**（取 `fdsOtp` → 取 `nonce` → RSA 加密账号/密码（**加密结果也要按 76 字节分段换行**）→ 同 so 的 `RIdAPyTgEt` 拼串 SHA1 → 提交）；★ 手法：Frida 先 hook native 方法**打印入参**，再交给 unidbg；★★ **unidbg 死在系统调用 ⇒ 直接换一份别人跑通的脚本**（换完反而更快看到「多了一串东西」） |
| 802 | `52pojie-1691013-xx度灰app加密算法分析还原.md` | `4690f3d7eee74830c0a2ed4d3a68c6de` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **Go/cgo 编译的 so 调用约定**：导出仍是 `Java_..._encrypt`，但函数体只做「塞数组 → `crosscall2(fn, args, size, runtime)`」，★ **传入 size = 20 = 5×4 而数组只有 4 个元素 ⇒ 多出的槽位是返回值落点**（`*(_DWORD*)(a6+16) = v10`）；★★ **参数完全走栈、且同一参数被重复传多遍**；★ 判据三合一：`crosscall2`/`cgoexp_` 符号 + 函数体短得可疑 + size 大于参数数；★ 用的是 Go `crypto/cipher`（**Go 的 CFB 与 Python 默认结果不同**）⇒ 已并入 `02-native-dynamic-tracing.md` §7.5 |
| 803 | `52pojie-1707725-记一次iOS的google admob ads 广告请求协议逆向.md` | `227953d6cb5b091e1deef11b905bd04a` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **iOS + protobuf + 自定义 base64 的组合样本**：① 待加密数据是 **protobuf**；② ★★ **前置长度字节**：`arg1 = 0x6C`（108）即 protobuf 长度，**尾部用 `rand()` 补到 `0xF0`（240）**；③ ★★ **整体 MD5 后把这 16 字节摘要插到字节码起始位置**；④ 再经 `gad_bs` **12 轮异或**加密；⑤ 输出走 **`A-Za-z0-9-_` 的自定义 base64**；★ 手法：在 `ggi_ged 0x9C724`（加密入口）hook 打印入参/出参 ⇒ 已并入 `06-ios-app-reverse.md` §8 |
| 804 | `52pojie-1711668-安卓协议逆向之frida hook百例.md` | `fb2df12b2c392b0019613e7d48aa39a2` | 2026-09-25 | `android-app-reverse` | evolve | ★ **入门级 frida hook 三件套**（Python 壳 + `Java.perform` + `Java.use('类').方法.implementation`）：Java 侧壳负责 `frida.get_remote_device().attach(名字)` / `create_script` / `on('message')` / `sys.stdin.read()` 阻塞；★ JS 侧只做「打印入参 + 转发原实现 + 打印返回值」；★ 由 `encodeMD5` 一次 hook 同时拿到 `pwd` 与 `sign`（同一函数被调用两次）⇒ 判据：**同一 hook 点多次命中 ⇒ 分别对应不同参数**；★ `3DES` 的 key 用 `getDesKey` hook 取出 |
| 805 | `52pojie-1712752-安卓协议逆向之frida hook百例二.md` | `5369d5b0ff0d37e67a84046e870b5c13` | 2026-09-25 | `android-app-reverse` | evolve | ★★ **加壳样本的完整处置**：`ApkScan-PKID` 查壳 → **反射大师（Xposed 激活）写出 DEX** → 修复 Magic → jadx 搜 `Encrypt`；★ 判据：**「搜 `Encrypt` 命中太多」时按「和抓包字段同时出现」的那一处去筛**（源文找 hook 点花了 10 多分钟）；★ `RequestUtil.encodeDesMap(String,String,String)` **必须按 overload 写三参版本**；★ `aes` 结果与抓包不一致时**回头核对 key/iv 与填充**（源文第二次比对才成功） |
| 806 | `52pojie-1729121-Android 某东Sign与cipher解密.md` | `066ea0a952a411febfc942184c585e67` | 2026-09-25 | `android-app-reverse` | evolve | ★ 同站 **`Sign` 与 `cipher` 两个参数各一套算法**（`sign` 走摘要、`cipher` 走对称）⇒ 判据：**「一个接口里多个加密参数 ⇒ 逐个独立定位，不要假设共用」**；★ 作为形态库里的「多参数并存」实例登记（源文以截图为主，未给完整公式） |
| 807 | `52pojie-1752032-新人第一次某头像app接口请求sign分析.md` | `b93e56ec15b61d66b1374de7899b3cac` | 2026-09-25 | `android-app-reverse` | evolve | ★★ 形态 14：`sign = APP_KEY + formatUrlMap(排序拼接)`（与形态 8 同族）；★★★ **破局动作（可迁移）**：`signTopRequest(map, secret, method)` 里 `accessKeySecret` **搜 `setAccessKeySecret` 找不到调用点** ⇒ **改搜其它接口的固定关键字，命中 `setParams`** ⇒ 顺着 `formatUrlMap` 找到 `str = APP_KEY + sb.toString()`；★ 元方法：「**一条路走进死胡同要及时换突破口**」；★ 判据：**改任一参数服务端就报签名失败 ⇒ 全部参数都参与签名** |
| 808 | `52pojie-1752900-【iOS逆向与安全】某茅台App算法分析还原.md` | `186f6c15a7f1d2485514b0410730b9fd` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **iOS `CCCrypt` 判据表（本库首次系统化）**：`op`(0加密/1解密) / `alg`(0=AES128..6=Blowfish) / **`options`(0=None,1=ECB,2=CBC,3=CFB,4=CTR,5=OFB,6=CFB8，`|0x1000` 才是 PKCS7)** / `keyLength`（**`0x20`=AES-256**）；★★ **ECB 时 App 也会传非零 `iv` ⇒ 判模式只看 `options`**；★ 输出长度读 `dataOutMoved` 指向的 `size_t`；★ 闪退三件套：IDA 搜 `SVC` NOP / `exit(0)` 的条件变量交叉引用 / hook `embedded.mobileprovision` 返回 0；★ 定位：`frida-trace -m 「*[NSMutableURLRequest setHTTPBody:]」` 拿 body + **堆栈** ⇒ 反推到 `XXReserveRequest`；★ 已新增 `06-ios-app-reverse.md` §2/§3/§4 |
| 809 | `52pojie-1778152-找出某U3D游戏http接口中的关键sign算法，Java复写该算法.md` | `4dc4bc64440091eaf23ad570daf142f3` | 2026-09-25 | `android-app-reverse` | evolve | ★★ **IL2CPP（U3D）路线**：`libil2cpp.so` + `global-metadata.dat` → **il2cppdumper** dump 出 `dump.cs` → 在**接口列表里搜参数名**定位 `so 地址`（`0xD55700`）→ **把 `script.json` / `stringliteral.json` 喂进 IDA 脚本**才能看到函数名；★ 形态 1.4：明文**补齐到 8 的倍数** → `DES/ECB/NoPadding` → base64 → **strip 掉空白** → MD5；★ 首尾各拼一段字符串（在汇编窗口里读）；★ 动态：**在把拼接结果写进 `x0` 的指令处下断，读 `x0`** 得到真实拼接形态 |
| 810 | `52pojie-1793143-记一次资金盘APPsign算法破解.md` | `8d0c1bba17ad36e97bf10fce31eceb46` | 2026-09-25 | `android-app-reverse` | evolve | ★ **uni-app 载体判据**：`AndroidManifest.xml` 有经典 uniapp 特征 ⇒ **Java 层没意义**，解压后进 `assets/` 找 **`app-service.js`**（可能是压缩过的一行，先解压）；★ 定位：搜 `sign` 直接命中 `e.header["sign"] = l.default.getSign(Object.assign(e.data, e.params))`；★★ 复现策略：**JS 侧算法能扣就扣，Python 侧用 `PyExecJS` 走「一大拖一小」**（源文原话「用 python 复写那段加密算法，写了一段，卧槽感觉好烦」）；★ 已并入 `01-recon-and-carriers.md` §1.4 |
| 811 | `52pojie-1807699-某摄像头协议分析.md` | `605d0373f3c54a60d13380335724ffa6` | 2026-09-25 | `protocol-reverse` | evolve | ★★★ **多 magic 分派 + SIMD 异或 + PC 端条件断点**：① 同一 UDP 连接并存四种结构（`3acf872a` 控制 / `10cf872a` 分片 / `f0debc0a` 业务 / I·P·AAC 媒体 magic），且 **换 magic 就不认 key**（`10cf872a` 是 `f0debc0a` 的外层容器）；② ★★ **NEON 批量异或的翻译法**：`vdupq_n_s32` ⇒ `np.full(4,key)`、`.n128_u64[0/1]` **按小端拆 4 个 int32**、索引 `& 7` ⇒ **8 个 int32 轮转**；③ 校验字段是**密文的 CRC32**；④ ★★ **frida 对多线程失效 ⇒ 转 PC 端 `x64dbg` 在 `recvfrom` 下「buf == magic」条件断点**，再把 IDA 基址改成 x64dbg 的基址跳同一地址；⑤ 参数是 XML（`gP2P_pCmdTable` / `gP2P_pParaTable` 两张字符串表）⇒ 已新增 `references/multi-magic-and-compression.md` §1/§2/§6/§7 |
| 812 | `52pojie-1894045-Android逆向-小红书签名x-b3-traceid,x-xray-traceid.md` | `7fa62dd8d2d6824305a57a6472ee43ba` | 2026-09-25 | `reverse-knowledge` | evolve | ★★★ **「搜字段名直接命中 ⇒ 生成逻辑就在宿主层」**：小红书 Android 高版本新增 `x-b3-traceid` / `x-xray-traceid`，**jadx 搜字段名即命中**（不需要 frida），逻辑是「`x-xray-traceid` 为空则由 `String.format("%016x%016x", long0, long1)` 生成」；★★ **与 Web 侧 mnsv2/x-s 完全是两回事**（那边是 XXTEA + 自定义 Base64）；⇒ 判据：**先花 30 秒搜一次字段名，再决定用哪套工具**；⇒ 蓝图 `xhs-x-s` **结论级升级 v1.1.0**（新增两个 header、2 条 mutations）+ `web-reverse-algorithm/references/15-call-site-locating-playbook.md` §8.7 |
| 813 | `52pojie-1908960-静态分析逆向 B站 大积分新请求签名.md` | `3cc67f739f21f7023fe083d2b321595e` | 2026-09-25 | `reverse-knowledge` | evolve | ★★★ **「形参 ↔ 实参对位」定位法**：搜 `task_sign` 命中 3 处 ⇒ 挑带 `activity` 的那个（`completeTask` 的形参）⇒ 把三张表（调用点实参 `r5..r13` / 被调方形参 / **带 `@Query("…")` 注解的业务形参**）**逐位对齐** ⇒ **在读懂任何逻辑之前**就确定 `r10 ↔ task_sign`；★ 还原：`task_sign = md5(timestamp + '固定盐' + token).toString('hex')`；★★ **同一平台可以有第二条、更简单的签名**（与 wbi 的 `w_rid` 无关）；★★ 源文**故意不公开固定盐**（「這是固定值，我這裏不放出來了」）⇒ **本库保留其脱敏形态，不代为还原**；⇒ 蓝图 `bilibili-w-rid` v1.1.0 + `web-reverse-algorithm/references/12-login-and-account-params.md` §9.2 |
| 814 | `52pojie-1915146-某手游登录SDK接口解密.md` | `69ff0204cd78664c568c8b7f32e42054` | 2026-09-25 | `android-app-reverse` | evolve | ★ 形态 17：`data = AES(json)`（key 是常量） + `sign = MD5( 排序后 k=v&… + signKey )`；★★ **判据：`data` / `sign` 字段名太大众 ⇒ 改搜「业务常量」**（本例 `productCode` 不变量，一次命中定位）；★ 复现工具 `CyberChef` 一把梭 |
| 815 | `52pojie-1916453-记一次逆向分析某诈骗APP的网络行为和加解密.md` | `78876bf58f74548b77c94917da583c5c` | 2026-09-25 | `protocol-reverse` | evolve | ★★ **「地址池」结构（恶意 App 的常规分发）**：① 服务地址池存在**云存储**（阿里云 OSS）⇒ **抗 DDoS 也抗拦截**；② 按域名分流（`aliyuncs.com` 走解密分支、其它走明文分支）；③ 地址池响应体是 **AES-ECB（无 IV）密文**，解出来就是一堆 URL；④ ★★ 安全侧结论：**拦截要打在「节点」上，不是首页域名**；⇒ 已并入 `multi-magic-and-compression.md` §8 |
| 816 | `52pojie-2014961-分析某库登录逻辑以及协议复现.md` | `c8545d398a10287a6e2eaea444a02aaa` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **`AES/ECB/PKCS7` 的三段拼接 + 「两串原文」**：明文 `s = hea + a1 + en + a2 + end` 只用于算 `md5(s)`；**实际加密串是** `a1 + "-36cd479b6b5-" + a2 + "-36cd479b6b5-" + md5(s)` ⇒ 复现时**不要只拼一次**；★★★ **「同 key 复用」判据**：拿请求的 key 去解响应，**解出来是明文 ⇒ 直接复用，不再逆向响应侧**（源文原话）；★ 定位：`hook HashMap.put` 且**只对 key == "username" 打堆栈** ⇒ 直奔登录函数；★ 载体：360 加固（雷电脱壳）+ 请求头 `common` / `BaseInfo` |
| 817 | `52pojie-2022522-iTunes登录接口加密算法.md` | `5b398668be6a0cb4df2dd2413b01a5cf` | 2026-09-25 | `android-app-reverse` | skip | **已精读，判为 skip**：求助帖（作者自陈「刚接触逆向…在线求大佬帮助」），**未给出任何算法**。唯一可登记的事实：iTunes 登录第一步响应是 `<plist>` 里的 **`sign-sap-setup-cert` 证书链**（不是签名结果），第二步需要 **`sign-sap-setup-buffer`** 签名 —— **证书 ≠ 签名** 这条判据已并入 `06-ios-app-reverse.md` §9；其余按 gap 挂起 |
| 818 | `52pojie-2089842-默往聊天app加密协议分析，一个宣称是端到端加密的即时通讯应用.md` | `9e65755e85002bd33e6783bd6125cf05` | 2026-09-25 | `websocket-reverse` | evolve | ★★★ **「宣称端到端加密」的三问定性法**：① 应用层**有没有密钥协商**（E2EE 必须双方协商，**只有 TLS 不算**——TLS 的握手对象就是服务端）② 加密密钥**从哪来**（**硬编码 ⇒ 服务端与任何人可解 ⇒ 不是 E2EE**）③ 业务数据**是明是密**（本例**明文 JSON** 直接交 `WebSocketManager.send()`）；★ 取证顺序：脱壳（网易易盾，在线工具拿到 dex 即可，**不必修复**）→ 找传输层（WS）→ 只看 `AESHelper`（**密钥是常量还是协商来的**）→ 只看 `MakeMsg`（**业务字段是明文还是密文**）→ 搜协商协议名（X3DH/MLS/libsignal，**一个都没有 ⇒ 结论成立**）；⇒ 已新增 `references/cases/case-e2ee-claim-audit.md` |
| 819 | `52pojie-2091183-某抑云eapi加密分析.md` | `69c0ec48155f6578cb74f4ed760e6517` | 2026-09-25 | `desktop-client-reverse` | evolve | ★★ **类 Electron 客户端 + native DLL 的完整定位链**：① `cloudmusic.exe --remote-debugging-port=9222` 开远程调试 → Edge 附加 → 事件断点追到 `serialData(t) → window.channel.serialData(t)`；② ★★ **`window.channel.*` 是 native 桥**，导出表**没有** `serialData` ⇒ **改搜字符串**，命中一串 `if (名字) → 函数指针` 的分发器（`encodeAnonymousId2` / `encryptId` / `serialData` / `serialData2` / `deSerialData`）；③ ★★★ **成串的 base64 宽字符串常量不是密钥本身，而是「密钥的密文」**，必须过 `sub_xxx(key,out,&len)` 解出来；④ 随后出现 `AES_set_encrypt_key()` + 循环 `AES_encrypt()`（16 字节步进）= **OpenSSL 系 AES 指纹**；⑤ ★★ **坑：frida 附加多进程客户端必须先挑对进程**（本例 4 个进程，**附加内存占用最大的那个**，否则 hook 完全不触发）；⇒ 已并入 `desktop-client-reverse/references/desktop-runtime-surfaces.md` §9 |
| 820 | `52pojie-2119947-Discord Android 协议逆向实战 —— 从 APK 反编译到 Windows 协议 DLL 全流程.md` | `34883a2b952fcfe17e972f6c7d3c2977` | 2026-09-25 | `protocol-reverse` | evolve | ★★★ **事件网关型 WS + 连续流压缩 + 双 TLS 栈**：① 三层结构（WSS / **zstd-stream 连续流** / JSON `{op,d,t,s}` 信封）；② ★★★ **zstd 三坑**：单帧 API（`ZSTD_decompressDCtx`）只能解出第一帧（OP10 HELLO）、**每帧 reset 会让后续全乱** ⇒ 必须 `createDStream`+`decompressStream` 且**只初始化一次**；★★ **Client→Server 压缩一律 `4002` ⇒ 改发 TEXT 帧（不压缩）**；③ ★★★ **双 TLS 栈**：REST 走 Rust `rustls`（**不信任用户 CA** ⇒ 抓不到）、WSS 走 Java OkHttp（走代理）⇒ **别在 MITM 上耗，改 hook**；④ ★★ **RN 桥 hook 不到 `OP2 IDENTIFY` 的真因**：业务走的是 **OkHttp 的 WebSocket 接口**，绕过 RN 桥 ⇒ hook `okhttp3.WebSocket.send`；⑤ 握手时序 HELLO→IDENTIFY→READY→心跳（OP1/OP11）；⑥ `X-Super-Properties` 与 IDENTIFY 里的**双份指纹必须一致**（改 UA 不够）；⑦ 落成 DLL 时三个坑（跨线程回调崩溃 ⇒ 事件队列 / `volatile` 状态 / GET 不该带 `Content-Type`）；⇒ 已新增 `references/multi-magic-and-compression.md` §3/§4/§5 + `websocket-reverse/references/cases/case-discord-gateway-zstd.md` |
| 821 | `52pojie-589291-合富APP加密算法分析.md` | `b2b1757950e538d1c576f86a0a2ab1bb` | 2026-09-25 | `android-app-reverse` | evolve | ★ **双参数各一套**：`_s = MD5(参数校验串)`、`pwd = 3DES(密码, key)`（key `GDgLwwdK270Qj1w4`，藏在 smali 里 java 源码查看失败 ⇒ 直接读 smali）；★ 手法：AndroidKiller 搜参数名 → 定位到类 → **ddms 在 `Encrypt.MD5` / `encrypt3DES` 里插打印代码**，输出加密前的明文做对拍（「改 smali 插日志」是很早期的标准动作）；★ 判据：**key 在 smali 里可见但 java 反编译失败 ⇒ 去看 smali** |
| 822 | `52pojie-634074-PCQQ8.1通信协议分析0836登陆包.md` | `84bdfe63dd7264baa0943658fe64a02f` | 2026-09-25 | `protocol-reverse` | evolve | ★★ **老式 TLV 协议的「固定字段表」读法**：包头 `02` / 包尾 `03` / 版本 `36 43` / 命令 `08 36`（一次登录）/ **包序号可随机**（服务端不校验）；★ 结构：`QQ号HEX[4] + dwClientType + dwPubNo_Ver + 固定 + 长度前缀 + publickey + 长度前缀 + 随机 key + 密文`；★★ **`00 19` 后面正好跟 25 字节**（长度前缀与 payload 严格对齐）；★ 二次加密段：TEA，**密钥算法 = `md5((密码MD5 结果分割加空格) + "00 00 00 00" + (十六进制 QQ 号分割加空格))`**；★ `crc32` 段是 `00 14 <16 字节随机> <4 字节 crc32>` |
| 823 | `52pojie-654477-Android逆向之旅-分析某直播App的协议加密原理以及调用加密方法进行协议参数构造.md` | `31b125460be773e7eca9342e58dda239` | 2026-09-25 | `android-app-reverse` | evolve | ★★★ **「搜不到」四猜想 + 自建 App 调 so + 缺 set 前置**：① **全局搜不到时按四条逐一排除**（多 dex / 动态加载插件 `DexClassLoader` / 在 so 里（`Shift+F12` 看字符串）/ **根本是服务端返回的**）；配套三条工具纪律（**jadx 打开 apk 卡死 ⇒ 只打开 dex**；右键查找用法为空 ⇒ **去父类/接口再右键**；**Xposed hook 多 dex 必须先 hook `Application.attach` 拿 ClassLoader**）；② ★★★ **自建 App 调 so**（包名路径必须与 so 注册的完全一致）⇒ **调用成功但拿不到加密字段** ⇒ 真因是**缺 `set(Context, serverTime, startCode, runCode)` 这一步配置注入**（三个值来自 `/user/account/token_v2`）；③ 定位配置来源：**hook 配置类的构造方法抛异常打堆栈**；⇒ 已并入 `01-recon-and-carriers.md` §1.2/§1.3 与 `04-rpc-and-boundary.md` §2.4.1 |
| 824 | `52pojie-678959-手机淘宝登陆RSA算法分析(全网唯一).md` | `7d72a59f9a194897aa366f5ee7b9fe7f` | 2026-09-25 | `android-app-reverse` | evolve | ★★ **插件化 App 的载体判据**：`lib/` 目录里出现 **`.apk`**（不是 so）⇒ **直接 unzip 那个 apk** 就能拿到登录模块（`com.taobao.login4android`）；★ 定位线：搜 Manifest 里的 login 关键字 → 进模块 → **顺 UI 布局（`taobaouser_login_fragment`）反查 Fragment → 父类 → 点击事件 `loginInCurrentEnv()`** ⇒ 纯静态也能走到算法；★ 判据：**参数全塞进一个实体类再统一上传**（`IS` 类里放参数与算法）；⇒ 已并入 `01-recon-and-carriers.md` §1.4 |
| 825 | `52pojie-825868-[SmallCake]PCQQ8.6协议分析.md` | `b2d00334665b8ee98a3d52f9c9298a25` | 2026-09-25 | `protocol-reverse` | evolve | ★★ **PCQQ 全代际字段表**（8.6 版）：`0825`（首次接触）/ `0836`（一次登录）两包结构逐字段拆解；★ ECDH `publickey`（25 字节）+ 随机 `key`（16 字节）+ 密文，**解密密钥由 ECC 协商得出**；★ 固定段 `fix1`/`fix2`/`fix3` 与 `0836Key1` / `0836Key2` / `0836Token` / `0825Token` 的命名口径；★ **令牌跨包复用**（`0826令牌` 在 0836 里继续用）；★ `crc32Data` 段结构；⇒ 与 634074 共同构成 `multi-magic-and-compression.md` §5 的 TLV 读法 |
| 826 | `52pojie-851229-安卓逆向协议分析~实现脱机注册登录完成任务操作.md` | `a8472b1c8a28a161468138e3bde729d9` | 2026-09-25 | `android-app-reverse` | evolve | ★ 形态 16：**只有密码参与加密**（抓多次对比即可判定）⇒ `RSA(密码)`；★★ **公钥两条来源**：本地安装目录 XML（`SharedPreferences`）**或**抓包响应里的 `rsa_public_key`（两条都该试）；★ 定位：搜 `uuid` / `password` 命中太多 ⇒ **改搜接口路径 `/user/register`**；★ 脱机任务链：登录拿 token → 传 token 调任务接口（列表/领取/签到） |
| 827 | `52pojie-980940-关于某鱼TV直播源sign值计算的讨论.md` | `c824371fcd82ca1822852f8568244257` | 2026-09-25 | `web-reverse-algorithm` | evolve | ★★★ **「流传的公式必须实测，不能采信」**：源文把网上/论坛流传的 **五条** `sign` 公式**逐条实测，五条全错**（服务端一律 `鉴权失败`）——它们其实是**不同 `aid` / 不同 `client_sys`（PC vs ISO vs WP）的杂糅**；★★ 判据链：① **「sign 不是服务器返回值 ⇒ 应该在本地 js 算」**（这个是非题先答）；② 用断点发现「可能由三个 js 计算」⇒ **列全移动端 + PC 端两套脚本清单再逐个筛**，不要一上来抠整个 bundle；⇒ 已并入 `web-reverse-algorithm/references/12-login-and-account-params.md` §9.1 |

---

## 批次 B43 · 2026-09-26（第四十四次执行）

**取材口径**：**积压队列就地取材** —— 归档线第 43 轮尚未产出（`git log` 最新归档提交仍为「第四十二轮归档 48 篇」，远端已同步），故按任务定义转从 **483 篇待处理队列**中选**两个内聚簇 + 一个遗留收口**：
① **解析接口 / 播放地址族**（7 篇，2020–2023）—— 同时消化 B42 排的优先级 ②（`stream-drm-reverse` 的 W7 遗留）；
② **字体反爬族**（5 篇，2019–2024）—— 对应休眠中的 `web-font-obfuscation`（B1 建、此后未再演化）；
③ 由 ① 的跨切面结论外溢到 `web-reverse-algorithm`（JS 私有进制 / CryptoJS key 形态 / 环境守卫）与 `wsam-reverse`（wasm 黑盒探针）。

**产出**：**0 新建技能**（能力缺口 = 0）· **演化 4 技能 / 12 文件（10 改 + 2 新建）** · **1 个遗留收口**（W7「无脚本入口」）· **1 个新建可复跑工具**（`font_name_table_check.py`，`--selftest` 24/24）。

| # | 文件 | md5 | 处理时间 | 关联技能 | 变更类型 | 核心萃取 |
| --- | --- | --- | --- | --- | --- | --- |
| 828 | `52pojie-1044889-58同城房源模块数字字体解析java版.md` | `1e773801fff314237af87b3e6fc57a6f` | 2026-09-26 | `web-font-obfuscation` | evolve | ★ 58 同城数字字体（Java 版，2019）：**字体以 base64 内嵌在页面里**（`data:...base64,`）且**每次访问都不一样**（源文推测按时间生成）⇒ 必须**每请求实时建表**、不能缓存；★ 与 `52pojie-1323885`（2020 Python 版）构成**同站跨一年 delta**：两篇合起来才拼出「`glyphNNNNN` 尾部索引 = 真值 + 1」这条公式 ⇒ 已并入 `font-cmap-decode.md` §8.1（本批字体簇的版本 delta 证据） |
| 829 | `52pojie-1323885-58租房爬取已破解字体加密但是gui方面还不完善.md` | `cf3aed94a4d76af589777c80b6afdc6f` | 2026-09-26 | `web-font-obfuscation` | evolve | ★★ **本批字体簇的关键一篇**：给出换算**公式** `num = int(num[-2:]) - 1`（58 的 glyph 名尾索引 = 真值 + 1，`glyph00010 → 9`）—— 既有文档只写「`glyph00010` 稳定对应 9」，**没有公式就无法迁移**；★ 另给出「base64 内嵌字体 + 取文本后按 `ord(char)` 查 cmap」的完整写法；★ 已并入 `font-cmap-decode.md` §8.1，并登记**三个静默坑**：尾 2 位隐含「索引恰好两位数」、偏移是**站点私有**（先拿 2~3 个已知数字反推）、`uniXXXX` 形态不适用本条 |
| 830 | `52pojie-1400684-【原创】【Python】一个有意思的自定义字体.md` | `ddcca5c708d06dfacd1d946a43315128` | 2026-09-26 | `web-font-obfuscation` | evolve | ★★ 易车论坛**静态字体**（`yc-ft.woff` 内容不变）⇒ 一次性校准即可（模板 A），**不必找官方字体**；★★ 本库据它把「**`uniXXXX` 的 XXXX 就是真值码位**」这条判据钉死：源文原话「页面上的 `&#xe` 和后面的分号是没用的、字体文件里的 `uni` 也是没用的」⇒ **判据是码位本身**（`chr(int(name[3:], 16))`），与 §7 形态表一致；★ 源文那句「找不对的话就一点点自己写映射关系吧」= 本库「先跑形态判据再决定要不要校准」的动机 |
| 831 | `52pojie-1465475-起点字体加密学习（小白日常学习 欢迎交流 学习使我快乐）.md` | `34437c1438730124ee5dc1df9470c69a` | 2026-09-26 | `web-font-obfuscation` | evolve | ★★ 起点 `cmap.getBestCmap()`：码位是**十进制**（页面写 `&#100288;`）、glyph 名是**英文语义名**（`eight`/`period`）；★★★ **源文那张 `mima` 表有一处手误**：`'one':'8'`（应为 `'1'`）⇒ 与 `'eight'` **撞值**、表**非双射**，且**不报错**（`one` 出现处静默变 8）；★ 本库据此**新建** `scripts/font_name_table_check.py`（单射 / 值域 / 基数 / 索引换算四项机械校验，`--selftest` **24/24**，**把源文那张含 `one:'8'` 的表当负面夹具**，实跑报出「撞值 {'8': [eight, one]} + 值域缺 ['1']」） |
| 832 | `52pojie-1987108-超星网页字体解密.md` | `eda436b58882c9325108fd9ceefdba58` | 2026-09-26 | `web-font-obfuscation` | evolve | ★★★ **「轮廓被改写」族的完整实战（超星 vs 思源黑体）**：① 逐点哈希**全数失配**（源文实测 `下` 字两侧点集不同）；② 边界盒 `xMin/yMin/xMax/yMax` 四值拼接只定到 **约 90%**；③ **归一化点集 + DTW**（`similarity = 1/(1+平均欧氏距离)`）同一字实测 **0.9757**；★ DTW 是 O(N×M) 全配对 ⇒ **必须多进程并行**（源文朴素双循环跑不动）；★★ 本库给的**顺序修正**：**先试常数时间的边界盒、剩下的再上 DTW**（源文顺序相反、多花一轮）；⇒ 已并入 `font-cmap-decode.md` §8.2 |
| 833 | `52pojie-1305472-某某解析网站js静态逆向分析.md` | `627f5c5b55a2c3539956daba0f31e716` | 2026-09-26 | `stream-drm-reverse` | evolve | ★★★ **解析站「五参数链」这一代（2020）**：`url` 来自 DOM（`$('#sigu_url').val()`）、`key = base64(AES-CBC-**ZeroPadding**，key/iv 由 `Hex.parse` 给 16 字节原始密钥)`、`key2/key3 = btoa → 逐字符 (K - code).toString(32) 拼接`（K 分别为 251 / 218）、响应经 `sigu_decode`（**按对** `parseInt(pair,32)` → `258 - v` → `atob`）；★★★ **`Number.toString(32)` 不是标准 base32**（字符集 `0-9a-v`、**不定宽**、只有差值域 ⊂ `[32,1023)` 才恒 2 位 ⇒ 解码才能按定宽切；且**编码 251 与解码 258 是两个常数**）；★★★ **环境守卫三写法**（整段定义包白名单 / **函数体内把形参静默换成硬编码假值** / 反格式化「格式化即内存爆破」）⇒ 已并入 `playback-address-interfaces.md` §3C.7·§3C.9 + `web-reverse-algorithm` 的 `16-*` §2.5 与 `07-*` §2.1/§2.2 |
| 834 | `52pojie-1484457-瞎猫碰死耗子式破解某直播网站在线心跳签名.md` | `9cd305febf1c4727a6b2022953211c21` | 2026-09-26 | `wsam-reverse` | evolve | ★★★ **wasm 黑盒探针五步法**（不动 wasm 也能定算法）：正常参数⇒无随机/无公钥 · 删字段找无关项（`ua` 无影响）· 改顺序无影响⇒内部有格式化 · 改 value 有变化而**改 key 报错**⇒按固定键名取值 · ★★★ **整数数组入参 = 算法选择器 ⇒ 用 0..N 逐个枚举**；★★ 由 `[0]..[6]` 枚举出 `HMAC-MD5/SHA1/SHA256/SHA244(源文原写法，疑为 SHA-224)/SHA512/SHA384`、`[6]` 越界返回**非 hex**；★★ 本库补 **HMAC 输出长度反查表**并用真机 WebCrypto 实测 `SHA-1/256/384/512 → 40/64/96/128` hex（6/6）⇒ 已并入 `wasm-runtime-reproduction.md` §10.5；★ 源文自陈 wasm 静态/动态工具不成熟 ⇒ **黑盒探针是低成本先手** |
| 835 | `52pojie-1590803-手撕某解析网站，获取真实播放地址（保姆级讲解）.md` | `4639d368c6b2e9fd1e17166dfd323e97` | 2026-09-26 | `web-reverse-algorithm` | evolve | ★★★ **「乱序重组型密钥」**：页面把密钥拆成 `(id, text)` 对注入（`vod_` 前缀的 meta 属性），JS 按 `id` **数值升序**重排后拼 `text` ⇒ **不需要逆向算法，读页面 + 排序即可**；真机 Chrome 实算（源文两串 `pr='dQb7shnQWf'` / `pu='5318769240'`）得 **`fbQQWdhs7n`**；★ 判据：比较函数是 `a - b`（**数值**序），单字符 id 时字典序恰好同序 ⇒ **样本落在重合域**，多位 id 立刻分叉；★★★ **`MD5(x).toString()` 的 32 个 ASCII 字符被切两半当 IV 与 key**（`Utf8.parse` 各 16 字节；**误用 `Hex.parse` 只剩 8 字节且不报错**）；★ 反调试 `check(debugger)` 只能 hook `Function.prototype.constructor`；★ 「反格式化」登记为**单源**（源文未给检测代码）⇒ 已并入 `15-*` §8.8、`16-*` §4.5、`07-*` §2.1/§2.2 |
| 836 | `52pojie-1683783-一次逆向四个影视站点获取视频真实链接[更新B站].md` | `6cfd18c9226c4cce9d6b373437983d5b` | 2026-09-26 | `stream-drm-reverse` | evolve | ★★ **四站四配方对照**（同页正则取 key / key+iv 写死且密文有特征前缀 `t9GkQwf` / POST 三参数直接返直链 / `urls.slice(8)`→base64→`[8:-8]`）；★★★ **「直链是一次性的」**：几分钟失效、且**访问错一次立即失效**（源文原话）⇒ 纪律：**解密后在同一段代码里立刻用，不要先打印再取**；★ ts 切片被伪装成 **1 像素图片**（去图片头即播）；★ 定位纪律：**断点下在「返回结果」处**（先确认解出来的是不是想要的，再回头读加密逻辑）⇒ 已并入 `playback-address-interfaces.md` §3C.8，并**新建 W8 双段剥离族**（`key-wrapper-families.md` §6.7 + `key_wrapper.py w8`，含「剥离长度必须 ≡ 0 mod 4」与「8 字符 ≠ 8 字节」两条判据） |
| 837 | `52pojie-1713067-[新人试投]某云音乐前端播放器逆向.md` | `628e39da7d86436c1c68b3a2dc853af2` | 2026-09-26 | `stream-drm-reverse` | evolve | ★★★ **「不改算法」的注入式路线（反向取舍）**：hook XHR 改写响应 —— `detail` 接口改 `privileges` 骗过权限判定、`url/v1` 接口在 `onreadystatechange` 里请求第三方接口再回填；★★ **回填字段不止 `url`**（还需 `code`（填 200）/`br`/`level`/`type`（填 `mp3`）），**缺一即静默失败**（表现为弹 VIP 提示或不播放）；★ 取证纪律：**前端有请求缓存 ⇒ 每次抓包/调试前先刷新页面**（「点按钮没请求」不等于没有该请求）；★ 跨域用 `GM_xmlhttpRequest` 取 **`e.finalUrl`**（跟随 302 后的最终地址）⇒ 已新增 `playback-address-interfaces.md` §3D，并写明取舍「能还原算法就不要注入」 |
| 838 | `52pojie-1753800-某浪视频key解密分析.md` | `a8cf86dd86b291ed2c247bd50ad0b481` | 2026-09-26 | `stream-drm-reverse` | evolve | ★★★ **本批最重的一篇：B34 起挂九批的 W7 遗留在此收口**。源文给出**两对** `play_licenses` 输入/输出（`muZOqFO3H6hTiHKVa9t3xGvYcZub → 1d45c6d6841e4752`、`ouZ2xWyJTcYA5R2rAIoclAHESuYE+RnYAJcetgmpFOBYsELAwA== → 991b8fe94e58466fb9f6592f69076bff`）；本库把已登记的 W7 链套上去，**两对逐字节复现全中**（Python 与真机 Chrome V8 各跑一遍）⇒ 由此反推三条源文没给的结论：① **通用式是「掐掉输出数组首 1 末 1」而不是「取中间 16」**（37 字节样本给出 **32 字节 key**）；② **长度律 `len(key) = len(atob) − 5`**；③ oracle 从「可打印 ASCII」**升格为「16/32 个 hex 字符」**（漏 popcount 的产物 `0c24a4a562.c1411` **全可打印**，旧判据会**假绿**）；★ 顺带解释既有文档登记的「末位 27 vs 公式 49」矛盾**对还原无影响**（末位永远被丢）；★ 落地：`key_wrapper.py w7`（`--selftest` **96/96**，含 3 样本 + 3 条负面对照 + 2 条 `--enforce` 拒绝路径），`key-wrapper-families.md` §6.6 全节改写 |
| 839 | `52pojie-1823993-简化加密M3U8下载新方式（2023）.md` | `5831701e8e449fdeedd3b974bab825aa` | 2026-09-26 | `stream-drm-reverse` | evolve | ★ 最朴素那条路（2023 网课站，源文自陈「技术套路都是类似的」）：在 `*player.js` 里搜 `decrypt` / `decryptdata` → 断点看 `decryptdata` 里的 **`key`** → 16 字节 hex 直接喂下载器（不必扣算法）；★ 判据：**先试「搜 decrypt + 看 key」，不要一上来就扣算法**；★ 与 §3C.3 的「`key`/`iv` 是**解析站自己的常量**、与目标视频站无关」互证（跨站不用重逆）⇒ 已并入 `playback-address-interfaces.md` §3C.8 末尾 |

> 本批编号 828–839，共 12 篇；收口后台账总数 **839**，待处理 **471**。

---

## 批次 B44 · 2026-09-26（第四十五次执行）

**取材口径**：**归档线第 43 轮新供就地取材** —— 本轮上游归档提交 `088d832`（「第四十三轮归档 15 篇」，2026-09-26 10:29 落盘）把 15 篇高置信源文投进语料，而 B43 在其产出前 30 分钟已收工 ⇒ 这 15 篇**当时不在台账、也不在 B43 的取材范围**，故按任务定义「优先消化未验证文章」直接吃下这一整批。

**天然聚簇（上游已按主题定稿）**：
① **App/接口层签名与参数**（7 篇：`1542726`/`1735462`/`1796265`/`1979662`/`2114063`/`1715751`/`1178741`）—— 其中 6 篇落 `android-app-reverse`、云 API 那 1 篇落 `reverse-knowledge`；
② **协议链路 / 二进制封包**（5 篇：`1468888`/`2057659`/`522759`/`292460`/`735209`/`413044`）—— 落 `protocol-reverse`（**新建** `private-binary-packet-families.md`）；
③ **Protobuf / 游戏协议重建**（1 篇：`2057659` 兼跨）—— 落 `protobuf-reverse`（**新增方言 E**）；
④ **小程序 / 前端 JS 链路**（2 篇：`1335742`/`1189150`）—— 落 `miniprogram-reverse` 与 `android-app-reverse`。

**产出**：**0 新建技能**（能力缺口 = 0；15 篇全部落到既有技能的最近邻模块）· **演化 4 技能 / 10 文件（8 改 + 2 新建）** · **新建蓝图 1 条**（`tencent-cos-authorization`）· **真机 Chrome 校验 17/17 全绿**（三条可失败断言 + 4 条阴性对照）。

| # | 文件 | md5 | 处理时间 | 关联技能 | 变更类型 | 核心萃取 |
| --- | --- | --- | --- | --- | --- | --- |
| 840 | `52pojie-292460-飘零商业4.0封包加解密与静态数据加解密源码.md` | `f9c01ccb4844d754391ae7f7849e586f` | 2026-09-26 | `protocol-reverse` | evolve | ★★★ **「传输密钥变换」的原始出处（2014 易语言）**：源文原话「通过取**传输密钥 +1 后与 4 异或**，再用**分割出来的封包内容**异或，得到明文封包」；★ 本库据此**在真机 Chrome 里逐字节复现了源文那一对样本**（传输密钥 `{1,9,9,3,0,5,1,6}` ⇒ 派生 `[6,14,14,0,5,2,6,3]`，89 字节密文 → 明文 `54107028{[good-piao]}91643234{...}` 逐字节相同，17/17 全绿）⇒ 已并入**新建** `private-binary-packet-families.md` §2.1；★ 另登记「封包头本身也有校验 ⇒ patch 时**不要覆盖原版封包头**」 |
| 841 | `52pojie-522759-一次艰辛的算法分析-飘零4.0封包分析.md` | `df5998689046a15d29d4b9a1d7cf6732` | 2026-09-26 | `protocol-reverse` | evolve | ★★★ **本批方法论最值钱的一句：『倒着逆』**（源文原话「从结果开始往上推算，可以轻松的发现上面的循环，**只不过是得出一个相应的解密表罢了**……追算法的时候，如果没什么头绪的话，**可以倒着逆**」）⇒ 已并入 `private-binary-packet-families.md` §4.1；★ 与 `52pojie-292460` 互证 `+1 ^ 4` 变换在汇编中的位置（`call 0041873A`）；★ **静态数据用 256 字节置换表解密**（由循环交换两个位置生成，地址动态）；★ 验收纪律：**程序自加密 vs 自实现加密逐字节对拍** |
| 842 | `52pojie-735209-多玩歪歪的登录协议包分析.md` | `3ae3e72ec87383426df697f7da7a235a` | 2026-09-26 | `protocol-reverse` | evolve | ★★ **逐字段标注法 + 响应列表结构**（多玩歪歪 2018）：请求/响应逐字节标出 `长度前缀 + 数据`、固定分隔 `FF FF FF FF`、固定应答码 `C8 00`（=200）；★★ **响应里的列表结构 = 「个数 + 每条 (长度前缀 + 数据 + 尾部定长)」**（IP 个数 6 → 每条 `IP长度(0x0F) + IP字符串 + 4 字节 + 端口数量(2) + 端口(2 字节**小端**)`，`5B 00`=91 / `9B 1F`=8091 ⇒ 同一服务高低两端口）；★ 已并入 `private-binary-packet-families.md` §3.2 |
| 843 | `52pojie-413044-OD下断 解密 石器时代8.0 客户端与服务器 通信封包，写给像我一样的新手[持续更新].md` | `e1c820fac93c10585d9fce6ce53b49e8` | 2026-09-26 | `protocol-reverse` | evolve | ★★ **封包头存的是「本包自身各段的偏移」**（石器时代 8.0）：`封包头(前 6 字节存偏移，且这 6 字节本身参与加解密) → 封包加密段(偏移 + **按偏移排序**封包段) → 封包正文(一级加密)`；★ 判据：**「同一逻辑字段在不同样本里的字节顺序会变」⇒ 包内存在排列/索引表**，不要按出现顺序直读；★ 定位三手段：栈回溯（`001872b8`/`001632e4`/`001832b8` 三个栈）+ WPE 单字段重放 + 明文/密文逐字节 diff ⇒ 已并入 `private-binary-packet-families.md` §3.1·§4.2 |
| 844 | `52pojie-1468888-赛尔号：通信协议逆向与模拟&中间人攻击窃取登录凭证.md` | `2094dbb1f526454c909a425409026b1d` | 2026-09-26 | `protocol-reverse` | evolve | ★★★ **本批最重的一篇（赛尔号 Flash 客户端，源文 2400+ 行）**：① ★★★ **「长度字段是唯一允许明文的一段」的设计必然性**（源文原话「如果封包长度也加密了，那么两条加密封包之间没有明显的分界线」）⇒ 成为新文件 §1 的总判据；② ★★★ **会话中换钥**：默认密钥 `!crAckmE4nOthIng:-)`（**命令号 < 1000 的封包直接明文**，因登录前 `RobotCoreDLL.swf` 未加载），收到 `LOGIN_IN` 后改为 `MD5(str(封包明文最后 4 字节 uint ^ 米米号))[:10]`（`setEncryptKeyStringArr` 的星号包裹是**防字符串搜索**、不是算法）；③ ★★★ **CrossBridge/FlasCC 产物判据**（AS 里 `li32`/`si32`/`ESP`/Itanium 修饰名 ⇒ C 编译来的）与处置「**读解密侧求逆**、别读加密侧」；④ 粘包/断包：自维护缓冲区 + 4 字节长度字段；⑤ **加密后长度 = 明文长度 + 1** ⇒ 已并入 `private-binary-packet-families.md` §1·§2.2·§4.3 |
| 845 | `52pojie-2057659-[修正版]某修仙肉鸽游戏协议逆向分析折腾记录.md` | `96a7397437c3a1d8499b30a2a4ea6398` | 2026-09-26 | `protobuf-reverse + protocol-reverse` | evolve | ★★★ **本批唯一新增「方言 E」**：Unity IL2CPP + XLua 手游的 schema 是预编译 `.bytes`（`Pb/base.bytes`/`Pb/CSProto.bytes`），但**启动时必然 `pb.load()` ⇒ schema 已活在内存里** ⇒ 用 `lua-protobuf` 的 **`pb.types()` / `pb.fields()` 反射**直接 dump 出全量 schema，再按 package 分组生成 `.proto`（含「同包去前缀 / 跨包留相对路径」的归一化）⇒ 已并入 `dialect-matrix.md` §4.1（**判据：语言是 Lua / 打包成 `.bytes` / 代码里 `pb.load` ⇒ 方言 E，不要逆 `.bytes` 二进制格式**）；★ 帧侧：**收发两侧长度字段宽度不同**（发 2 字节 / 收 4 字节，`PackSendHeaderLength` vs `PackRecvHeaderLength`）+ **只有部分协议加密**（`EncryptFilter:needEncrypt(cmd)`）⇒ 已并入 `framing-and-transport.md` §2b；★ **资源体 XOR 解密密钥 = AssetBundle 文件名**（解出来 `startswith(b'UnityFS')` 即命中）⇒ 新文件 §2.3 |
| 846 | `52pojie-1542726-某东到家 app so signkeyV1 参数分析.md` | `e6bf70e000d084fdb1f2ff23f41a0469` | 2026-09-26 | `android-app-reverse` | evolve | ★★ **「64 位 hex ⇒ 先猜 HMAC-SHA256 / SHA-256」**（京东到家 `signKeyV1`）；★★★ **目标用标准库时的现成 hook 锚点**：符号未 strip ⇒ 直接按名挂 —— **`HMAC_Init_ex` 的 `args[1]` 是 key**、**`HMAC_Update` 的 `args[1]` 是消息**、`hmac_sha256` 是一步式封装 ⇒ 已并入 `02-native-dynamic-tracing.md` §4.1（并补「§4 自己划边界 vs §4.1 用作者划好的边界」的成本序）；★ 形态登记见 `03-signature-and-packet-families.md` §1 形态 19 |
| 847 | `52pojie-1735462-某上市导购平台Android APP签到sign逆向分析并使用Python复写加密算法.md` | `e859386584b4c31ee1612eefe1267919` | 2026-09-26 | `android-app-reverse` | evolve | ★★★ **「三处 hook 分工」定位范式**（什么值得买签到）：① 入口方法拿 map + sign（确认是不是这个接口）② `ZDMKeyUtil` 拿拼在末尾的 **key** ③ MD5 类拿**加密前明文 + 密文**；★ 判据：**搜 `sign` 命中一堆第三方 SDK ⇒ 改看「混淆类名（`e.e.b.a.n.a.a` 式）+ 方法体内出现目标接口参数名」的那一个**；★★ 明文口径 `f=android&sk=…&time=<秒>000&token=…&v=9.9.12&weixin=1&key=<固定>` → **MD5 大写**；★ 同文登记「App 端无滑块、Web 端有滑块 ⇒ 同一个业务接口两条风控路径」⇒ 已并入 `03-signature-and-packet-families.md` §1 形态 20 |
| 848 | `52pojie-1796265-iOS某车之家登录参数_sign&userpwd分析.md` | `5df1500f357c1ef5ea94bd932a8a784c` | 2026-09-26 | `android-app-reverse` | evolve | ★★★ **iOS 侧 `CC_MD5` hook 定位法**（汽车之家）：一次 hook 同时**打印入参明文 + `Thread.backtrace(Backtracer.ACCURATE)`** ⇒ 源文靠「`userpwd` 那条 md5 日志**下面**的另一条 md5」认出 `_sign`，堆栈直接给出 `+[AHLoginServicesMD5 MD5WithString:]` 与业务方法名；★★ **待签串头尾同一个哨兵串包裹**（`@7U$aPOE@$` 开头**和**结尾各一次，只拼一遍必错）；★★ 归因纪律：**`validcode` 必须回抓包取原文**（源文拿本地生成的值拼串 ⇒ 对不上）；★ `otool -l | grep crypt` 判 `cryptid`（1 未脱壳 / 0 已脱壳）+ `frida-ios-dump` ⇒ 已并入 `06-ios-app-reverse.md` §3b 与 `03` §1 形态 21 |
| 849 | `52pojie-1979662-某一卡通app免密盗刷，Sign值算法以及支付二维码计算方法分析.md` | `538a493009afd131fe26c49f5142ee7c` | 2026-09-26 | `android-app-reverse` | evolve | ★★★ **「排序的是 key 名数组，取 value 用的是 key 在原数组里的下标」**（一卡通 `sign`）：`strArr3`（key）被 `Arrays.sort`，再用排序后的 key 回 **原序数组**找下标取 `strArr2[i]` ⇒ **复现时不能把 `(k,v)` 一起排序**，否则 value 与 key 错配（这是「看起来是标准 `k=v` 排序 MD5、却怎么都算不对」的根因）；★ 固定盐硬编码在拼接末尾；★★ **双段凭证族**：`QRCODE = DES3(明文) + 「,」 + eqLength(明文)`，后半是**自定义 CRC-16**（256 项站点私有表）⇒ 本库在**真机 Chrome 里复现出 `12351`**（与源文一致，含改 1 bit 表项的阴性对照）；★ **24 字节 DES 密钥是拼的**（`bArr2[0:16]=bArr[0:16]`、`bArr2[16:24]=bArr[0:8]`，前 8 字节复用）；★ 「函数名会骗人」：按 `GetQRCode` 找到的是**扫码**函数（节点写的是 `Scan`）⇒ 靠交叉引用找另一处同名调用 ⇒ 已并入 `03` §1 形态 22 与 §5.1 |
| 850 | `52pojie-2114063-RR视频.apk客户端登录协议分析.md` | `e35bb2d1dd03a4be9d15fa9e248a54da` | 2026-09-26 | `android-app-reverse` | evolve | ★★ **同一接口两处 sig、密钥相同**（人人网 RR）：对签名函数做**交叉引用** ⇒ 四处调用点里「实际序列化/传输入口」那个才是发出去的 sig；★★★ **资源混淆（AndResGuard 等）后 `res/values/strings.xml` 被物理删除、常量并入二进制 `resources.arsc`** ⇒ 用 Jadx 打开会**虚拟重建** `strings.xml`，在「资源管理器」里搜 key 名即可拿到 `secretkey`（**别在 APK 里搜 `strings.xml`**，会搜不到）；★ **RSA 公钥取不到时降级 MD5**（`dPl != null ? RSA : Md5`，含 `lAT` 标记位）⇒ 已并入 `03` §1 形态 23 与 §1.1 盐来源表 |
| 851 | `52pojie-1715751-安卓某app_sign逆向分析过程.md` | `b5de44a0eed82eaa6b405d2cd5be86ef` | 2026-09-26 | `android-app-reverse` | evolve | ★★ **动态注册 + `VX+XXX` 假指针的处置**：`JNI_OnLoad` 里 `VX` 是未修复的指针 ⇒ **点上去按 `Y` 修复**，再顺 `JNINativeMethod` 表定到目标函数；★★ **「小写转大写紧跟 MD5」= 输出是大写 MD5**（`sub_F39A8` 里 `toupper` 循环 + `sub_4BBE4` 哈希）；★ **frida 特征被检测 ⇒ 闪退**（源文用去特征版 strongR-frida ），与 `2057659` 的 `libmsaoaidsec.so` 三方案（线程暂停 / 函数 patch / 去特征工具）互相印证；★ hook 骨架：`get_func_addr` 里 **arm32 要 `+1`**（Thumb） |
| 852 | `52pojie-1178741-TX云Authorization签名算法，更详细的说明，请求改写JS.md` | `a6186a11526d5c5eddf20caa71ca6e63` | 2026-09-26 | `reverse-knowledge` | evolve | ★★★ **公开文档型签名规律入库**：腾讯云 COS `Authorization`（`q-sign-algorithm=sha1`）的八步链 = `KeyTime → SignKey = HMAC-SHA1(SecretKey, KeyTime) → UrlParamList/HttpParameters → HeaderList/HttpHeaders → HttpString = method\nuri\nparams\nheaders\n → StringToSign = sha1\nKeyTime\nSHA1(HttpString)\n → Signature = HMAC-SHA1(SignKey, StringToSign)`；★ **本库新建蓝图 `tencent-cos-authorization`**（metadata/parts/mutations/workflow 四件套 + `index.json` 登记，lint error 0）；★★ **两条口径判据**：① `HttpString` **结尾必须有换行**、空段也要留（`get\n/exampleobject\n\n\n`）；② **URL 参数不要重复 UrlEncode**（请求行还会再编一次）；★ 本库在**真机 Chrome 用 WebCrypto 与 Python `hmac` 跨实现对拍**：`HttpParameters`/`HttpHeaders` 与源文示例逐字一致、`SignKey`/`Signature` 两侧完全相同（17/17）；★ **`q-key-time` 漏填报「参数缺失」而非「签名不匹配」**（易误判成算法错） |
| 853 | `52pojie-1335742-PC下frida注入30行代码让你拥有百万小程序未加密代码包.md` | `73bd1ad012f87d3c27c3f538695beace` | 2026-09-26 | `miniprogram-reverse` | evolve | ★★★ **「不解密」路线：hook `EncryptBufToFile` 直接拿未加密缓冲**（PC 微信 3.0.0.47）⇒ 不必实现 `V1MMWX` 算法：`onEnter` 记 `ptr(args[1]).readPointer().readAnsiString()`（**指针的指针**）与 `Memory.readByteArray(args[2], args[3])`，`onLeave` 直接落盘；★★★ **别写死偏移**（前人脚本 `baseAddr.add(0x1800F)` 一升级即失效）⇒ 改 `Module.findExportByName('WeChatAppHost.dll','EncryptBufToFile')`；★ 前置纪律：**先点开小程序面板**（否则 `WeChatAppHost.dll` 未加载、`findBaseAddress` 为 null）+ **注入前清空 `WeChat Files\Applet\`** ⇒ 已并入 `unpack-and-decrypt.md` §1b，并在 §0 平台表标注该旁路 |
| 854 | `52pojie-1189150-仅需10行代码，使用 Frida 完成 AutoJS脚本解密.md` | `416aa09517f345886befee35bc394d87` | 2026-09-26 | `android-app-reverse` | evolve | ★★★ **「只调用一次的解密」必须 spawn 注入**（AutoJS 脚本解密，源文原话「原 APK 的解密函数只在程序刚开始启动的时候调用。如果 APK 启动之后再注入脚本，是获取不到……的」）⇒ 必须 `frida -U -l hook.js -f <pkg> --no-pause`，**attach 模式 100% 空手而归且不报错**（表现为「hook 装了但没打印」）；★★ **必须精确匹配重载**：`Cls.$init.overload('java.lang.String','java.lang.String').implementation = ...`（源文「看上去有 2 个构造函数……进行了重载」），只写 `$init.implementation` 会命中错的那一个；★ hook 点 = `com.stardust.autojs.script.StringScriptSource` 的 `(String,String)` 重载，第 2 个参数即**解密后源码** ⇒ 已并入 `02-native-dynamic-tracing.md` §3b（与 `1335742` 合起来是「**hook 解密产物而非还原算法**」的同一范式） |

> 本批编号 840–854，共 15 篇；收口后台账总数 **854**，待处理 **471**。
